...
#define SCRIVI_PLUGIN_PYTHON_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_PLUGIN_PYTHON, ScriviPluginPythonPrivate))

static GObjectClass *parent_class;

struct _ScriviPluginPythonPrivate 
{
	PyObject *instance;
};

static void	 scrivi_plugin_python_class_init		(ScriviPluginPythonClass *klass);
static void	 scrivi_plugin_python_init 		(ScriviPluginPython      *plugin);

G_DEFINE_TYPE (ScriviPluginPython, scrivi_plugin_python, SCRIVI_TYPE_PLUGIN)

static PyObject *
call_python_method (ScriviPluginPythonPrivate *priv,
		    ScriviWindow		     *window,
		    gchar		     *method)
{
		...
		ScriviWindow *window)
{
	...
	ScriviPluginPythonPrivate *priv = SCRIVI_PLUGIN_PYTHON(plugin)->priv;
	...
	{		
		...
		SCRIVI_PLUGIN_CLASS (parent_class)->update_ui (plugin, window);
		 ...
		 ScriviWindow *window)
{
	...
	{		
		...
		SCRIVI_PLUGIN_CLASS (parent_class)->deactivate (plugin, window);
	       ...
	       ScriviWindow *window)
{
	...
	if (PyObject_HasAttrString (priv->instance, "activate"))
	{
		...
		SCRIVI_PLUGIN_CLASS (parent_class)->activate (plugin, window);
...
impl_create_configure_dialog (ScriviPlugin *plugin)
{
	...
	if (PyObject_HasAttrString (priv->instance, "create_configure_dialog"))
	{
		...
		ret = SCRIVI_PLUGIN_CLASS (parent_class)->create_configure_dialog (plugin);
...
static gboolean
impl_is_configurable (ScriviPlugin *plugin)
{
	PyGILState_STATE state = pyg_gil_state_ensure ();
	ScriviPluginPythonPrivate *priv = SCRIVI_PLUGIN_PYTHON(plugin)->priv;
...
void
_scrivi_plugin_python_set_instance (ScriviPluginPython *plugin, 
				  PyObject 	    *instance)
{
	PyGILState_STATE state = pyg_gil_state_ensure ();
...
PyObject *
_scrivi_plugin_python_get_instance (ScriviPluginPython *plugin)
{
	return plugin->priv->instance;
}

static void
scrivi_plugin_python_init (ScriviPluginPython *plugin)
{
	plugin->priv = SCRIVI_PLUGIN_PYTHON_GET_PRIVATE(plugin);

	scrivi_debug_message (DEBUG_PLUGINS, "Creating Python plugin instance");
	plugin->priv->instance = 0;
}

static void
scrivi_plugin_python_finalize (GObject *object)
{
	PyGILState_STATE state;
	
	scrivi_debug_message (DEBUG_PLUGINS, "Finalizing Python plugin instance");

	state = pyg_gil_state_ensure ();
	Py_XDECREF (SCRIVI_PLUGIN_PYTHON(object)->priv->instance);
	pyg_gil_state_release (state);
...
static void
scrivi_plugin_python_class_init (ScriviPluginPythonClass *klass)
{
	ScriviPluginClass *plugin_class = SCRIVI_PLUGIN_CLASS (klass);

	parent_class = g_type_class_peek_parent (klass);

	g_type_class_add_private (klass, sizeof (ScriviPluginPythonPrivate));
	G_OBJECT_CLASS (klass)->finalize = scrivi_plugin_python_finalize;
