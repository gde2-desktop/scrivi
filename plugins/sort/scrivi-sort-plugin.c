 ...
 * Ported to Scrivi2 by Lee Mallabone <gde2@fonicmonkey.net>
 *
...
#define SCRIVI_SORT_PLUGIN_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_SORT_PLUGIN, ScriviSortPluginPrivate))

/* Key in case the plugin ever needs any settings. */
#define SORT_BASE_KEY "/apps/scrivi/plugins/sort"

#define WINDOW_DATA_KEY "ScriviSortPluginWindowData"
#define MENU_PATH "/MenuBar/EditMenu/EditOps_6"

SCRIVI_PLUGIN_REGISTER_TYPE(ScriviSortPlugin, scrivi_sort_plugin)
...
static void
get_current_selection (ScriviWindow *window, SortDialog *dialog)
{
	ScriviDocument *doc;
...
get_sort_dialog (ActionData *action_data)
{
	...
	if (!ret)
	{
		...
		scrivi_warning (GTK_WINDOW (action_data->window),
			       "%s", err_message);
...
sort_real (SortDialog *dialog)
{
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Building list...");
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Sort list...");
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Rebuilding document...");
	       ...
	       ScriviWindow *window)
{
	...
	data->ui_action_group = gtk_action_group_new ("ScriviSortPluginActions");
...
static void
scrivi_sort_plugin_init (ScriviSortPlugin *plugin)
{
	scrivi_debug_message (DEBUG_PLUGINS, "ScriviSortPlugin initializing");
}

static void
scrivi_sort_plugin_finalize (GObject *object)
{
	scrivi_debug_message (DEBUG_PLUGINS, "ScriviSortPlugin finalizing");

	G_OBJECT_CLASS (scrivi_sort_plugin_parent_class)->finalize (object);
}

static void
scrivi_sort_plugin_class_init (ScriviSortPluginClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	ScriviPluginClass *plugin_class = SCRIVI_PLUGIN_CLASS (klass);

	object_class->finalize = scrivi_sort_plugin_finalize;
