...
#include "scrivi-docinfo-plugin.h"
...
#define WINDOW_DATA_KEY "ScriviDocInfoWindowData"
#define MENU_PATH "/MenuBar/ToolsMenu/ToolsOps_2"

SCRIVI_PLUGIN_REGISTER_TYPE(ScriviDocInfoPlugin, scrivi_docinfo_plugin)
...
get_docinfo_dialog (ScriviWindow *window,
		    WindowData	*data)
{
	DocInfoDialog *dialog;
	gchar *data_dir;
	...
	data_dir = scrivi_plugin_get_data_dir (data->plugin);
	...
	if (!ret)
	{
		...
		scrivi_warning (GTK_WINDOW (window), "%s", err_message);
...
static void 
calculate_info (ScriviDocument *doc,
		GtkTextIter   *start,
		GtkTextIter   *end, 
		gint          *chars,
		gint          *words,
...
static void
docinfo_real (ScriviDocument *doc,
	      DocInfoDialog *dialog)
{
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Chars: %d", chars);
	scrivi_debug_message (DEBUG_PLUGINS, "Lines: %d", lines);
	scrivi_debug_message (DEBUG_PLUGINS, "Words: %d", words);
	scrivi_debug_message (DEBUG_PLUGINS, "Chars non-space: %d", chars - white_chars);
	scrivi_debug_message (DEBUG_PLUGINS, "Bytes: %d", bytes);
...
static void
selectioninfo_real (ScriviDocument *doc,
		    DocInfoDialog *dialog)
{
	gboolean sel;
	GtkTextIter start, end;
	...
	if (sel)
	{
		...
		scrivi_debug_message (DEBUG_PLUGINS, "Selected chars: %d", chars);
		scrivi_debug_message (DEBUG_PLUGINS, "Selected lines: %d", lines);
		scrivi_debug_message (DEBUG_PLUGINS, "Selected words: %d", words);
		scrivi_debug_message (DEBUG_PLUGINS, "Selected chars non-space: %d", chars - white_chars);
		scrivi_debug_message (DEBUG_PLUGINS, "Selected bytes: %d", bytes);
	...
	else
	{
		...
		scrivi_debug_message (DEBUG_PLUGINS, "Selection empty");
	}
			    ...
			    ScriviWindow *window)
{
	...
	switch (res_id)
	{
		...
		{
			scrivi_debug_message (DEBUG_PLUGINS, "GTK_RESPONSE_CLOSE");
...
static void
scrivi_docinfo_plugin_init (ScriviDocInfoPlugin *plugin)
{
	scrivi_debug_message (DEBUG_PLUGINS, "ScriviDocInfoPlugin initializing");
}

static void
scrivi_docinfo_plugin_finalize (GObject *object)
{
	scrivi_debug_message (DEBUG_PLUGINS, "ScriviDocInfoPlugin finalizing");

	G_OBJECT_CLASS (scrivi_docinfo_plugin_parent_class)->finalize (object);
}
	       ...
	       ScriviWindow *window)
{
	...
	data->dialog = NULL;
	data->ui_action_group = gtk_action_group_new ("ScriviDocInfoPluginActions");
...
static void
scrivi_docinfo_plugin_class_init (ScriviDocInfoPluginClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	ScriviPluginClass *plugin_class = SCRIVI_PLUGIN_CLASS (klass);

	object_class->finalize = scrivi_docinfo_plugin_finalize;
