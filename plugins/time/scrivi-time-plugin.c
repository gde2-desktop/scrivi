...
#define SCRIVI_TIME_PLUGIN_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
					      SCRIVI_TYPE_TIME_PLUGIN, \
					      ScriviTimePluginPrivate))

#define WINDOW_DATA_KEY "ScriviTimePluginWindowData"
#define MENU_PATH "/MenuBar/EditMenu/EditOps_4"

/* GSettings keys */
#define TIME_SCHEMA			"org.gde2.scrivi.plugins.time"
...
} ScriviTimePluginPromptType;

struct _ScriviTimePluginPrivate
{
	GSettings *settings;
};

SCRIVI_PLUGIN_REGISTER_TYPE(ScriviTimePlugin, scrivi_time_plugin)
...
static void
scrivi_time_plugin_init (ScriviTimePlugin *plugin)
{
	scrivi_debug_message (DEBUG_PLUGINS, "ScriviTimePlugin initializing");

	plugin->priv = SCRIVI_TIME_PLUGIN_GET_PRIVATE (plugin);

	plugin->priv->settings = g_settings_new (TIME_SCHEMA);
}

static void
scrivi_time_plugin_finalize (GObject *object)
{
	ScriviTimePlugin *plugin = SCRIVI_TIME_PLUGIN (object);

	scrivi_debug_message (DEBUG_PLUGINS, "ScriviTimePlugin finalizing");

	g_object_unref (G_OBJECT (plugin->priv->settings));

	G_OBJECT_CLASS (scrivi_time_plugin_parent_class)->finalize (object);
}
		...
		WindowData   *data)
{
	...
	scrivi_debug_message (DEBUG_PLUGINS, "View: %p", view);
	       ...
	       ScriviWindow *window)
{
	...
	action_data->plugin = SCRIVI_TIME_PLUGIN (plugin);
	action_data->window = window;

	manager = scrivi_window_get_ui_manager (window);

	data->action_group = gtk_action_group_new ("ScriviTimePluginActions");
...
static ScriviTimePluginPromptType
get_prompt_type (ScriviTimePlugin *plugin)
{
	ScriviTimePluginPromptType prompt_type;
...
static void
set_prompt_type (ScriviTimePlugin           *plugin,
		 ScriviTimePluginPromptType  prompt_type)
{
...
static gchar *
get_selected_format (ScriviTimePlugin *plugin)
{
	gchar *sel_format;
...
static void
set_selected_format (ScriviTimePlugin *plugin,
		     const gchar     *format)
{
...
static gchar *
get_custom_format (ScriviTimePlugin *plugin)
{
	gchar *format;
...
static void
set_custom_format (ScriviTimePlugin *plugin,
		   const gchar     *format)
{
	g_return_if_fail (format != NULL);
	      ...
	      ScriviTimePlugin *plugin)
{
	...
	while (formats[i] != NULL)
	{
		...
		scrivi_debug_message (DEBUG_PLUGINS, "%d : %s", i, str);
...
get_format_from_list (GtkWidget *listview)
{
	...
	if (gtk_tree_selection_get_selected (selection, NULL, &iter))
	{
		...
		scrivi_debug_message (DEBUG_PLUGINS, "Sel value: %d", selected_value);
...
get_configure_dialog (ScriviTimePlugin *plugin)
{
	...
	ui_file = g_build_filename (data_dir, "scrivi-time-setup-dialog.ui", NULL);
...
{
	scrivi_debug_message (DEBUG_PLUGINS, "Insert: %s", the_time);
			  ...
			  ScriviTimePlugin           *plugin)
{
	...
	data_dir = scrivi_plugin_get_data_dir (SCRIVI_PLUGIN (plugin));
	ui_file = g_build_filename (data_dir, "scrivi-time-dialog.ui", NULL);
	ret = scrivi_utils_get_ui_objects (ui_file,
					  NULL,
					  &error_widget,
	...
	switch (response)
	{
		case GTK_RESPONSE_HELP:
		{
			...
			scrivi_help_display (GTK_WINDOW (widget),
					    NULL,
					    "scrivi-insert-date-time-plugin");
			break;
		}
	 ...
	 ActionData *data)
{
	...
	buffer = GTK_TEXT_BUFFER (scrivi_window_get_active_document (data->window));
...
ok_button_pressed (TimeConfigureDialog *dialog)
{
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Sel: %d", sel_format);
}
			      ...
			      TimeConfigureDialog *dialog)
{
	switch (response)
	{
		case GTK_RESPONSE_HELP:
		{
			...
			scrivi_help_display (GTK_WINDOW (widget),
					    NULL,
					    "scrivi-insert-date-time-plugin#scrivi-date-time-configure");
			break;
		}
		case GTK_RESPONSE_OK:
		{
...
impl_create_configure_dialog (ScriviPlugin *plugin)
{
	...
	dialog = get_configure_dialog (SCRIVI_TIME_PLUGIN (plugin));

	dialog->plugin = SCRIVI_TIME_PLUGIN (plugin);
...
static void
scrivi_time_plugin_class_init (ScriviTimePluginClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	ScriviPluginClass *plugin_class = SCRIVI_PLUGIN_CLASS (klass);

	object_class->finalize = scrivi_time_plugin_finalize;
	...
	g_type_class_add_private (object_class, sizeof (ScriviTimePluginPrivate));
}
