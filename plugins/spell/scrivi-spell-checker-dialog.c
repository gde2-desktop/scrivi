...
#include "scrivi-spell-marshal.h"

struct _ScriviSpellCheckerDialog 
{
	GtkWindow parent_instance;

	ScriviSpellChecker 	*spell_checker;
...
static void	update_suggestions_list_model 			(ScriviSpellCheckerDialog *dlg, 
								 GSList *suggestions);
...
G_DEFINE_TYPE(ScriviSpellCheckerDialog, scrivi_spell_checker_dialog, GTK_TYPE_WINDOW)

static void
scrivi_spell_checker_dialog_dispose (GObject *object)
{
	ScriviSpellCheckerDialog *dlg = SCRIVI_SPELL_CHECKER_DIALOG (object);
	...
	G_OBJECT_CLASS (scrivi_spell_checker_dialog_parent_class)->dispose (object);
}

static void
scrivi_spell_checker_dialog_class_init (ScriviSpellCheckerDialogClass * klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);

	object_class->dispose = scrivi_spell_checker_dialog_dispose;

	signals[IGNORE] = 
		g_signal_new ("ignore",
 			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviSpellCheckerDialogClass, ignore),
			      NULL, NULL,
			      scrivi_marshal_VOID__STRING,
			      G_TYPE_NONE, 
			      1, 
			      G_TYPE_STRING);
	...
	signals[IGNORE_ALL] = 
		g_signal_new ("ignore_all",
 			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviSpellCheckerDialogClass, ignore_all),
			      NULL, NULL,
			      scrivi_marshal_VOID__STRING,
			      G_TYPE_NONE, 
			      1, 
	...
	signals[CHANGE] = 
		g_signal_new ("change",
 			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviSpellCheckerDialogClass, change),
			      NULL, NULL,
			      scrivi_marshal_VOID__STRING_STRING,
			      G_TYPE_NONE, 
			      2, 
			      G_TYPE_STRING,
			      G_TYPE_STRING);
	...
	signals[CHANGE_ALL] = 
		g_signal_new ("change_all",
 			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviSpellCheckerDialogClass, change_all),
			      NULL, NULL,
			      scrivi_marshal_VOID__STRING_STRING,
			      G_TYPE_NONE, 
			      2, 
	...
	signals[ADD_WORD_TO_PERSONAL] = 
		g_signal_new ("add_word_to_personal",
 			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviSpellCheckerDialogClass, add_word_to_personal),
			      NULL, NULL,
			      scrivi_marshal_VOID__STRING,
			      G_TYPE_NONE, 
			      1, 
...
static void
create_dialog (ScriviSpellCheckerDialog *dlg,
...
static void
scrivi_spell_checker_dialog_init (ScriviSpellCheckerDialog *dlg)
{
}

GtkWidget *
scrivi_spell_checker_dialog_new (const gchar *data_dir)
{
	ScriviSpellCheckerDialog *dlg;

	dlg = SCRIVI_SPELL_CHECKER_DIALOG (
			g_object_new (SCRIVI_TYPE_SPELL_CHECKER_DIALOG, NULL));
...
GtkWidget *
scrivi_spell_checker_dialog_new_from_spell_checker (ScriviSpellChecker *spell,
						   const gchar *data_dir)
{
	ScriviSpellCheckerDialog *dlg;
	...
	scrivi_spell_checker_dialog_set_spell_checker (dlg, spell);

	return GTK_WIDGET (dlg);
}

void
scrivi_spell_checker_dialog_set_spell_checker (ScriviSpellCheckerDialog *dlg, ScriviSpellChecker *spell)
{
	const ScriviSpellCheckerLanguage* language;
	const gchar *lang;
	gchar *tmp;
	
	g_return_if_fail (SCRIVI_IS_SPELL_CHECKER_DIALOG (dlg));
	...
	language = scrivi_spell_checker_get_language (dlg->spell_checker);

	lang = scrivi_spell_checker_language_to_string (language);
	...
	if (dlg->misspelled_word != NULL)
		scrivi_spell_checker_dialog_set_misspelled_word (dlg, dlg->misspelled_word, -1);
	else
...
void
scrivi_spell_checker_dialog_set_misspelled_word (ScriviSpellCheckerDialog *dlg, 
						const gchar             *word,
						gint                     len)
{
	gchar *tmp;
	...
	g_return_if_fail (!scrivi_spell_checker_check_word (dlg->spell_checker, word, -1));
	...
	sug = scrivi_spell_checker_get_suggestions (dlg->spell_checker,
		       				   dlg->misspelled_word, 
		       				   -1);
...
static void
update_suggestions_list_model (ScriviSpellCheckerDialog *dlg, GSList *suggestions)
{
	GtkListStore *store;
	GtkTreeIter iter;
...
static void
word_entry_changed_handler (GtkEditable *editable, ScriviSpellCheckerDialog *dlg)
{
	const gchar *text;
...
static void
close_button_clicked_handler (GtkButton *button, ScriviSpellCheckerDialog *dlg)
{
	g_return_if_fail (SCRIVI_IS_SPELL_CHECKER_DIALOG (dlg));
...
static void
check_word_button_clicked_handler (GtkButton *button, ScriviSpellCheckerDialog *dlg)
{
	const gchar *word;
	gssize len;
	...
	if (scrivi_spell_checker_check_word (dlg->spell_checker, word, len))
	{
		GtkListStore *store;
		GtkTreeIter iter;
...
static void
add_word_button_clicked_handler (GtkButton *button, ScriviSpellCheckerDialog *dlg)
{
	gchar *word;	
	...
	scrivi_spell_checker_add_word_to_personal (dlg->spell_checker, 
						  dlg->misspelled_word, 
						  -1);
...
static void
ignore_button_clicked_handler (GtkButton *button, ScriviSpellCheckerDialog *dlg)
{
	gchar *word;
...
static void
ignore_all_button_clicked_handler (GtkButton *button, ScriviSpellCheckerDialog *dlg)
{
	gchar *word;
	...
	scrivi_spell_checker_add_word_to_session (dlg->spell_checker,
						 dlg->misspelled_word, 
						 -1);
...
static void
change_button_clicked_handler (GtkButton *button, ScriviSpellCheckerDialog *dlg)
{
	...
	scrivi_spell_checker_set_correction (dlg->spell_checker, 
...
static void
change_all_button_clicked_handler (GtkButton *button, ScriviSpellCheckerDialog *dlg)
{
	const gchar *entry_text;
	gchar *change;
	gchar *word;
...
void 
scrivi_spell_checker_dialog_set_completed (ScriviSpellCheckerDialog *dlg)
{
	gchar *tmp;
