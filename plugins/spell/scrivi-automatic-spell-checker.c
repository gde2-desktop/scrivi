...
struct _ScriviAutomaticSpellChecker {
	ScriviDocument		*doc;
	GSList 			*views;
...
static void scrivi_automatic_spell_checker_free_internal (ScriviAutomaticSpellChecker *spell);

static void
view_destroy (ScriviView *view, ScriviAutomaticSpellChecker *spell)
{
	scrivi_automatic_spell_checker_detach_view (spell, view);
}

static void
check_word (ScriviAutomaticSpellChecker *spell, GtkTextIter *start, GtkTextIter *end) 
{
	gchar *word;
	...
	*/
	...
	if (!scrivi_spell_checker_check_word (spell->spell_checker, word, -1)) 
	{
		/*
...
static void
check_range (ScriviAutomaticSpellChecker *spell, 
	     GtkTextIter                 start, 
	     GtkTextIter                 end,
	     gboolean                    force_all) 
{
	...
	*/
	...
	while (scrivi_spell_utils_skip_no_spell_check (&wstart, &end) &&
...
static void
check_deferred_range (ScriviAutomaticSpellChecker *spell, 
		      gboolean                    force_all) 
{
...
insert_text_before (GtkTextBuffer *buffer, GtkTextIter *iter,
		gchar *text, gint len, ScriviAutomaticSpellChecker *spell) 
{
...
static void
remove_tag_to_word (ScriviAutomaticSpellChecker *spell, const gchar *word)
{
	GtkTextIter iter;
...
static void
add_to_dictionary (GtkWidget *menuitem, ScriviAutomaticSpellChecker *spell) 
{
	gchar *word;
	...
	scrivi_spell_checker_add_word_to_personal (spell->spell_checker, word, -1);

	g_free (word);
}

static void
ignore_all (GtkWidget *menuitem, ScriviAutomaticSpellChecker *spell) 
{
	gchar *word;
	...
	scrivi_spell_checker_add_word_to_session (spell->spell_checker, word, -1);

	g_free (word);
}

static void
replace_word (GtkWidget *menuitem, ScriviAutomaticSpellChecker *spell) 
{
	gchar *oldword;
	const gchar *newword;
	...
	scrivi_spell_checker_set_correction (spell->spell_checker, 
...
static GtkWidget *
build_suggestion_menu (ScriviAutomaticSpellChecker *spell, const gchar *word) 
{
	...
	suggestions = scrivi_spell_checker_get_suggestions (spell->spell_checker, word, -1);
...
static void
populate_popup (GtkTextView *textview, GtkMenu *menu, ScriviAutomaticSpellChecker *spell) 
{
	GtkWidget *img, *mi;
	GtkTextIter start, end;
	char *word;
...
void
scrivi_automatic_spell_checker_recheck_all (ScriviAutomaticSpellChecker *spell)
{
	GtkTextIter start, end;
...
static void 
add_word_signal_cb (ScriviSpellChecker          *checker, 
		    const gchar                *word, 
		    gint                        len,
		    ScriviAutomaticSpellChecker *spell)
{
...
static void 
set_language_cb (ScriviSpellChecker               *checker,
		 const ScriviSpellCheckerLanguage *lang,
		 ScriviAutomaticSpellChecker      *spell)
{
	scrivi_automatic_spell_checker_recheck_all (spell);
}

static void 
clear_session_cb (ScriviSpellChecker          *checker,
		  ScriviAutomaticSpellChecker *spell)
{
	scrivi_automatic_spell_checker_recheck_all (spell);
}
 ...
 */
static gboolean
popup_menu_event (GtkTextView *view, ScriviAutomaticSpellChecker *spell) 
{
	GtkTextIter iter;
...
static void
spell_tag_destroyed (ScriviAutomaticSpellChecker *spell,
                     GObject                    *where_the_object_was)
{
	spell->tag_highlight = NULL;
}

ScriviAutomaticSpellChecker *
scrivi_automatic_spell_checker_new (ScriviDocument     *doc,
				   ScriviSpellChecker *checker)
{
	...
	g_return_val_if_fail (SCRIVI_IS_SPELL_CHECKER (checker), NULL);
	g_return_val_if_fail ((spell = scrivi_automatic_spell_checker_get_from_document (doc)) == NULL,
			      spell);
	
	/* attach to the widget */
	spell = g_new0 (ScriviAutomaticSpellChecker, 1);
	...
	if (automatic_spell_checker_id == 0)
	{
		automatic_spell_checker_id = 
			g_quark_from_string ("ScriviAutomaticSpellCheckerID");
	}
	if (suggestion_id == 0)
	{
		suggestion_id = g_quark_from_string ("ScriviAutoSuggestionID");
	}

	g_object_set_qdata_full (G_OBJECT (doc), 
				 automatic_spell_checker_id, 
				 spell, 
				 (GDestroyNotify)scrivi_automatic_spell_checker_free_internal);
	...
	spell->mark_insert_start = gtk_text_buffer_get_mark (GTK_TEXT_BUFFER (doc),
					"scrivi-automatic-spell-checker-insert-start");
	...
	spell->mark_insert_end = gtk_text_buffer_get_mark (GTK_TEXT_BUFFER (doc),
					"scrivi-automatic-spell-checker-insert-end");
	...
	spell->mark_click = gtk_text_buffer_get_mark (GTK_TEXT_BUFFER (doc),
					"scrivi-automatic-spell-checker-click");
...
ScriviAutomaticSpellChecker *
scrivi_automatic_spell_checker_get_from_document (const ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), NULL);
...
void
scrivi_automatic_spell_checker_free (ScriviAutomaticSpellChecker *spell)
{
	g_return_if_fail (spell != NULL);
	g_return_if_fail (scrivi_automatic_spell_checker_get_from_document (spell->doc) == spell);
...
static void
scrivi_automatic_spell_checker_free_internal (ScriviAutomaticSpellChecker *spell)
{
	...
	while (list != NULL)
	{
		ScriviView *view = SCRIVI_VIEW (list->data);
...
void
scrivi_automatic_spell_checker_attach_view (
		ScriviAutomaticSpellChecker *spell, 
		ScriviView *view)
{
	g_return_if_fail (spell != NULL);
...
void 
scrivi_automatic_spell_checker_detach_view (
		ScriviAutomaticSpellChecker *spell,
		ScriviView *view)
{
	g_return_if_fail (spell != NULL);
