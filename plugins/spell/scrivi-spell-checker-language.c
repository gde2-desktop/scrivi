...
struct _ScriviSpellCheckerLanguage 
{
	gchar *abrev;
	gchar *name;	
};
		  ...
		  gpointer user_data)
{
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Loading ISO-%d codes", iso);
...
static gint
lang_cmp (const ScriviSpellCheckerLanguage *a,
          const ScriviSpellCheckerLanguage *b)
{
...
build_langs_list (const gchar *key, 
		  ...
		  gpointer     data)
{
	ScriviSpellCheckerLanguage *lang = g_new (ScriviSpellCheckerLanguage, 1);
...
const GSList *
scrivi_spell_checker_get_available_languages (void)
{
...
const gchar *
scrivi_spell_checker_language_to_string (const ScriviSpellCheckerLanguage *lang)
{
	if (lang == NULL)
...
const gchar *
scrivi_spell_checker_language_to_key (const ScriviSpellCheckerLanguage *lang)
{
...
scrivi_spell_checker_language_from_key (const gchar *key)
{
	const GSList *langs;
	...
	while (langs != NULL)
	{
		const ScriviSpellCheckerLanguage *l = (const ScriviSpellCheckerLanguage *)langs->data;
