...
/*
 * Modified by the scrivi Team, 2002-2006. See the AUTHORS file for a
...
struct _ScriviSpellChecker
{
	GObject parent_instance;

	EnchantDict                     *dict;
	EnchantBroker                   *broker;
	const ScriviSpellCheckerLanguage *active_lang;
};
...
G_DEFINE_TYPE(ScriviSpellChecker, scrivi_spell_checker, G_TYPE_OBJECT)

static void
scrivi_spell_checker_set_property (GObject *object,
			   guint prop_id,
			   const GValue *value,
			   GParamSpec *pspec)
{
	/*
	ScriviSpellChecker *spell = SCRIVI_SPELL_CHECKER (object);
	*/
...
static void
scrivi_spell_checker_get_property (GObject *object,
			   guint prop_id,
			   GValue *value,
			   GParamSpec *pspec)
{
...
static void
scrivi_spell_checker_finalize (GObject *object)
{
	ScriviSpellChecker *spell_checker;

	g_return_if_fail (SCRIVI_IS_SPELL_CHECKER (object));

	spell_checker = SCRIVI_SPELL_CHECKER (object);
	...
	G_OBJECT_CLASS (scrivi_spell_checker_parent_class)->finalize (object);
}

static void
scrivi_spell_checker_class_init (ScriviSpellCheckerClass * klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);

	object_class->set_property = scrivi_spell_checker_set_property;
	object_class->get_property = scrivi_spell_checker_get_property;

	object_class->finalize = scrivi_spell_checker_finalize;
	...
	signals[ADD_WORD_TO_PERSONAL] =
	    g_signal_new ("add_word_to_personal",
			  ...
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (ScriviSpellCheckerClass, add_word_to_personal),
			  NULL, NULL,
			  scrivi_marshal_VOID__STRING_INT,
			  G_TYPE_NONE,
			  2,
			  G_TYPE_STRING,
			  G_TYPE_INT);
	...
	signals[ADD_WORD_TO_SESSION] =
	    g_signal_new ("add_word_to_session",
			  G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (ScriviSpellCheckerClass, add_word_to_session),
			  NULL, NULL,
			  scrivi_marshal_VOID__STRING_INT,
			  G_TYPE_NONE,
			  2,
	...
	signals[SET_LANGUAGE] =
	    g_signal_new ("set_language",
			  G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (ScriviSpellCheckerClass, set_language),
			  NULL, NULL,
			  scrivi_marshal_VOID__POINTER,
			  G_TYPE_NONE,
			  1,
			  G_TYPE_POINTER);
	...
	signals[CLEAR_SESSION] =
	    g_signal_new ("clear_session",
			  G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (ScriviSpellCheckerClass, clear_session),
			  NULL, NULL,
			  scrivi_marshal_VOID__VOID,
			  G_TYPE_NONE,
			  0);
}

static void
scrivi_spell_checker_init (ScriviSpellChecker *spell_checker)
{
...
ScriviSpellChecker *
scrivi_spell_checker_new	(void)
{
	ScriviSpellChecker *spell;

	spell = SCRIVI_SPELL_CHECKER (
			g_object_new (SCRIVI_TYPE_SPELL_CHECKER, NULL));
...
static gboolean
lazy_init (ScriviSpellChecker               *spell,
	   const ScriviSpellCheckerLanguage *language)
{
	...
	else
	{
		...
		const ScriviSpellCheckerLanguage *l;
		gint i = 0;
		const gchar * const *lang_tags = g_get_language_names ();

		while (lang_tags [i])
		{
			l = scrivi_spell_checker_language_from_key (lang_tags [i]);
	...
	if (spell->active_lang == NULL)
		spell->active_lang = scrivi_spell_checker_language_from_key ("en_US");

	/* Last try to get a default language */
	if (spell->active_lang == NULL)
	{
		const GSList *langs;
		langs = scrivi_spell_checker_get_available_languages ();
		if (langs != NULL)
			spell->active_lang = (const ScriviSpellCheckerLanguage *)langs->data;
	}
	...
	if (spell->active_lang != NULL)
	{
		...
		key = scrivi_spell_checker_language_to_key (spell->active_lang);
...
gboolean
scrivi_spell_checker_set_language (ScriviSpellChecker               *spell,
				  const ScriviSpellCheckerLanguage *language)
{
	gboolean ret;

	g_return_val_if_fail (SCRIVI_IS_SPELL_CHECKER (spell), FALSE);
	...
	if (ret)
		...
		g_warning ("Spell checker plugin: cannot use language %s.",
		           scrivi_spell_checker_language_to_string (language));
...
{
	g_return_val_if_fail (SCRIVI_IS_SPELL_CHECKER (spell), NULL);
...
gboolean
scrivi_spell_checker_check_word (ScriviSpellChecker *spell,
				const gchar       *word,
				gssize             len)
{
	gint enchant_result;
	...
	if (strcmp (word, "scrivi") == 0)
		return TRUE;

	if (scrivi_spell_utils_is_digit (word, len))
		return TRUE;
...
GSList *
scrivi_spell_checker_get_suggestions (ScriviSpellChecker *spell,
				     const gchar       *word,
				     gssize             len)
{
	gchar **suggestions;
...
gboolean
scrivi_spell_checker_add_word_to_personal (ScriviSpellChecker *spell,
					  const gchar       *word,
					  gssize             len)
{
	g_return_val_if_fail (SCRIVI_IS_SPELL_CHECKER (spell), FALSE);
...
gboolean
scrivi_spell_checker_add_word_to_session (ScriviSpellChecker *spell,
					 const gchar       *word,
					 gssize             len)
{
	g_return_val_if_fail (SCRIVI_IS_SPELL_CHECKER (spell), FALSE);
...
gboolean
scrivi_spell_checker_clear_session (ScriviSpellChecker *spell)
{
	g_return_val_if_fail (SCRIVI_IS_SPELL_CHECKER (spell), FALSE);
 ...
 */
gboolean
scrivi_spell_checker_set_correction (ScriviSpellChecker *spell,
				    const gchar       *word,
				    gssize             w_len,
