...
#define SCRIVI_TAGLIST_PLUGIN_PANEL_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						       SCRIVI_TYPE_TAGLIST_PLUGIN_PANEL, \
						       ScriviTaglistPluginPanelPrivate))
...
struct _ScriviTaglistPluginPanelPrivate
{
	ScriviWindow  *window;
...
SCRIVI_PLUGIN_DEFINE_TYPE (ScriviTaglistPluginPanel, scrivi_taglist_plugin_panel, GTK_TYPE_BOX)
#else
SCRIVI_PLUGIN_DEFINE_TYPE (ScriviTaglistPluginPanel, scrivi_taglist_plugin_panel, GTK_TYPE_VBOX)
#endif
...
static void
set_window (ScriviTaglistPluginPanel *panel,
	    ScriviWindow             *window)
{
...
static void
scrivi_taglist_plugin_panel_set_property (GObject      *object,
					 guint         prop_id,
					 const GValue *value,
					 GParamSpec   *pspec)
{
	ScriviTaglistPluginPanel *panel = SCRIVI_TAGLIST_PLUGIN_PANEL (object);
...
static void
scrivi_taglist_plugin_panel_get_property (GObject    *object,
					 guint       prop_id,
					 GValue     *value,
					 GParamSpec *pspec)
{
	...
	switch (prop_id)
	{
		case PROP_WINDOW:
			g_value_set_object (value,
					    SCRIVI_TAGLIST_PLUGIN_PANEL_GET_PRIVATE (panel)->window);
			break;
		default:
			G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
			break;
...
static void
scrivi_taglist_plugin_panel_finalize (GObject *object)
{
	ScriviTaglistPluginPanel *panel = SCRIVI_TAGLIST_PLUGIN_PANEL (object);
	
	g_free (panel->priv->data_dir);

	G_OBJECT_CLASS (scrivi_taglist_plugin_panel_parent_class)->finalize (object);
}

static void
scrivi_taglist_plugin_panel_class_init (ScriviTaglistPluginPanelClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = scrivi_taglist_plugin_panel_finalize;
	object_class->get_property = scrivi_taglist_plugin_panel_get_property;
	object_class->set_property = scrivi_taglist_plugin_panel_set_property;

	g_object_class_install_property (object_class,
					 PROP_WINDOW,
					 g_param_spec_object ("window",
							 "Window",
							 "The ScriviWindow this ScriviTaglistPluginPanel is associated with",
							 SCRIVI_TYPE_WINDOW,
							 G_PARAM_READWRITE |
							 G_PARAM_CONSTRUCT_ONLY));

	g_type_class_add_private (object_class, sizeof(ScriviTaglistPluginPanelPrivate));
}

static void
insert_tag (ScriviTaglistPluginPanel *panel,
	    Tag                     *tag,
	    gboolean                 grab_focus)
{
	ScriviView *view;
	...
	view = scrivi_window_get_active_view (panel->priv->window);
...
tag_list_row_activated_cb (GtkTreeView             *tag_list,
			   ...
			   ScriviTaglistPluginPanel *panel)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
	gint index;
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Index: %d", index);
			     ...
			     ScriviTaglistPluginPanel *panel)
{
	...
	if (event->keyval == GDK_KEY_Return)
	{
		...
		scrivi_debug_message (DEBUG_PLUGINS, "RETURN Pressed");
...
static GtkTreeModel*
create_model (ScriviTaglistPluginPanel *panel)
{
	gint i = 0;
	GtkListStore *store;
	GtkTreeIter iter;
	...
	while (list != NULL)
	{
		...
		scrivi_debug_message (DEBUG_PLUGINS, "%d : %s", i, tag_name);
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Rows: %d ",
...
static void
populate_tags_list (ScriviTaglistPluginPanel *panel)
{
	GtkTreeModel* model;
...
static void
populate_tag_groups_combo (ScriviTaglistPluginPanel *panel)
{
	GList *l;
	GtkComboBox *combo;
...
static void
update_preview (ScriviTaglistPluginPanel *panel,
		Tag                     *tag)
{
	gchar *str;
			    ...
			    gpointer     data)
{
	...
	ScriviTaglistPluginPanel *panel = (ScriviTaglistPluginPanel *)data;
...
expose_event_cb (GtkWidget      *panel,
                 ...
                 gpointer        user_data)
{
	ScriviTaglistPluginPanel *ppanel = SCRIVI_TAGLIST_PLUGIN_PANEL (panel);
	...
	populate_tag_groups_combo (SCRIVI_TAGLIST_PLUGIN_PANEL (panel));
...
static void
add_preview_widget (ScriviTaglistPluginPanel *panel)
{
	GtkWidget *expander;
	GtkWidget *frame;
...
static void
scrivi_taglist_plugin_panel_init (ScriviTaglistPluginPanel *panel)
{
	GtkWidget *sw;
	GtkTreeViewColumn *column;
	...
	panel->priv = SCRIVI_TAGLIST_PLUGIN_PANEL_GET_PRIVATE (panel);
	...
	scrivi_utils_set_atk_name_description (panel->priv->tag_groups_combo,
					      _("Available Tag Lists"),
					      NULL);
	scrivi_utils_set_atk_name_description (panel->priv->tags_list,
					      _("Tags"),
					      NULL);
	scrivi_utils_set_atk_relation (panel->priv->tag_groups_combo,
				      panel->priv->tags_list,
				      ATK_RELATION_CONTROLLER_FOR);
	scrivi_utils_set_atk_relation (panel->priv->tags_list,
				      panel->priv->tag_groups_combo,
...
GtkWidget *
scrivi_taglist_plugin_panel_new (ScriviWindow *window,
				const gchar *data_dir)
{
	...
	panel = g_object_new (SCRIVI_TYPE_TAGLIST_PLUGIN_PANEL,
			      "window", window,
			      NULL);
