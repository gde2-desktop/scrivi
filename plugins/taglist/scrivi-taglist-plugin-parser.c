...
#define USER_SCRIVI_TAGLIST_PLUGIN_LOCATION "scrivi/taglist/"
...
{
	/*
	scrivi_debug_message (DEBUG_PLUGINS, "  Tag name: %s", tag->name);
	*/
	...
	while (cur != NULL)
	{
		    ...
		    (cur->ns == ns))
		{
			...
			/*
			scrivi_debug_message (DEBUG_PLUGINS, "    - Begin: %s", tag->begin);
			*/
		}
		    ...
		    (cur->ns == ns))
		{
			...
			/*
			scrivi_debug_message (DEBUG_PLUGINS, "    - End: %s", tag->end);
			*/
		}
...
{
	scrivi_debug_message (DEBUG_PLUGINS, "Parse TagGroup: %s", tg->name);
 ...
 *
 *      <scrivi:TagGroup name="XSLT - Elements">
 *      </scrivi:TagGroup>
 *      <scrivi:TagGroup xml:lang="am" name="LOCALIZED TEXT">
 *      </scrivi:TagGroup>
 *      <scrivi:TagGroup xml:lang="ar" name="LOCALIZED TEXT">
 *      </scrivi:TagGroup>
 *      .....
 *      <scrivi:TagGroup name="XSLT - Functions">
 *      </scrivi:TagGroup>
 *      .....
...
parse_taglist_file (const gchar* filename)
{
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Parse file: %s", filename);
	...
	*/
	...
	ns = xmlSearchNsByHref (doc, cur,
			(const xmlChar *) "http://scrivi.sourceforge.net/some-location");

	if (ns == NULL)
	{
		g_warning ("The tag list file '%s' is of the wrong type, "
			   "scrivi namespace not found.", filename);
		xmlFreeDoc (doc);
...
{
	/*
	scrivi_debug_message (DEBUG_PLUGINS, "Tag: %s", tag->name);
	*/
...
free_tag_group (TagGroup *tag_group)
{
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Tag group: %s", tag_group->name);
...
void free_taglist(void)
{
	...
	scrivi_debug_message(DEBUG_PLUGINS, "ref_count: %d", taglist_ref_count);
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Really freed");
}
...
static TagList* parse_taglist_dir(const gchar* dir)
{
	...
	if (!d)
	{
		scrivi_debug_message(DEBUG_PLUGINS, "%s", error->message);
		g_error_free (error);
		return taglist;
	}
...
TagList* create_taglist(const gchar* data_dir)
{
	...
	if (home != NULL)
	{
		pdir = g_build_filename(home, ".config", USER_SCRIVI_TAGLIST_PLUGIN_LOCATION, NULL);
