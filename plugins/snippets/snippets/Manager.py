...
class Manager:
        ...
        model = None
        drag_icons = ('gde2-mime-application-x-tarz', 'gde2-package', 'package')
        ...
        def build_model(self, force_reload = False):
                window = scrivi.app_get_default().get_active_window()
                ...
                if not self.model or force_reload:
                        ...
                        langs = scrivi.language_manager_list_languages_sorted(manager, True)
        ...
        def on_dialog_snippets_response(self, dlg, resp):                                
                if resp == gtk.RESPONSE_HELP:
                        scrivi.help_display(self.dlg, 'scrivi', 'scrivi-snippets-plugin')
                        return
        ...
        def import_snippets(self, filenames):
                ...
                for filename in filenames:
                        if not scrivi.utils.uri_has_file_scheme(filename):
                                continue
