...
class ToolLibrary(Singleton):
    ...
    def set_locations(self, datadir):
        ...
        for d in self.get_xdg_data_dirs():
            self.locations.append(os.path.join(d, 'scrivi', 'plugins', 'externaltools', 'tools'))

        self.locations.append(datadir)

        # self.locations[0] is where we save the custom scripts
        toolsdir = os.path.expanduser('~/.config/scrivi/tools')
    ...
    def import_old_xml_store(self):
        ...
        filename = os.path.expanduser('~/.config/scrivi/scrivi-tools.xml')
...
class Tool(object):
    ...
    def _load(self):
        ...
        for line in fp:
            if not in_block:
                in_block = line.startswith('# [Scrivi Tool]')
                continue
    ...
    def get_script(self):
        ...
        for line in fp:
            if line.startswith('# [Scrivi Tool]'):
                break
            lines.append(line)
        # in the block:
        for line in fp:
