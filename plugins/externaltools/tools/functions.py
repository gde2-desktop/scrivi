...
def run_external_tool(window, node):
    ...
    capture.set_env(SCRIVI_CWD = cwd)
    ...
    if view is not None:
        ...
        # Current line number
        piter = document.get_iter_at_mark(document.get_insert())
        capture.set_env(SCRIVI_CURRENT_LINE_NUMBER=str(piter.get_line() + 1))
        ...
        capture.set_env(SCRIVI_CURRENT_LINE=piter.get_text(end))
        ...
        if node.input != 'selection' and node.input != 'selection-document':
            ...
            if bounds:
                capture.set_env(SCRIVI_SELECTED_TEXT=bounds[0].get_text(bounds[1]))
        
        bounds = current_word(document)
        capture.set_env(SCRIVI_CURRENT_WORD=bounds[0].get_text(bounds[1]))
        
        capture.set_env(SCRIVI_CURRENT_DOCUMENT_TYPE=document.get_mime_type())
        
        if uri is not None:
            gfile = gio.File(uri)
            scheme = gfile.get_uri_scheme()
            name = os.path.basename(uri)
            capture.set_env(SCRIVI_CURRENT_DOCUMENT_URI    = uri,
                            SCRIVI_CURRENT_DOCUMENT_NAME   = name,
                            SCRIVI_CURRENT_DOCUMENT_SCHEME = scheme)
            if scrivi.utils.uri_has_file_scheme(uri):
                path = gfile.get_path()
                cwd = os.path.dirname(path)
                capture.set_cwd(cwd)
                capture.set_env(SCRIVI_CURRENT_DOCUMENT_PATH = path,
                                SCRIVI_CURRENT_DOCUMENT_DIR  = cwd)
        ...
        documents_path = [gio.File(uri).get_path()
                                 ...
                                 if scrivi.utils.uri_has_file_scheme(uri)]
        capture.set_env(SCRIVI_DOCUMENTS_URI  = ' '.join(documents_uri),
                        SCRIVI_DOCUMENTS_PATH = ' '.join(documents_path))
...
class MultipleDocumentsSaver:
    def __init__(self, window, docs, node):
        ...
        for doc in docs:
            ...
            scrivi.commands.save_document(window, doc)
...
def capture_end_execute_panel(capture, exit_code, panel, view, output_type):
    ...
    if output_type in ('new-document','replace-document'):
        ...
        lmanager = scrivi.get_language_manager()
