/*
 * scrivi-file-browser-widget.c - Scrivi plugin providing easy file access 
 * from the sidepanel
 *
...
#define SCRIVI_FILE_BROWSER_WIDGET_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE((object), \
						      SCRIVI_TYPE_FILE_BROWSER_WIDGET, \
						      ScriviFileBrowserWidgetPrivate))

#define XML_UI_FILE "scrivi-file-browser-widget-ui.xml"
#define LOCATION_DATA_KEY "scrivi-file-browser-widget-location"
...
struct _ScriviFileBrowserWidgetPrivate 
{
	ScriviFileBrowserView *treeview;
	ScriviFileBrowserStore *file_store;
	ScriviFileBookmarksStore *bookmarks_store;
...
static void set_enable_delete		       (ScriviFileBrowserWidget *obj,
						gboolean enable);
static void on_model_set                       (GObject * gobject, 
						GParamSpec * arg1,
						ScriviFileBrowserWidget * obj);
static void on_treeview_error                  (ScriviFileBrowserView * tree_view,
						guint code, 
						gchar * message,
						ScriviFileBrowserWidget * obj);
static void on_file_store_error                (ScriviFileBrowserStore * store, 
						guint code,
						gchar * message,
						ScriviFileBrowserWidget * obj);
static gboolean on_file_store_no_trash 	       (ScriviFileBrowserStore * store, 
						GList * files, 
						ScriviFileBrowserWidget * obj);
static void on_combo_changed                   (GtkComboBox * combo,
						ScriviFileBrowserWidget * obj);
static gboolean on_treeview_popup_menu         (ScriviFileBrowserView * treeview,
						ScriviFileBrowserWidget * obj);
static gboolean on_treeview_button_press_event (ScriviFileBrowserView * treeview,
						GdkEventButton * event,
						ScriviFileBrowserWidget * obj);
static gboolean on_treeview_key_press_event    (ScriviFileBrowserView * treeview, 
						GdkEventKey * event,
						ScriviFileBrowserWidget * obj);
static void on_selection_changed               (GtkTreeSelection * selection,
						ScriviFileBrowserWidget * obj);

static void on_virtual_root_changed            (ScriviFileBrowserStore * model,
						GParamSpec *param,
						ScriviFileBrowserWidget * obj);

static gboolean on_entry_filter_activate       (ScriviFileBrowserWidget * obj);
...
static void on_bookmarks_row_deleted           (GtkTreeModel * model, 
                                                GtkTreePath * path,
                                                ScriviFileBrowserWidget * obj);
static void on_filter_mode_changed	       (ScriviFileBrowserStore * model,
                                                GParamSpec * param,
                                                ScriviFileBrowserWidget * obj);
...
#if GTK_CHECK_VERSION (3, 0, 0)
SCRIVI_PLUGIN_DEFINE_TYPE (ScriviFileBrowserWidget, scrivi_file_browser_widget,
	                  GTK_TYPE_BOX)
#else
SCRIVI_PLUGIN_DEFINE_TYPE (ScriviFileBrowserWidget, scrivi_file_browser_widget,
	                  GTK_TYPE_VBOX)
...
static FilterFunc *
filter_func_new (ScriviFileBrowserWidget * obj,
		 ScriviFileBrowserWidgetFilterFunc func,
		 gpointer user_data,
...
static gboolean
combo_find_by_id (ScriviFileBrowserWidget * obj, guint id,
		  GtkTreeIter * iter)
{
	guint checkid;
...
static void
remove_path_items (ScriviFileBrowserWidget * obj)
{
	GtkTreeIter iter;
...
static void
cancel_async_operation (ScriviFileBrowserWidget *widget)
{
...
static void
scrivi_file_browser_widget_finalize (GObject * object)
{
	ScriviFileBrowserWidget *obj = SCRIVI_FILE_BROWSER_WIDGET (object);
	GList *loc;

	remove_path_items (obj);
	scrivi_file_browser_store_set_filter_func (obj->priv->file_store,
						  NULL, NULL);
	...
	G_OBJECT_CLASS (scrivi_file_browser_widget_parent_class)->finalize (object);
}

static void
scrivi_file_browser_widget_get_property (GObject    *object,
			               guint       prop_id,
			               GValue     *value,
			               GParamSpec *pspec)
{
...
static void
scrivi_file_browser_widget_set_property (GObject      *object,
			               guint         prop_id,
			               const GValue *value,
			               GParamSpec   *pspec)
{
	...
	{
		case PROP_FILTER_PATTERN:
			scrivi_file_browser_widget_set_filter_pattern (obj,
...
static void
scrivi_file_browser_widget_class_init (ScriviFileBrowserWidgetClass * klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = scrivi_file_browser_widget_finalize;

	object_class->get_property = scrivi_file_browser_widget_get_property;
	object_class->set_property = scrivi_file_browser_widget_set_property;
	...
	signals[URI_ACTIVATED] =
	    g_signal_new ("uri-activated",
			  ...
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (ScriviFileBrowserWidgetClass,
	...
	signals[CONFIRM_DELETE] =
	    g_signal_new ("confirm-delete", G_OBJECT_CLASS_TYPE (object_class),
	                  ...
	                  NULL,
	                  scrivi_file_browser_marshal_BOOL__OBJECT_POINTER,
	                  G_TYPE_BOOLEAN,
	                  2,
	                  G_TYPE_OBJECT,
	                  G_TYPE_POINTER);
	...
	g_type_class_add_private (object_class,
				  sizeof (ScriviFileBrowserWidgetPrivate));
}

static void
add_signal (ScriviFileBrowserWidget * obj, gpointer object, gulong id)
{
...
static void
clear_signals (ScriviFileBrowserWidget * obj)
{
	GSList *item;
	SignalNode *node;
...
static gboolean
get_from_bookmark_file (ScriviFileBrowserWidget * obj, GFile * file,
...
static void
insert_path_item (ScriviFileBrowserWidget * obj, 
                  GFile * file,
		  GtkTreeIter * after, 
		  GtkTreeIter * iter, 
		  guint indent)
{
	...
	if (!get_from_bookmark_file (obj, file, &unescape, &icon)) {
		...
		unescape = scrivi_file_browser_utils_file_basename (file);
		
		/* Get the icon */
		icon = scrivi_file_browser_utils_pixbuf_from_file (file, GTK_ICON_SIZE_MENU);
	}
...
static void
insert_separator_item (ScriviFileBrowserWidget * obj)
{
	GtkTreeIter iter;
...
static void
combo_set_active_by_id (ScriviFileBrowserWidget * obj, guint id)
{
	GtkTreeIter iter;
...
static void
insert_location_path (ScriviFileBrowserWidget * obj)
{
	Location *loc;
	GFile *current = NULL;
	GFile * tmp;
	...
	while (current != NULL) {
		...
		if (g_file_equal (current, loc->root) || !scrivi_utils_file_has_parent (current)) {
			if (current != loc->virtual_root)
...
static void
check_current_item (ScriviFileBrowserWidget * obj, gboolean show_path)
{
	GtkTreeIter separator;
	gboolean has_sep;
...
static void
fill_combo_model (ScriviFileBrowserWidget * obj)
{
	GtkTreeStore *store = obj->priv->combo_model;
	GtkTreeIter iter;
	GdkPixbuf *icon;

	icon = scrivi_file_browser_utils_pixbuf_from_theme (GTK_STOCK_HOME, GTK_ICON_SIZE_MENU);
...
static void
create_combo (ScriviFileBrowserWidget * obj)
{
...
{
	{"FileMoveToTrash", "gde2-stock-trash", N_("_Move to Trash"), NULL,
...
static void
create_toolbar (ScriviFileBrowserWidget * obj,
...
static void 
set_enable_delete (ScriviFileBrowserWidget *obj,
		   gboolean enable)
{
	GtkAction *action;
...
static gboolean
filter_real (ScriviFileBrowserStore * model, GtkTreeIter * iter,
	     ScriviFileBrowserWidget * obj)
{
	GSList *item;
	FilterFunc *func;
...
static void
add_bookmark_hash (ScriviFileBrowserWidget * obj,
                   GtkTreeIter * iter)
{
	GtkTreeModel *model;
	GdkPixbuf * pixbuf;
	...
	uri = scrivi_file_bookmarks_store_get_uri (obj->priv->
						  bookmarks_store,
						  iter);
	...
	gtk_tree_model_get (model, iter,
			    SCRIVI_FILE_BOOKMARKS_STORE_COLUMN_ICON,
			    &pixbuf,
...
static void
init_bookmarks_hash (ScriviFileBrowserWidget * obj)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
...
static void
on_begin_loading (ScriviFileBrowserStore  *model, 
		  GtkTreeIter            *iter,
		  ScriviFileBrowserWidget *obj)
{
...
static void
on_end_loading (ScriviFileBrowserStore  *model, 
		GtkTreeIter            *iter,
		ScriviFileBrowserWidget *obj)
{
	if (!GDK_IS_WINDOW (gtk_widget_get_window (GTK_WIDGET (obj->priv->treeview))))
...
static void
create_tree (ScriviFileBrowserWidget * obj)
{
	GtkWidget *sw;

	obj->priv->file_store = scrivi_file_browser_store_new (NULL);
	obj->priv->bookmarks_store = scrivi_file_bookmarks_store_new ();
	obj->priv->treeview =
	    SCRIVI_FILE_BROWSER_VIEW (scrivi_file_browser_view_new ());

	scrivi_file_browser_view_set_restore_expand_state (obj->priv->treeview, TRUE);

	scrivi_file_browser_store_set_filter_mode (obj->priv->file_store,
						  SCRIVI_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN
						  |
...
static void
create_filter (ScriviFileBrowserWidget * obj)
{
	GtkWidget *expander;
	GtkWidget *vbox;
	GtkWidget *entry;
...
static void
scrivi_file_browser_widget_init (ScriviFileBrowserWidget * obj)
{
#if GTK_CHECK_VERSION (3, 16, 0)
	GdkDisplay *display;
#endif
	obj->priv = SCRIVI_FILE_BROWSER_WIDGET_GET_PRIVATE (obj);
...
static void
update_sensitivity (ScriviFileBrowserWidget * obj)
{
	GtkTreeModel *model =
	...
	if (SCRIVI_IS_FILE_BROWSER_STORE (model)) {
		...
		mode =
		    scrivi_file_browser_store_get_filter_mode
		    (SCRIVI_FILE_BROWSER_STORE (model));
		...
		gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action),
					      !(mode &
						SCRIVI_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN));
	} else if (SCRIVI_IS_FILE_BOOKMARKS_STORE (model)) {
		gtk_action_group_set_sensitive (obj->priv->action_group,
						FALSE);
		gtk_action_group_set_sensitive (obj->priv->bookmark_action_group,
						TRUE);
...
static gboolean
scrivi_file_browser_widget_get_first_selected (ScriviFileBrowserWidget *obj,
					      GtkTreeIter *iter)
{
...
static gboolean
popup_menu (ScriviFileBrowserWidget * obj, GdkEventButton * event, GtkTreeModel * model)
{
	GtkWidget *menu;
	...
	if (event != NULL) {
		...
		gtk_menu_popup (GTK_MENU (menu), NULL, NULL,
				scrivi_utils_menu_position_under_tree_view,
				obj->priv->treeview, 0,
...
static gboolean
filter_glob (ScriviFileBrowserWidget * obj, ScriviFileBrowserStore * store,
	     GtkTreeIter * iter, gpointer user_data)
{
	...
	gtk_tree_model_get (GTK_TREE_MODEL (store), iter,
			    SCRIVI_FILE_BROWSER_STORE_COLUMN_NAME, &name,
...
static void
rename_selected_file (ScriviFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	...
	if (scrivi_file_browser_widget_get_first_selected (obj, &iter))
		scrivi_file_browser_view_start_rename (obj->priv->treeview,
						      &iter);
}

static GList *
get_deletable_files (ScriviFileBrowserWidget *obj) {
	GtkTreeSelection *selection;
	GtkTreeModel *model;
	GList *rows;
	GList *row;
...
static gboolean
delete_selected_files (ScriviFileBrowserWidget * obj, gboolean trash)
{
	GtkTreeModel *model;
	gboolean confirm;
	...
	result = scrivi_file_browser_store_delete_all (SCRIVI_FILE_BROWSER_STORE (model),
						      rows, trash);
	
	g_list_foreach (rows, (GFunc)gtk_tree_path_free, NULL);
	g_list_free (rows);

	return result == SCRIVI_FILE_BROWSER_STORE_RESULT_OK;
}

static gboolean
on_file_store_no_trash (ScriviFileBrowserStore * store, 
			GList * files, 
			ScriviFileBrowserWidget * obj)
{
...
static GtkWidget *
create_goto_menu_item (ScriviFileBrowserWidget * obj, GList * item,
		       GdkPixbuf * icon)
{
	GtkWidget *result;
	GtkWidget *image;
	...
	if (!get_from_bookmark_file (obj, loc->virtual_root, &unescape, &pixbuf)) {
		unescape = scrivi_file_browser_utils_file_basename (loc->virtual_root);
...
static void
jump_to_location (ScriviFileBrowserWidget * obj, GList * item,
		  gboolean previous)
{
	Location *loc;
	GtkWidget *widget;
	...
	scrivi_file_browser_widget_set_root_and_virtual_root (obj,
							     root,
							     virtual_root);
...
static void
clear_next_locations (ScriviFileBrowserWidget * obj)
{
	GList *children;
	GList *item;
...
static void
update_filter_mode (ScriviFileBrowserWidget * obj, 
                    GtkAction * action, 
                    ScriviFileBrowserStoreFilterMode mode)
{
	...
	if (SCRIVI_IS_FILE_BROWSER_STORE (model)) {
		...
		scrivi_file_browser_store_set_filter_mode
		    (SCRIVI_FILE_BROWSER_STORE (model), now);
	}
}

static void
set_filter_pattern_real (ScriviFileBrowserWidget * obj,
                        gchar const * pattern,
                        gboolean update_entry)
{
	...
	if (pattern == NULL) {
		if (obj->priv->glob_filter_id != 0) {
			scrivi_file_browser_widget_remove_filter (obj,
								 obj->
								 priv->
								 glob_filter_id);
		...
		if (obj->priv->glob_filter_id == 0)
			obj->priv->glob_filter_id =
			    scrivi_file_browser_widget_add_filter (obj,
								  filter_glob,
								  NULL,
								  NULL);
	}
	...
	if (SCRIVI_IS_FILE_BROWSER_STORE (model))
		scrivi_file_browser_store_refilter (SCRIVI_FILE_BROWSER_STORE
						   (model));
...
GtkWidget *
scrivi_file_browser_widget_new (const gchar *data_dir)
{
	ScriviFileBrowserWidget *obj =
	    g_object_new (SCRIVI_TYPE_FILE_BROWSER_WIDGET, NULL);
	...
	scrivi_file_browser_widget_show_bookmarks (obj);

	return GTK_WIDGET (obj);
}

void
scrivi_file_browser_widget_show_bookmarks (ScriviFileBrowserWidget * obj)
{
	...
	scrivi_file_browser_view_set_model (obj->priv->treeview,
					   GTK_TREE_MODEL (obj->priv->
							   bookmarks_store));
}

static void
show_files_real (ScriviFileBrowserWidget *obj,
...
void
scrivi_file_browser_widget_show_files (ScriviFileBrowserWidget * obj)
{
						     ...
						     gchar const *virtual_root)
{
	...
	if (!virtual_root)
		result =
		    scrivi_file_browser_store_set_root_and_virtual_root 
	...
	if (result == SCRIVI_FILE_BROWSER_STORE_RESULT_NO_CHANGE)
		show_files_real (obj, TRUE);
}

void
scrivi_file_browser_widget_set_root (ScriviFileBrowserWidget * obj,
				    gchar const *root, 
				    gboolean virtual_root)
{
	...
	scrivi_file_browser_widget_set_root_and_virtual_root
	    (obj, str, root);
...
gulong
scrivi_file_browser_widget_add_filter (ScriviFileBrowserWidget * obj,
				      ScriviFileBrowserWidgetFilterFunc func, 
				      gpointer user_data,
				      GDestroyNotify notify)
{
...
void
scrivi_file_browser_widget_remove_filter (ScriviFileBrowserWidget * obj,
					 gulong id)
{
	GSList *item;
	FilterFunc *func;
...
void 
scrivi_file_browser_widget_set_filter_pattern (ScriviFileBrowserWidget * obj,
                                              gchar const *pattern)
{
						  ...
						  GtkTreeIter * iter)
{
	...
	if (!scrivi_file_browser_widget_get_first_selected (obj, iter)) {
		if (!scrivi_file_browser_store_get_iter_virtual_root
		    (SCRIVI_FILE_BROWSER_STORE (model), iter))
			return FALSE;
	}
...
static guint
scrivi_file_browser_widget_get_num_selected_files_or_directories (ScriviFileBrowserWidget *obj,
								 guint                  *files,
								 guint                  *dirs)
{
	GList *rows, *row;
	...
	if (SCRIVI_IS_FILE_BOOKMARKS_STORE (model))
		return 0;
...
static AsyncData *
async_data_new (ScriviFileBrowserWidget *widget)
{
	AsyncData *ret;
...
static void
set_busy (ScriviFileBrowserWidget *obj, gboolean busy)
{
	GdkWindow *window;
...
static void try_mount_volume (ScriviFileBrowserWidget *widget, GVolume *volume);

static void
activate_mount (ScriviFileBrowserWidget *widget,
		GVolume		       *volume,
		GMount		       *mount)
{
	GFile *root;
	...
	scrivi_file_browser_widget_set_root (widget, uri, FALSE);
...
static void
try_activate_drive (ScriviFileBrowserWidget *widget,
		    GDrive 		   *drive)
{
	GList *volumes;
	GVolume *volume;
...
static void
activate_drive (ScriviFileBrowserWidget *obj,
		GtkTreeIter	       *iter)
{
	GDrive *drive;
	AsyncData *async;
...
static void 
try_mount_volume (ScriviFileBrowserWidget *widget, 
		  GVolume 		 *volume)
{
...
static void
activate_volume (ScriviFileBrowserWidget *obj,
		 GtkTreeIter	        *iter)
{
	GVolume *volume;
...
void 
scrivi_file_browser_widget_refresh (ScriviFileBrowserWidget *obj)
{
	GtkTreeModel *model =
	    gtk_tree_view_get_model (GTK_TREE_VIEW (obj->priv->treeview));

	if (SCRIVI_IS_FILE_BROWSER_STORE (model))
		scrivi_file_browser_store_refresh (SCRIVI_FILE_BROWSER_STORE
						  (model));
	else if (SCRIVI_IS_FILE_BOOKMARKS_STORE (model)) {
		g_hash_table_ref (obj->priv->bookmarks_hash);
		g_hash_table_destroy (obj->priv->bookmarks_hash);

		scrivi_file_bookmarks_store_refresh
		    (SCRIVI_FILE_BOOKMARKS_STORE (model));
	}
}

void 
scrivi_file_browser_widget_history_back (ScriviFileBrowserWidget *obj)
{
	if (obj->priv->locations) {
		if (obj->priv->current_location)
...
void
scrivi_file_browser_widget_history_forward (ScriviFileBrowserWidget *obj)
{
	if (obj->priv->locations)
...
static void
bookmark_open (ScriviFileBrowserWidget *obj,
	       GtkTreeModel           *model,
	       GtkTreeIter            *iter)
{
	gchar *uri;
	...
	if (flags & SCRIVI_FILE_BOOKMARKS_STORE_IS_DRIVE)
	{
		/* handle a drive node */
		scrivi_file_browser_store_cancel_mount_operation (obj->priv->file_store);
		activate_drive (obj, iter);
		return;
	}
	else if (flags & SCRIVI_FILE_BOOKMARKS_STORE_IS_VOLUME)
	{
	...
	uri =
	    scrivi_file_bookmarks_store_get_uri
	    (SCRIVI_FILE_BOOKMARKS_STORE (model), iter);
	...
	if (uri) {
		/* here we check if the bookmark is a mount point, or if it
		   ...
		   mount point anymore) */
		if ((flags & SCRIVI_FILE_BOOKMARKS_STORE_IS_MOUNT) ||
		    (flags & SCRIVI_FILE_BOOKMARKS_STORE_IS_REMOTE_BOOKMARK)) {
			scrivi_file_browser_widget_set_root (obj,
							    uri,
							    FALSE);
		} else {
			scrivi_file_browser_widget_set_root (obj,
...
static void
file_open  (ScriviFileBrowserWidget *obj,
	    GtkTreeModel           *model,
	    GtkTreeIter            *iter)
{
	gchar *uri;
...
static gboolean
directory_open (ScriviFileBrowserWidget *obj,
		GtkTreeModel           *model,
		GtkTreeIter            *iter)
{
...
static void
on_bookmark_activated (ScriviFileBrowserView   *tree_view,
		       GtkTreeIter            *iter,
		       ScriviFileBrowserWidget *obj)
{
...
static void
on_file_activated (ScriviFileBrowserView   *tree_view, 
		   GtkTreeIter            *iter,
		   ScriviFileBrowserWidget *obj)
{
	GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (tree_view));
...
static gboolean
virtual_root_is_root (ScriviFileBrowserWidget * obj, 
                      ScriviFileBrowserStore  * model)
{
	GtkTreeIter root;
	GtkTreeIter virtual_root;
	
	if (!scrivi_file_browser_store_get_iter_root (model, &root))
		return TRUE;
	
	if (!scrivi_file_browser_store_get_iter_virtual_root (model, &virtual_root))
		return TRUE;
	
	return scrivi_file_browser_store_iter_equal (model, &root, &virtual_root);
}

static void
on_virtual_root_changed (ScriviFileBrowserStore * model,
			 GParamSpec * param,
			 ScriviFileBrowserWidget * obj)
{
	GtkTreeIter iter;
	...
	if (scrivi_file_browser_store_get_iter_virtual_root (model, &iter)) {
		...
		if (scrivi_file_browser_store_get_iter_root (model, &root)) {
			if (!obj->priv->changing_location) {
				...
				root_uri =
				    scrivi_file_browser_store_get_root
				    (model);
				...
				gtk_tree_model_get (GTK_TREE_MODEL (model),
						    &iter,
						    SCRIVI_FILE_BROWSER_STORE_COLUMN_ICON,
						    &pixbuf, -1);
	      ...
	      ScriviFileBrowserWidget * obj)
{
	...
	} else if (SCRIVI_IS_FILE_BROWSER_STORE (model)) {
...
static void
on_file_store_error (ScriviFileBrowserStore * store, guint code,
		     gchar * message, ScriviFileBrowserWidget * obj)
{
	g_signal_emit (obj, signals[ERROR], 0, code, message);
}

static void
on_treeview_error (ScriviFileBrowserView * tree_view, guint code,
		   gchar * message, ScriviFileBrowserWidget * obj)
{
	g_signal_emit (obj, signals[ERROR], 0, code, message);
}
...
static void
on_combo_changed (GtkComboBox * combo, ScriviFileBrowserWidget * obj)
{
	GtkTreeIter iter;
	guint id;
	gchar * uri;
...
static gboolean
on_treeview_popup_menu (ScriviFileBrowserView * treeview,
			ScriviFileBrowserWidget * obj)
{
...
static gboolean
on_treeview_button_press_event (ScriviFileBrowserView * treeview,
				GdkEventButton * event,
				ScriviFileBrowserWidget * obj)
{
...
static gboolean
do_change_directory (ScriviFileBrowserWidget * obj,
                     GdkEventKey            * event)
{
...
static gboolean
on_treeview_key_press_event (ScriviFileBrowserView * treeview,
			     GdkEventKey * event,
			     ScriviFileBrowserWidget * obj)
{
	guint modifiers;
	...
	if (!SCRIVI_IS_FILE_BROWSER_STORE
	    (gtk_tree_view_get_model (GTK_TREE_VIEW (treeview))))
		return FALSE;
		      ...
		      ScriviFileBrowserWidget * obj)
{
	...
	if (SCRIVI_IS_FILE_BROWSER_STORE (model))
	{
		selected = scrivi_file_browser_widget_get_num_selected_files_or_directories (obj,
											    &files,
											    &dirs);
	}
...
static gboolean
on_entry_filter_activate (ScriviFileBrowserWidget * obj)
{
	gchar const *text;
                          ...
                          ScriviFileBrowserWidget *obj)
{
	...
	uri = scrivi_file_bookmarks_store_get_uri (obj->priv->bookmarks_store, &iter);
...
static void 
on_filter_mode_changed (ScriviFileBrowserStore * model,
                        GParamSpec * param,
                        ScriviFileBrowserWidget * obj)
{
	gint mode;
	...
	action = GTK_TOGGLE_ACTION (gtk_action_group_get_action (obj->priv->action_group, 
	                                                         "FilterHidden"));
	active = !(mode & SCRIVI_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN);
	...
	action = GTK_TOGGLE_ACTION (gtk_action_group_get_action (obj->priv->action_group, 
	                                                         "FilterBinary"));
	active = !(mode & SCRIVI_FILE_BROWSER_STORE_FILTER_MODE_HIDE_BINARY);
...
static void
on_action_directory_next (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	scrivi_file_browser_widget_history_forward (obj);
}
...
on_action_directory_previous (GtkAction * action,
			      ScriviFileBrowserWidget * obj)
{
	scrivi_file_browser_widget_history_back (obj);
}
			...
			ScriviFileBrowserWidget * obj)
{
	...
	scrivi_file_browser_store_set_virtual_root_up (SCRIVI_FILE_BROWSER_STORE (model));	
}

static void
on_action_directory_new (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeIter parent;
	GtkTreeIter iter;
	...
	if (!scrivi_file_browser_widget_get_selected_directory (obj, &parent))
		return;

	if (scrivi_file_browser_store_new_directory
	    (SCRIVI_FILE_BROWSER_STORE (model), &parent, &iter)) {
		scrivi_file_browser_view_start_rename (obj->priv->treeview,
						      &iter);
	}
}
...
static void
on_action_file_open (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeSelection *selection;
	GList *rows;
...
static void
on_action_file_new (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeIter parent;
	GtkTreeIter iter;
	...
	if (scrivi_file_browser_store_new_file
	    (SCRIVI_FILE_BROWSER_STORE (model), &parent, &iter)) {
		scrivi_file_browser_view_start_rename (obj->priv->treeview,
						      &iter);
	}
...
static void
on_action_file_rename (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	rename_selected_file (obj);
}

static void
on_action_file_delete (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	delete_selected_files (obj, FALSE);
}

static void
on_action_file_move_to_trash (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	delete_selected_files (obj, TRUE);
}
...
on_action_directory_refresh (GtkAction * action,
			     ScriviFileBrowserWidget * obj)
{
	scrivi_file_browser_widget_refresh (obj);
}

static void
on_action_directory_open (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeSelection *selection;
	GList *rows;
	...
	if (!directory_opened) {
		if (scrivi_file_browser_widget_get_selected_directory (obj, &iter))
...
static void
on_action_filter_hidden (GtkAction * action, ScriviFileBrowserWidget * obj)
{
...
static void
on_action_filter_binary (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	update_filter_mode (obj, 
	                    action, 
	                    SCRIVI_FILE_BROWSER_STORE_FILTER_MODE_HIDE_BINARY);
...
static void
on_action_bookmark_open (GtkAction * action, ScriviFileBrowserWidget * obj)
{
	GtkTreeModel *model;
	GtkTreeSelection *selection;
	GtkTreeIter iter;
