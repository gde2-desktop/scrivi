/*
 * scrivi-file-browser-view.c - Scrivi plugin providing easy file access 
 * from the sidepanel
 *
...
#define SCRIVI_FILE_BROWSER_VIEW_GET_PRIVATE(object)( \
		G_TYPE_INSTANCE_GET_PRIVATE((object), \
		SCRIVI_TYPE_FILE_BROWSER_VIEW, ScriviFileBrowserViewPrivate))

struct _ScriviFileBrowserViewPrivate 
{
	...
	/* CLick policy */
	ScriviFileBrowserViewClickPolicy click_policy;
...
SCRIVI_PLUGIN_DEFINE_TYPE (ScriviFileBrowserView, scrivi_file_browser_view,
	                  GTK_TYPE_TREE_VIEW)

static void on_cell_edited 		(GtkCellRendererText 	* cell, 
				 	 gchar 			* path,
				 	 gchar 			* new_text,
				 	ScriviFileBrowserView 	* tree_view);

static void on_begin_refresh 		(ScriviFileBrowserStore 	* model, 
					 ScriviFileBrowserView 	* view);
static void on_end_refresh 		(ScriviFileBrowserStore 	* model, 
					 ScriviFileBrowserView 	* view);

static void on_unload			(ScriviFileBrowserStore 	* model, 
					 gchar const		* uri,
					 ScriviFileBrowserView 	* view);

static void on_row_inserted		(ScriviFileBrowserStore 	* model, 
					 GtkTreePath		* path,
					 GtkTreeIter		* iter,
					 ScriviFileBrowserView 	* view);
		 
static void
scrivi_file_browser_view_finalize (GObject * object)
{
	ScriviFileBrowserView *obj = SCRIVI_FILE_BROWSER_VIEW(object);
	...
	G_OBJECT_CLASS (scrivi_file_browser_view_parent_class)->
	    finalize (object);
}

static void
add_expand_state (ScriviFileBrowserView * view,
		  gchar const * uri)
{
	GFile * file;
...
static void
remove_expand_state (ScriviFileBrowserView * view,
		     gchar const * uri)
{
	GFile * file;
...
row_expanded (GtkTreeView * tree_view,
	      ...
	      GtkTreePath * path)
{
	ScriviFileBrowserView *view = SCRIVI_FILE_BROWSER_VIEW (tree_view);
	gchar * uri;

	if (GTK_TREE_VIEW_CLASS (scrivi_file_browser_view_parent_class)->row_expanded)
		GTK_TREE_VIEW_CLASS (scrivi_file_browser_view_parent_class)->row_expanded (tree_view, iter, path);

	if (!SCRIVI_IS_FILE_BROWSER_STORE (view->priv->model))
		return;
	...
	_scrivi_file_browser_store_iter_expanded (SCRIVI_FILE_BROWSER_STORE (view->priv->model),
						 iter);
}
	       ...
	       GtkTreePath * path)
{
	...
	if (GTK_TREE_VIEW_CLASS (scrivi_file_browser_view_parent_class)->row_collapsed)
		GTK_TREE_VIEW_CLASS (scrivi_file_browser_view_parent_class)->row_collapsed (tree_view, iter, path);
	...
	_scrivi_file_browser_store_iter_collapsed (SCRIVI_FILE_BROWSER_STORE (view->priv->model),
						  iter);
}
...
{
	ScriviFileBrowserView *view = SCRIVI_FILE_BROWSER_VIEW (widget);

	if (view->priv->click_policy == SCRIVI_FILE_BROWSER_VIEW_CLICK_POLICY_SINGLE &&
	...
	// Chainup
	return GTK_WIDGET_CLASS (scrivi_file_browser_view_parent_class)->leave_notify_event (widget, event);
}
		    ...
		    GdkEventCrossing *event)
{
	...
	// Chainup
	return GTK_WIDGET_CLASS (scrivi_file_browser_view_parent_class)->enter_notify_event (widget, event);
}
		     ...
		     GdkEventMotion * event)
{
	...
	// Chainup
	return GTK_WIDGET_CLASS (scrivi_file_browser_view_parent_class)->motion_notify_event (widget, event);
}

static void
set_click_policy_property (ScriviFileBrowserView            *obj,
			   ScriviFileBrowserViewClickPolicy  click_policy)
{
	GtkTreeIter iter;
	GdkDisplay *display;
	...
	if (click_policy == SCRIVI_FILE_BROWSER_VIEW_CLICK_POLICY_SINGLE) {
...
#endif
	} else if (click_policy == SCRIVI_FILE_BROWSER_VIEW_CLICK_POLICY_DOUBLE) {
...
static void
directory_activated (ScriviFileBrowserView *view, 
		     GtkTreeIter          *iter)
{
	scrivi_file_browser_store_set_virtual_root (SCRIVI_FILE_BROWSER_STORE (view->priv->model), iter);
}

static void
activate_selected_files (ScriviFileBrowserView *view) {
	...
	for (row = rows; row; row = row->next) {
		...
		gtk_tree_model_get (view->priv->model, &iter, SCRIVI_FILE_BROWSER_STORE_COLUMN_FLAGS, &flags, -1);
...
static void
activate_selected_bookmark (ScriviFileBrowserView *view) {
	GtkTreeView *tree_view = GTK_TREE_VIEW (view);
	GtkTreeSelection *selection = gtk_tree_view_get_selection (tree_view);
	GtkTreeIter iter;
...
static void
activate_selected_items (ScriviFileBrowserView *view)
{
	if (SCRIVI_IS_FILE_BROWSER_STORE (view->priv->model))
		activate_selected_files (view);
	else if (SCRIVI_IS_FILE_BOOKMARKS_STORE (view->priv->model))
		activate_selected_bookmark (view);
}

static void
toggle_hidden_filter (ScriviFileBrowserView *view)
{
	ScriviFileBrowserStoreFilterMode mode;

	if (SCRIVI_IS_FILE_BROWSER_STORE (view->priv->model))
	{
		mode = scrivi_file_browser_store_get_filter_mode
			(SCRIVI_FILE_BROWSER_STORE (view->priv->model));
		mode ^=	SCRIVI_FILE_BROWSER_STORE_FILTER_MODE_HIDE_HIDDEN;
		scrivi_file_browser_store_set_filter_mode
			(SCRIVI_FILE_BROWSER_STORE (view->priv->model), mode);
	}
}
	    ...
	    GdkDragContext *context)
{
	...
	/* Chain up */
	GTK_WIDGET_CLASS (scrivi_file_browser_view_parent_class)->drag_begin (widget, context);
}

static void
did_not_drag (ScriviFileBrowserView *view,
		      ...
		      GdkEventButton *event)
{
	...
	/* Chain up */
	return GTK_WIDGET_CLASS (scrivi_file_browser_view_parent_class)->button_release_event (widget, event);		
}
		    ...
		    GdkEventButton *event)
{
	...
	gboolean selected;
	GtkWidgetClass *widget_parent = GTK_WIDGET_CLASS(scrivi_file_browser_view_parent_class);

	tree_view = GTK_TREE_VIEW (widget);
	view = SCRIVI_FILE_BROWSER_VIEW (widget);
	selection = gtk_tree_view_get_selection (tree_view);
		...
		if (event->type == GDK_2BUTTON_PRESS) {
			...
			if ((event->button == 1 || event->button == 2) &&
				...
				if (selected) {
					...
					view->priv->ignore_release = call_parent && view->priv->click_policy != SCRIVI_FILE_BROWSER_VIEW_CLICK_POLICY_SINGLE;
		 ...
		 GdkEventKey *event)
{
	...
	/* Chain up */
	if (!handled)
		return GTK_WIDGET_CLASS (scrivi_file_browser_view_parent_class)->key_press_event (widget, event);
	
	return TRUE;
}

static void
fill_expand_state (ScriviFileBrowserView * view, GtkTreeIter * iter)
{
	GtkTreePath * path;
	GtkTreeIter child;
	gchar * uri;
...
static void
uninstall_restore_signals (ScriviFileBrowserView * tree_view,
			   GtkTreeModel * model)
{
...
static void
install_restore_signals (ScriviFileBrowserView * tree_view,
			 GtkTreeModel * model)
{
...
static void
set_restore_expand_state (ScriviFileBrowserView * view,
			  gboolean state)
{
	...
	if (state)
	{
		...
		if (view->priv->model && SCRIVI_IS_FILE_BROWSER_STORE (view->priv->model))
		{
	...
	else if (view->priv->model && SCRIVI_IS_FILE_BROWSER_STORE (view->priv->model))
	{
...
static void
scrivi_file_browser_view_class_init (ScriviFileBrowserViewClass * klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkTreeViewClass *tree_view_class = GTK_TREE_VIEW_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);
	
	object_class->finalize = scrivi_file_browser_view_finalize;
	...
	g_object_class_install_property (object_class, PROP_CLICK_POLICY,
					 g_param_spec_enum ("click-policy",
					 		    "Click Policy",
					 		    "The click policy",
					 		     SCRIVI_TYPE_FILE_BROWSER_VIEW_CLICK_POLICY,
	...
	signals[ERROR] =
	    g_signal_new ("error",
			  ...
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (ScriviFileBrowserViewClass,
					   error), NULL, NULL,
	      ...
	      ScriviFileBrowserView * obj)
{
	...
	if (obj->priv->click_policy == SCRIVI_FILE_BROWSER_VIEW_CLICK_POLICY_SINGLE) {
		if (obj->priv->hover_path != NULL && 
		    gtk_tree_path_compare (path, obj->priv->hover_path) == 0)
			underline = PANGO_UNDERLINE_SINGLE;	
	}

	if (SCRIVI_IS_FILE_BROWSER_STORE (tree_model))
	{
...
static void
scrivi_file_browser_view_init (ScriviFileBrowserView * obj)
{
#if GTK_CHECK_VERSION (3, 16, 0)
	GdkDisplay *display;
#endif
	obj->priv = SCRIVI_FILE_BROWSER_VIEW_GET_PRIVATE (obj);
			  ...
			  gpointer user_data)
{
	...
	return (flags & SCRIVI_FILE_BOOKMARKS_STORE_IS_SEPARATOR);
}

/* Public */
GtkWidget *
scrivi_file_browser_view_new (void)
{
	ScriviFileBrowserView *obj =
	    SCRIVI_FILE_BROWSER_VIEW (g_object_new
				     (SCRIVI_TYPE_FILE_BROWSER_VIEW, NULL));

	return GTK_WIDGET (obj);
}

void
scrivi_file_browser_view_set_model (ScriviFileBrowserView * tree_view,
				   GtkTreeModel * model)
{
	GtkTreeSelection *selection;
	...
	if (SCRIVI_IS_FILE_BROWSER_STORE (tree_view->priv->model)) {
		if (tree_view->priv->restore_expand_state)
...
void
scrivi_file_browser_view_start_rename (ScriviFileBrowserView * tree_view,
				      GtkTreeIter * iter)
{
	guint flags;
	...
	g_return_if_fail (SCRIVI_IS_FILE_BROWSER_VIEW (tree_view));
	g_return_if_fail (SCRIVI_IS_FILE_BROWSER_STORE
			  (tree_view->priv->model));
...
void
scrivi_file_browser_view_set_click_policy (ScriviFileBrowserView *tree_view,
...
void
scrivi_file_browser_view_set_restore_expand_state (ScriviFileBrowserView * tree_view,
						  gboolean restore_expand_state)
{
	g_return_if_fail (SCRIVI_IS_FILE_BROWSER_VIEW (tree_view));
		...
		ScriviFileBrowserView * tree_view)
{
	...
	if (ret) {
		if (scrivi_file_browser_store_rename (SCRIVI_FILE_BROWSER_STORE (tree_view->priv->model),
		    &iter, new_text, &error)) {
...
static void 
on_begin_refresh (ScriviFileBrowserStore * model, 
		  ScriviFileBrowserView * view)
{
...
static void 
on_end_refresh (ScriviFileBrowserStore * model, 
		ScriviFileBrowserView * view)
{
	/* Store the refresh state, so we can handle unloading of nodes while
	   refreshing properly */
...
static void
on_unload (ScriviFileBrowserStore * model, 
	   gchar const * uri,
	   ScriviFileBrowserView * view)
{
...
static void
restore_expand_state (ScriviFileBrowserView * view,
...
static void 
on_row_inserted (ScriviFileBrowserStore * model, 
		 GtkTreePath * path,
		 GtkTreeIter * iter,
		 ScriviFileBrowserView * view)
{
