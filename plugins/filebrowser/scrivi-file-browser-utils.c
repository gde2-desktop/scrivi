...
GdkPixbuf *
scrivi_file_browser_utils_pixbuf_from_theme (gchar const * name,
                                            GtkIconSize size)
{
	gint width;
	GError *error = NULL;
...
GdkPixbuf *
scrivi_file_browser_utils_pixbuf_from_icon (GIcon * icon,
                                           GtkIconSize size)
{
	GdkPixbuf * ret = NULL;
	GtkIconTheme *theme;
...
GdkPixbuf *
scrivi_file_browser_utils_pixbuf_from_file (GFile * file,
                                           GtkIconSize size)
{
	GIcon * icon;
	GFileInfo * info;
	...
	if (icon != NULL)
		ret = scrivi_file_browser_utils_pixbuf_from_icon (icon, size);
...
gchar *
scrivi_file_browser_utils_file_basename (GFile * file)
{
	gchar *uri;
	gchar *ret;
	
	uri = g_file_get_uri (file);
	ret = scrivi_file_browser_utils_uri_basename (uri);
	g_free (uri);
...
gchar *
scrivi_file_browser_utils_uri_basename (gchar const * uri)
{
	return scrivi_utils_basename_for_display (uri);
}

gboolean
scrivi_file_browser_utils_confirmation_dialog (ScriviWindow * window,
                                              GtkMessageType type,
                                              gchar const *message,
