			...
			ScriviDocumentNewlineType newline_type)
{
	...
	out = scrivi_document_output_stream_new (doc);
	...
	type = scrivi_document_output_stream_detect_newline_type (SCRIVI_DOCUMENT_OUTPUT_STREAM (out));
...
test_empty ()
{
	test_consecutive_write ("", "", 10, SCRIVI_DOCUMENT_NEWLINE_TYPE_DEFAULT);
	test_consecutive_write ("\r\n", "", 10, SCRIVI_DOCUMENT_NEWLINE_TYPE_CR_LF);
	test_consecutive_write ("\r", "", 10, SCRIVI_DOCUMENT_NEWLINE_TYPE_CR);
	test_consecutive_write ("\n", "", 10, SCRIVI_DOCUMENT_NEWLINE_TYPE_LF);
}
