...
get_encoded_text (const gchar         *text,
                  gsize                nread,
		  const ScriviEncoding *to,
		  const ScriviEncoding *from,
		  ...
		  gboolean             care_about_error)
{
	...
	converter = g_charset_converter_new (scrivi_encoding_get_charset (to),
					     scrivi_encoding_get_charset (from),
					     NULL);
...
get_all_encodings ()
{
	...
	while (TRUE)
	{
		...
		enc = scrivi_encoding_get_from_index (i);
         ...
         const ScriviEncoding **guessed)
{
	...
	{
		encodings = NULL;
		encodings = g_slist_prepend (encodings, (gpointer)scrivi_encoding_get_from_charset (enc));
	}

	converter = scrivi_smart_charset_converter_new (encodings);
	...
	if (guessed != NULL)
		*guessed = scrivi_smart_charset_converter_get_guessed (converter);
...
do_test_roundtrip (const char *str, const char *charset)
{
	...
	ScriviSmartCharsetConverter *c2;
	gsize n, tot;
	GError *err;
	GSList *enc = NULL;
	...
	enc = g_slist_prepend (enc, (gpointer)scrivi_encoding_get_from_charset (charset));
	c2 = scrivi_smart_charset_converter_new (enc);
	g_slist_free (enc);
...
test_xxx_xxx ()
{
	...
	{
		do_test_roundtrip (TEXT_TO_CONVERT, scrivi_encoding_get_charset ((const ScriviEncoding *)l->data));
	}
...
test_empty ()
{
	...
	encodings = g_slist_prepend (encodings, (gpointer)scrivi_encoding_get_from_charset ("UTF-16"));
	encodings = g_slist_prepend (encodings, (gpointer)scrivi_encoding_get_from_charset ("ISO-8859-15"));

	out = do_test ("", NULL, encodings, 0, &guessed);

	g_assert_cmpstr (out, ==, "");

	g_assert (guessed == scrivi_encoding_get_utf8 ());
}
...
test_guessed ()
{
	...
	aux = get_encoded_text (TEXT_TO_GUESS, -1,
	                        scrivi_encoding_get_from_charset ("UTF-16"),
	                        scrivi_encoding_get_from_charset ("UTF-8"),
	                        &aux_len,
	                        TRUE);

	fail = get_encoded_text (aux, aux_len,
	                         scrivi_encoding_get_from_charset ("UTF-8"),
	                         scrivi_encoding_get_from_charset ("ISO-8859-15"),
	                         &fail_len,
	                         FALSE);
	...
	encs = g_slist_append (encs, (gpointer)scrivi_encoding_get_from_charset ("ISO-8859-15"));
	encs = g_slist_append (encs, (gpointer)scrivi_encoding_get_from_charset ("UTF-16"));

	aux2 = do_test (aux, NULL, encs, aux_len, &guessed);

	g_assert (guessed == scrivi_encoding_get_from_charset ("UTF-16"));
}
