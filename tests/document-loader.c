...
static void
on_document_loaded (ScriviDocument  *document,
                    GError         *error,
                    LoaderTestData *data)
{
	...
	{
		g_assert_cmpint (scrivi_document_get_newline_type (document),
		                 ==,
		                 data->newline_type);
	}
             ...
             gint         newline_type)
{
	...
	scrivi_document_load (document, uri, scrivi_encoding_get_utf8 (), 0, FALSE);
