...
#include "scrivi-encodings-combo-box.h"
#include "scrivi-language-manager.h"
#include "scrivi-prefs-manager-app.h"
#include "scrivi-debug.h"
#include "scrivi-enum-types.h"

#define SCRIVI_FILE_CHOOSER_DIALOG_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_FILE_CHOOSER_DIALOG, ScriviFileChooserDialogPrivate))
...
struct _ScriviFileChooserDialogPrivate
{
...
G_DEFINE_TYPE(ScriviFileChooserDialog, scrivi_file_chooser_dialog, GTK_TYPE_FILE_CHOOSER_DIALOG)

static void
scrivi_file_chooser_dialog_class_init (ScriviFileChooserDialogClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (object_class, sizeof(ScriviFileChooserDialogPrivate));
}

static void
create_option_menu (ScriviFileChooserDialog *dialog)
{
	GtkWidget *label;
	GtkWidget *menu;
	...
	menu = scrivi_encodings_combo_box_new (
...
static void
update_newline_visibility (ScriviFileChooserDialog *dialog)
{
                      ...
                      ScriviDocumentNewlineType  newline_type)
{
	...
	if (newline_type == SCRIVI_DOCUMENT_NEWLINE_TYPE_DEFAULT)
	{
...
static void
create_newline_combo (ScriviFileChooserDialog *dialog)
{
	...
	store = gtk_list_store_new (2, G_TYPE_STRING, SCRIVI_TYPE_DOCUMENT_NEWLINE_TYPE);
...
static void
create_extra_widget (ScriviFileChooserDialog *dialog)
{
...
static void
action_changed (ScriviFileChooserDialog *dialog,
		GParamSpec	       *pspec,
		gpointer		data)
{
...
static void
filter_changed (ScriviFileChooserDialog *dialog,
		GParamSpec	       *pspec,
		gpointer		data)
{
	GtkFileFilter *filter;

	if (!scrivi_prefs_manager_active_file_filter_can_set ())
		return;
	...
	if (filter != NULL)
	{
		...
		scrivi_debug_message (DEBUG_COMMANDS, "Active filter: %s (%d)", name, id);

		scrivi_prefs_manager_set_active_file_filter (id);
	}
}
		       ...
		       gpointer                 data)
{
	...
	if (known_mime_types == NULL)
	{
		...
		lm = scrivi_get_language_manager ();
...
static void
scrivi_file_chooser_dialog_init (ScriviFileChooserDialog *dialog)
{
	dialog->priv = SCRIVI_FILE_CHOOSER_DIALOG_GET_PRIVATE (dialog);
}

static GtkWidget *
scrivi_file_chooser_dialog_new_valist (const gchar          *title,
				      GtkWindow            *parent,
				      GtkFileChooserAction  action,
				      ...
				      va_list               varargs)
{
	...
	result = g_object_new (SCRIVI_TYPE_FILE_CHOOSER_DIALOG,
			       "title", title,
#if !GTK_CHECK_VERSION (3, 0, 0)
	...
	create_extra_widget (SCRIVI_FILE_CHOOSER_DIALOG (result));
	...
	if (encoding != NULL)
		scrivi_encodings_combo_box_set_selected_encoding (
				SCRIVI_ENCODINGS_COMBO_BOX (SCRIVI_FILE_CHOOSER_DIALOG (result)->priv->option_menu),
				encoding);

	active_filter = scrivi_prefs_manager_get_active_file_filter ();
	scrivi_debug_message (DEBUG_COMMANDS, "Active filter: %d", active_filter);
...
/**
 * scrivi_file_chooser_dialog_new:
 ...
 *
 * Creates a new #ScriviFileChooserDialog.  This function is analogous to
 * gtk_dialog_new_with_buttons().
 *
 * Return value: a new #ScriviFileChooserDialog
 *
 **/
GtkWidget *
scrivi_file_chooser_dialog_new (const gchar          *title,
			       GtkWindow            *parent,
			       GtkFileChooserAction  action,
			       const ScriviEncoding  *encoding,
			       const gchar          *first_button_text,
			       ...)
{
	...
	result = scrivi_file_chooser_dialog_new_valist (title, parent, action,
...
void
scrivi_file_chooser_dialog_set_encoding (ScriviFileChooserDialog *dialog,
					const ScriviEncoding    *encoding)
{
	g_return_if_fail (SCRIVI_IS_FILE_CHOOSER_DIALOG (dialog));
	g_return_if_fail (SCRIVI_IS_ENCODINGS_COMBO_BOX (dialog->priv->option_menu));

	scrivi_encodings_combo_box_set_selected_encoding (
				SCRIVI_ENCODINGS_COMBO_BOX (dialog->priv->option_menu),
				encoding);
}
...
{
	g_return_val_if_fail (SCRIVI_IS_FILE_CHOOSER_DIALOG (dialog), NULL);
	g_return_val_if_fail (SCRIVI_IS_ENCODINGS_COMBO_BOX (dialog->priv->option_menu), NULL);
	...
	return scrivi_encodings_combo_box_get_selected_encoding (
				SCRIVI_ENCODINGS_COMBO_BOX (dialog->priv->option_menu));
}

void
scrivi_file_chooser_dialog_set_newline_type (ScriviFileChooserDialog  *dialog,
					    ScriviDocumentNewlineType newline_type)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
...
scrivi_file_chooser_dialog_get_newline_type (ScriviFileChooserDialog *dialog)
{
	...
	g_return_val_if_fail (SCRIVI_IS_FILE_CHOOSER_DIALOG (dialog), SCRIVI_DOCUMENT_NEWLINE_TYPE_DEFAULT);
