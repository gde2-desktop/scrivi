...
list_encodings_and_quit (void)
{
	...
	while ((enc = scrivi_encoding_get_from_index (i)) != NULL)
	{
...
static const GOptionEntry options [] =
{
	...
	{ "new-window", '\0', 0, G_OPTION_ARG_NONE, &new_window_option,
	  N_("Create a new top-level window in an existing instance of scrivi"), NULL },

	{ "new-document", '\0', 0, G_OPTION_ARG_NONE, &new_document_option,
	  N_("Create a new document in an existing instance of scrivi"), NULL },
...
static void
scrivi_get_command_line_data (void)
{
	if (remaining_args)
	{
		gint i;
	...
	if (encoding_charset &&
	    (scrivi_encoding_get_from_charset (encoding_charset) == NULL))
	{
...
on_message_received (const char *message,
		     gpointer    data)
{
	const ScriviEncoding *encoding = NULL;
	gchar **commands;
	gchar **params;
	gint workspace;
	gint viewport_x;
	...
	scrivi_debug_message (DEBUG_APP, "Received message:\n%s\n", message);
	...
	for (i = 1; commands[i] != NULL; i++)
	{
		...
		else if (strcmp (params[0], "OPEN-URIS") == 0)
		{
			...
			if (params[2] != '\0')
				encoding = scrivi_encoding_get_from_charset (params[2]);
	...
	else
	{
		...
		window = _scrivi_app_get_window_in_viewport (app,
							    screen,
							    workspace,
							    viewport_x,
							    viewport_y);
	...
	if (file_list != NULL)
	{
		_scrivi_cmd_load_files_from_prompt (window,
						   file_list,
						   encoding,
						   line_position);
	...
	else
	{
		...
		if (doc == NULL ||
		    !scrivi_document_is_untouched (doc) ||
		    new_document_option)
			scrivi_window_create_tab (window, TRUE);
	}
...
send_bacon_message (void)
{
	...
	scrivi_debug_message (DEBUG_APP, "Display: %s", display_name);
	scrivi_debug_message (DEBUG_APP, "Screen: %d", screen_number);

	ws = scrivi_utils_get_current_workspace (screen);
	scrivi_utils_get_current_viewport (screen, &viewport_x, &viewport_y);
	...
	scrivi_debug_message (DEBUG_APP, "Bacon Message: %s", command->str);
...
main (int argc, char *argv[])
{
	...
	/* Setup debugging */
	scrivi_debug_init ();
	scrivi_debug_message (DEBUG_APP, "Startup");

	setlocale (LC_ALL, "");

	dir = scrivi_dirs_get_scrivi_locale_dir ();
	...
	scrivi_debug_message (DEBUG_APP, "Create bacon connection");

	connection = bacon_message_connection_new ("scrivi");

	if (connection != NULL)
	{
		if (!bacon_message_connection_get_is_server (connection))
		{
			scrivi_debug_message (DEBUG_APP, "I'm a client");

			scrivi_get_command_line_data ();
		...
		{
		  	scrivi_debug_message (DEBUG_APP, "I'm a server");
	...
	{
		g_warning ("Cannot create the 'scrivi' connection.");
	}

	scrivi_debug_message (DEBUG_APP, "Set icon");

	dir = scrivi_dirs_get_scrivi_data_dir ();
	...
	egg_set_desktop_file (DATADIR "/applications/scrivi.desktop");

	/* Load user preferences */
	scrivi_debug_message (DEBUG_APP, "Init prefs manager");
	scrivi_prefs_manager_app_init ();

	/* Init plugins engine */
	scrivi_debug_message (DEBUG_APP, "Init plugins");
	engine = scrivi_plugins_engine_get_default ();

	/* Initialize session management */
	scrivi_debug_message (DEBUG_APP, "Init session manager");
	scrivi_session_init ();

	if (scrivi_session_is_restored ())
		restored = scrivi_session_load ();

	if (!restored)
	{
		scrivi_debug_message (DEBUG_APP, "Analyze command line data");
		scrivi_get_command_line_data ();

		scrivi_debug_message (DEBUG_APP, "Get default app");
		app = scrivi_app_get_default ();

		scrivi_debug_message (DEBUG_APP, "Create main window");
		window = scrivi_app_create_window (app, NULL);
		...
		if (file_list != NULL)
		{
			...
			if (encoding_charset)
				encoding = scrivi_encoding_get_from_charset (encoding_charset);

			scrivi_debug_message (DEBUG_APP, "Load files");
			_scrivi_cmd_load_files_from_prompt (window,
							   file_list,
							   encoding,
							   line_position);
		...
		{
			scrivi_debug_message (DEBUG_APP, "Create tab");
			scrivi_window_create_tab (window, TRUE);
		}

		scrivi_debug_message (DEBUG_APP, "Show window");
		gtk_widget_show (GTK_WIDGET (window));

		free_command_line_data ();
	}

	scrivi_debug_message (DEBUG_APP, "Start gtk-main");
	...
	scrivi_prefs_manager_app_shutdown ();

#ifndef ENABLE_GVFS_METADATA
	scrivi_metadata_manager_shutdown ();
#endif
