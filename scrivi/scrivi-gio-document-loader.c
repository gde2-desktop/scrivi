...
#include "scrivi-gio-document-loader.h"
#include "scrivi-document-output-stream.h"
#include "scrivi-smart-charset-converter.h"
...
typedef struct
{
	ScriviGioDocumentLoader *loader;
...
#define REMOTE_QUERY_ATTRIBUTES G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE "," \
				...
				SCRIVI_METADATA_ATTRIBUTE_ENCODING

#define SCRIVI_GIO_DOCUMENT_LOADER_GET_PRIVATE(object) \
				(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
				 SCRIVI_TYPE_GIO_DOCUMENT_LOADER,   \
				 ScriviGioDocumentLoaderPrivate))

static void	    scrivi_gio_document_loader_load		(ScriviDocumentLoader *loader);
static gboolean     scrivi_gio_document_loader_cancel		(ScriviDocumentLoader *loader);
static goffset      scrivi_gio_document_loader_get_bytes_read	(ScriviDocumentLoader *loader);

static void open_async_read (AsyncData *async);

struct _ScriviGioDocumentLoaderPrivate
{
	...
	ScriviSmartCharsetConverter *converter;
...
G_DEFINE_TYPE(ScriviGioDocumentLoader, scrivi_gio_document_loader, SCRIVI_TYPE_DOCUMENT_LOADER)

static void
scrivi_gio_document_loader_dispose (GObject *object)
{
	ScriviGioDocumentLoaderPrivate *priv;

	priv = SCRIVI_GIO_DOCUMENT_LOADER (object)->priv;
	...
	G_OBJECT_CLASS (scrivi_gio_document_loader_parent_class)->dispose (object);
}

static void
scrivi_gio_document_loader_class_init (ScriviGioDocumentLoaderClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	ScriviDocumentLoaderClass *loader_class = SCRIVI_DOCUMENT_LOADER_CLASS (klass);

	object_class->dispose = scrivi_gio_document_loader_dispose;

	loader_class->load = scrivi_gio_document_loader_load;
	loader_class->cancel = scrivi_gio_document_loader_cancel;
	loader_class->get_bytes_read = scrivi_gio_document_loader_get_bytes_read;

	g_type_class_add_private (object_class, sizeof(ScriviGioDocumentLoaderPrivate));
}

static void
scrivi_gio_document_loader_init (ScriviGioDocumentLoader *gvloader)
{
	gvloader->priv = SCRIVI_GIO_DOCUMENT_LOADER_GET_PRIVATE (gvloader);
...
static AsyncData *
async_data_new (ScriviGioDocumentLoader *gvloader)
{
	AsyncData *async;
...
static const ScriviEncoding *
get_metadata_encoding (ScriviDocumentLoader *loader)
{
	const ScriviEncoding *enc = NULL;

#ifndef ENABLE_GVFS_METADATA
	gchar *charset;
	const gchar *uri;

	uri = scrivi_document_loader_get_uri (loader);

	charset = scrivi_metadata_manager_get (uri, "encoding");
	...
	if (g_file_info_has_attribute (info, SCRIVI_METADATA_ATTRIBUTE_ENCODING))
	{
		const gchar *charset;
...
static void
remote_load_completed_or_failed (ScriviGioDocumentLoader *loader, AsyncData *async)
{
	scrivi_document_loader_loading (SCRIVI_DOCUMENT_LOADER (loader),
				       TRUE,
				       loader->priv->error);
			     ...
			     AsyncData     *async)
{
	...
	scrivi_debug_message (DEBUG_SAVER, "Finished closing input stream");
...
{
	ScriviGioDocumentLoader *gvloader;
	 ...
	 */
	scrivi_document_loader_loading (SCRIVI_DOCUMENT_LOADER (gvloader),
				       FALSE,
				       NULL);
	       ...
	       AsyncData    *async)
{
	...
	if (async->read == 0)
	{
		...
		loader = SCRIVI_DOCUMENT_LOADER (gvloader);
		...
		loader->auto_detected_encoding =
			scrivi_smart_charset_converter_get_guessed (gvloader->priv->converter);

		loader->auto_detected_newline_type =
			scrivi_document_output_stream_detect_newline_type (SCRIVI_DOCUMENT_OUTPUT_STREAM (gvloader->priv->output));
		...
		/*if ((scrivi_smart_charset_converter_get_num_fallbacks (gvloader->priv->converter) != 0) &&
		    gvloader->priv->error == NULL)
		{
...
static GSList *
get_candidate_encodings (ScriviGioDocumentLoader *gvloader)
{
	const ScriviEncoding *metadata;
	GSList *encodings = NULL;

	encodings = scrivi_prefs_manager_get_auto_detected_encodings ();

	metadata = get_metadata_encoding (SCRIVI_DOCUMENT_LOADER (gvloader));
	if (metadata != NULL)
	{
...
finish_query_info (AsyncData *async)
{
	...
	gvloader->priv->converter = scrivi_smart_charset_converter_new (candidate_encodings);
	...
	/* Output stream */
	gvloader->priv->output = scrivi_document_output_stream_new (loader->document);
	       ...
	       AsyncData    *async)
{
	...
	SCRIVI_DOCUMENT_LOADER (gvloader)->info = info;
...
recover_not_mounted (AsyncData *async)
{
	...
	doc = scrivi_document_loader_get_document (SCRIVI_DOCUMENT_LOADER (async->loader));
...
static void
scrivi_gio_document_loader_load (ScriviDocumentLoader *loader)
{
	ScriviGioDocumentLoader *gvloader = SCRIVI_GIO_DOCUMENT_LOADER (loader);
	AsyncData *async;
...
static goffset
scrivi_gio_document_loader_get_bytes_read (ScriviDocumentLoader *loader)
{
	return SCRIVI_GIO_DOCUMENT_LOADER (loader)->priv->bytes_read;
}

static gboolean
scrivi_gio_document_loader_cancel (ScriviDocumentLoader *loader)
{
	ScriviGioDocumentLoader *gvloader = SCRIVI_GIO_DOCUMENT_LOADER (loader);
