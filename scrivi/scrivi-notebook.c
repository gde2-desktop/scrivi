...
#define SCRIVI_NOTEBOOK_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_NOTEBOOK, ScriviNotebookPrivate))

struct _ScriviNotebookPrivate
{
	GList         *focused_pages;
...
G_DEFINE_TYPE(ScriviNotebook, scrivi_notebook, GTK_TYPE_NOTEBOOK)

static void scrivi_notebook_finalize (GObject *object);

static gboolean scrivi_notebook_change_current_page (GtkNotebook *notebook,
						    gint         offset);

static void move_current_tab_to_another_notebook  (ScriviNotebook  *src,
						   ScriviNotebook  *dest,
...
static void
scrivi_notebook_dispose (GObject *object)
{
	ScriviNotebook *notebook = SCRIVI_NOTEBOOK (object);
	...
	if (!notebook->priv->destroy_has_run)
	{
		...
		{
			scrivi_notebook_remove_tab (notebook,
						   SCRIVI_TAB (l->data));
		}
	...
	G_OBJECT_CLASS (scrivi_notebook_parent_class)->dispose (object);
}

static void
scrivi_notebook_class_init (ScriviNotebookClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkNotebookClass *notebook_class = GTK_NOTEBOOK_CLASS (klass);

	object_class->finalize = scrivi_notebook_finalize;
	object_class->dispose = scrivi_notebook_dispose;

	notebook_class->change_current_page = scrivi_notebook_change_current_page;

	signals[TAB_ADDED] =
		g_signal_new ("tab_added",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviNotebookClass, tab_added),
			      NULL, NULL,
	...
	signals[TAB_REMOVED] =
		g_signal_new ("tab_removed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviNotebookClass, tab_removed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	signals[TAB_DETACHED] =
		g_signal_new ("tab_detached",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviNotebookClass, tab_detached),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	signals[TABS_REORDERED] =
		g_signal_new ("tabs_reordered",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviNotebookClass, tabs_reordered),
			      NULL, NULL,
	...
	signals[TAB_CLOSE_REQUEST] =
		g_signal_new ("tab-close-request",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviNotebookClass, tab_close_request),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	g_type_class_add_private (object_class, sizeof(ScriviNotebookPrivate));
}

static ScriviNotebook *
find_notebook_at_pointer (gint abs_x, gint abs_y)
{
	...
	/* toplevel should be an ScriviWindow */
	if ((toplevel != NULL) && 
	    SCRIVI_IS_WINDOW (toplevel))
	{
		return SCRIVI_NOTEBOOK (_scrivi_window_get_notebook
						(SCRIVI_WINDOW (toplevel)));
	}
...
static gboolean
is_in_notebook_window (ScriviNotebook *notebook,
		       gint           abs_x, 
		       gint           abs_y)
{
	ScriviNotebook *nb_at_pointer;
...
static gint
find_tab_num_at_pos (ScriviNotebook *notebook, 
		     gint           abs_x, 
		     gint           abs_y)
{
...
/**
 * scrivi_notebook_move_tab:
 * @src: a #ScriviNotebook
 * @dest: a #ScriviNotebook
 * @tab: a #ScriviTab
 ...
 */
void
scrivi_notebook_move_tab (ScriviNotebook *src,
			 ScriviNotebook *dest,
			 ScriviTab      *tab,
			 gint           dest_position)
{
	g_return_if_fail (SCRIVI_IS_NOTEBOOK (src));	
	g_return_if_fail (SCRIVI_IS_NOTEBOOK (dest));
	g_return_if_fail (src != dest);
	g_return_if_fail (SCRIVI_IS_TAB (tab));

	/* make sure the tab isn't destroyed while we move it */
	g_object_ref (tab);
	scrivi_notebook_remove_tab (src, tab);
	scrivi_notebook_add_tab (dest, tab, dest_position, TRUE);
	g_object_unref (tab);
}

/**
 * scrivi_notebook_reorder_tab:
 * @src: a #ScriviNotebook
 * @tab: a #ScriviTab
 * @dest_position: the position for @tab
 *
 ...
 */
void
scrivi_notebook_reorder_tab (ScriviNotebook *src,
			    ScriviTab      *tab,
			    gint           dest_position)
{
	gint old_position;
...
static void
drag_start (ScriviNotebook *notebook,
	    guint32        time)
{
...
static void
drag_stop (ScriviNotebook *notebook)
{
 ...
 */
static void
move_current_tab (ScriviNotebook *notebook,
	          gint           dest_position)
{
	gint cur_page_num;
	...
	if (dest_position != cur_page_num)
	{
		...
		scrivi_notebook_reorder_tab (SCRIVI_NOTEBOOK (notebook),
					    SCRIVI_TAB (cur_tab),
					    dest_position);
	}
}
...
static gboolean
motion_notify_cb (ScriviNotebook  *notebook,
		  GdkEventMotion *event,
		  gpointer        data)
{
	ScriviNotebook *dest;
...
static void
move_current_tab_to_another_notebook (ScriviNotebook  *src,
				      ScriviNotebook  *dest,
				      GdkEventMotion *event,
				      gint            dest_position)
{
	...
	tab = SCRIVI_TAB (gtk_notebook_get_nth_page (GTK_NOTEBOOK (src), 
						    cur_page));
	...
	scrivi_notebook_move_tab (src, dest, tab, dest_position);
...
static gboolean
button_release_cb (ScriviNotebook  *notebook,
...
static gboolean
button_press_cb (ScriviNotebook  *notebook,
		 GdkEventButton *event,
		 gpointer        data)
{
	gint tab_clicked;
 ...
 * scrivi_notebook_new:
 *
 * Creates a new #ScriviNotebook object.
 *
 * Returns: a new #ScriviNotebook
 */
GtkWidget *
scrivi_notebook_new (void)
{
	return GTK_WIDGET (g_object_new (SCRIVI_TYPE_NOTEBOOK, NULL));
}

static void
scrivi_notebook_switch_page_cb (GtkNotebook     *notebook,
                               GtkWidget       *page,
                               guint            page_num,
                               gpointer         data)
{
	ScriviNotebook *nb = SCRIVI_NOTEBOOK (notebook);
	GtkWidget *child;
	ScriviView *view;
	...
	view = scrivi_tab_get_view (SCRIVI_TAB (child));
 ...
 */
static void
update_tabs_visibility (ScriviNotebook *nb, 
...
static void
scrivi_notebook_init (ScriviNotebook *notebook)
{
	notebook->priv = SCRIVI_NOTEBOOK_GET_PRIVATE (notebook);
	...
	g_signal_connect_after (G_OBJECT (notebook), 
				"switch_page",
                                G_CALLBACK (scrivi_notebook_switch_page_cb),
                                NULL);
}

static void
scrivi_notebook_finalize (GObject *object)
{
	ScriviNotebook *notebook = SCRIVI_NOTEBOOK (object);

	g_list_free (notebook->priv->focused_pages);

	G_OBJECT_CLASS (scrivi_notebook_parent_class)->finalize (object);
}
 ...
 */
static gboolean
scrivi_notebook_change_current_page (GtkNotebook *notebook,
				    gint         offset)
{
...
static void
close_button_clicked_cb (ScriviTabLabel *tab_label, ScriviNotebook *notebook)
{
	ScriviTab *tab;

	tab = scrivi_tab_label_get_tab (tab_label);
	g_signal_emit (notebook, signals[TAB_CLOSE_REQUEST], 0, tab);
}

static GtkWidget *
create_tab_label (ScriviNotebook *nb,
		  ScriviTab      *tab)
{
	GtkWidget *tab_label;

	tab_label = scrivi_tab_label_new (tab);
...
static void
remove_tab_label (ScriviNotebook *nb,
		  ScriviTab      *tab)
{
	GtkWidget *tab_label;
...
/**
 * scrivi_notebook_set_always_show_tabs:
 * @nb: a #ScriviNotebook
 ...
 */
void
scrivi_notebook_set_always_show_tabs (ScriviNotebook *nb, 
				     gboolean       show_tabs)
{
	g_return_if_fail (SCRIVI_IS_NOTEBOOK (nb));
 ...
 */
void
scrivi_notebook_add_tab (ScriviNotebook *nb,
		        ScriviTab      *tab,
		        gint           position,
		        gboolean       jump_to)
{
...
static void
smart_tab_switching_on_closure (ScriviNotebook *nb,
				ScriviTab      *tab)
{
	gboolean jump_to;
 ...
 */
void
scrivi_notebook_remove_tab (ScriviNotebook *nb,
			   ScriviTab      *tab)
{
	gint position, curr;
...
/**
 * scrivi_notebook_remove_all_tabs:
 * @nb: a #ScriviNotebook
 *
 * Removes all #ScriviTab from @nb.
 */
void
scrivi_notebook_remove_all_tabs (ScriviNotebook *nb)
{	
	g_return_if_fail (SCRIVI_IS_NOTEBOOK (nb));
...
static void
set_close_buttons_sensitivity (ScriviTab      *tab,
                               ScriviNotebook *nb)
{
	GtkWidget *tab_label;
	...
	scrivi_tab_label_set_close_button_sensitive (SCRIVI_TAB_LABEL (tab_label),
						    nb->priv->close_buttons_sensitive);
}

/**
 * scrivi_notebook_set_close_buttons_sensitive:
 * @nb: a #ScriviNotebook
 ...
 */
void
scrivi_notebook_set_close_buttons_sensitive (ScriviNotebook *nb,
					    gboolean       sensitive)
{
	g_return_if_fail (SCRIVI_IS_NOTEBOOK (nb));
...
/**
 * scrivi_notebook_get_close_buttons_sensitive:
 * @nb: a #ScriviNotebook
 *
 ...
 */
gboolean
scrivi_notebook_get_close_buttons_sensitive (ScriviNotebook *nb)
{
	g_return_val_if_fail (SCRIVI_IS_NOTEBOOK (nb), TRUE);

	return nb->priv->close_buttons_sensitive;
}

/**
 * scrivi_notebook_set_tab_drag_and_drop_enabled:
 * @nb: a #ScriviNotebook
 ...
 */
void
scrivi_notebook_set_tab_drag_and_drop_enabled (ScriviNotebook *nb,
					      gboolean       enable)
{
	g_return_if_fail (SCRIVI_IS_NOTEBOOK (nb));
...
/**
 * scrivi_notebook_get_tab_drag_and_drop_enabled:
 * @nb: a #ScriviNotebook
 *
 ...
 */
gboolean	
scrivi_notebook_get_tab_drag_and_drop_enabled (ScriviNotebook *nb)
{
	g_return_val_if_fail (SCRIVI_IS_NOTEBOOK (nb), TRUE);
