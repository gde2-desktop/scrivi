...
#define SCRIVI_DOCUMENTS_PANEL_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						  SCRIVI_TYPE_DOCUMENTS_PANEL,            \
						  ScriviDocumentsPanelPrivate))

struct _ScriviDocumentsPanelPrivate
{
	ScriviWindow  *window;
...
G_DEFINE_TYPE(ScriviDocumentsPanel, scrivi_documents_panel, GTK_TYPE_BOX)
#else
G_DEFINE_TYPE(ScriviDocumentsPanel, scrivi_documents_panel, GTK_TYPE_VBOX)
#endif
...
static void
get_iter_from_tab (ScriviDocumentsPanel *panel, ScriviTab *tab, GtkTreeIter *iter)
{
	gint num;
	GtkWidget *nb;
	GtkTreePath *path;

	nb = _scrivi_window_get_notebook (panel->priv->window);
...
static void
window_active_tab_changed (ScriviWindow         *window,
			   ScriviTab            *tab,
			   ScriviDocumentsPanel *panel)
{	
	g_return_if_fail (tab != NULL);

	if (!_scrivi_window_is_removing_tabs (window))
	{
		GtkTreeIter iter;
...
static void
refresh_list (ScriviDocumentsPanel *panel)
{
	...
	active_tab = scrivi_window_get_active_tab (panel->priv->window);
	...
	{	
		...
		name = tab_get_name (SCRIVI_TAB (l->data));
		pixbuf = _scrivi_tab_get_icon (SCRIVI_TAB (l->data));
...
static void
sync_name_and_icon (ScriviTab            *tab,
		    GParamSpec          *pspec,
		    ScriviDocumentsPanel *panel)
{
	GdkPixbuf *pixbuf;
	...
	pixbuf = _scrivi_tab_get_icon (tab);
...
static void
window_tab_removed (ScriviWindow         *window,
		    ScriviTab            *tab,
		    ScriviDocumentsPanel *panel)
{
...
static void
window_tab_added (ScriviWindow         *window,
		  ScriviTab            *tab,
		  ScriviDocumentsPanel *panel)
{
	GtkTreeIter iter;
...
static void
window_tabs_reordered (ScriviWindow         *window,
		       ScriviDocumentsPanel *panel)
{
...
static void
set_window (ScriviDocumentsPanel *panel,
	    ScriviWindow         *window)
{
			 ...
			 ScriviDocumentsPanel *panel)
{
	...
	if (gtk_tree_selection_get_selected (selection, NULL, &iter))
	{
		...
		if (scrivi_window_get_active_tab (panel->priv->window) != tab)
		{
			scrivi_window_set_active_tab (panel->priv->window,
						     SCRIVI_TAB (tab));
		}
	}
}
...
static void
scrivi_documents_panel_set_property (GObject      *object,
				    guint         prop_id,
				    const GValue *value,
				    GParamSpec   *pspec)
{
	ScriviDocumentsPanel *panel = SCRIVI_DOCUMENTS_PANEL (object);
...
static void
scrivi_documents_panel_get_property (GObject    *object,
				    guint       prop_id,
				    GValue     *value,
				    GParamSpec *pspec)
{
	...
	switch (prop_id)
	{
		case PROP_WINDOW:
			g_value_set_object (value,
					    SCRIVI_DOCUMENTS_PANEL_GET_PRIVATE (panel)->window);
			break;
		default:
			G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
			break;
...
static void
scrivi_documents_panel_finalize (GObject *object)
{
	/* ScriviDocumentsPanel *tab = SCRIVI_DOCUMENTS_PANEL (object); */
	
	/* TODO: disconnect signal with window */

	G_OBJECT_CLASS (scrivi_documents_panel_parent_class)->finalize (object);
}

static void
scrivi_documents_panel_dispose (GObject *object)
{
	ScriviDocumentsPanel *panel = SCRIVI_DOCUMENTS_PANEL (object);
	...
	G_OBJECT_CLASS (scrivi_documents_panel_parent_class)->dispose (object);
}

static void 
scrivi_documents_panel_class_init (ScriviDocumentsPanelClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = scrivi_documents_panel_finalize;
	object_class->dispose = scrivi_documents_panel_dispose;
	object_class->get_property = scrivi_documents_panel_get_property;
	object_class->set_property = scrivi_documents_panel_set_property;

	g_object_class_install_property (object_class,
					 PROP_WINDOW,
					 g_param_spec_object ("window",
							      "Window",
							      "The ScriviWindow this ScriviDocumentsPanel is associated with",
							      SCRIVI_TYPE_WINDOW,
							      G_PARAM_READWRITE |
							      G_PARAM_CONSTRUCT_ONLY |
							      G_PARAM_STATIC_STRINGS));

	g_type_class_add_private (object_class, sizeof (ScriviDocumentsPanelPrivate));
}

static GtkTreePath *
get_current_path (ScriviDocumentsPanel *panel)
{
	gint num;
	GtkWidget *nb;
	GtkTreePath *path;
...
static gboolean
show_popup_menu (ScriviDocumentsPanel *panel,
		 GdkEventButton      *event)
{
	GtkWidget *menu;

	menu = gtk_ui_manager_get_widget (scrivi_window_get_ui_manager (panel->priv->window),
					 "/NotebookPopup");
			...
			gpointer    data)
{
	...
	tip = _scrivi_tab_get_tooltips (SCRIVI_TAB (tab));
		       ...
		       ScriviDocumentsPanel *panel)
{
	...
	tab = scrivi_window_get_active_tab (panel->priv->window);
	g_return_if_fail (tab != NULL);
	...
	scrivi_notebook_reorder_tab (SCRIVI_NOTEBOOK (nb),
				    tab,
				    new_position);
...
static void
scrivi_documents_panel_init (ScriviDocumentsPanel *panel)
{
	GtkWidget 		*sw;
	...
	panel->priv = SCRIVI_DOCUMENTS_PANEL_GET_PRIVATE (panel);
...
GtkWidget *
scrivi_documents_panel_new (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);

	return GTK_WIDGET (g_object_new (SCRIVI_TYPE_DOCUMENTS_PANEL,
					 "window", window,
					 NULL));
}
