...
struct _ScriviPrintPreviewPrivate
{
...
#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE (ScriviPrintPreview, scrivi_print_preview, GTK_TYPE_BOX)
#else
G_DEFINE_TYPE (ScriviPrintPreview, scrivi_print_preview, GTK_TYPE_VBOX)
#endif

static void 
scrivi_print_preview_get_property (GObject    *object,
				  guint       prop_id,
				  GValue     *value,
				  GParamSpec *pspec)
{
	//ScriviPrintPreview *preview = SCRIVI_PRINT_PREVIEW (object);
...
static void 
scrivi_print_preview_set_property (GObject      *object,
				  guint	        prop_id,
				  const GValue *value,
				  GParamSpec   *pspec)
{
...
static void
scrivi_print_preview_finalize (GObject *object)
{
	//ScriviPrintPreview *preview = SCRIVI_PRINT_PREVIEW (object);

	G_OBJECT_CLASS (scrivi_print_preview_parent_class)->finalize (object);
}

static void
scrivi_print_preview_grab_focus (GtkWidget *widget)
{
	ScriviPrintPreview *preview;

	preview = SCRIVI_PRINT_PREVIEW (widget);

	gtk_widget_grab_focus (GTK_WIDGET (preview->priv->layout));
}

static void						 
scrivi_print_preview_class_init (ScriviPrintPreviewClass *klass)
{
	...
	object_class->get_property = scrivi_print_preview_get_property;
	object_class->set_property = scrivi_print_preview_set_property;
	object_class->finalize = scrivi_print_preview_finalize;

	widget_class->grab_focus = scrivi_print_preview_grab_focus;

	g_type_class_add_private (object_class, sizeof(ScriviPrintPreviewPrivate));	
}

static void
update_layout_size (ScriviPrintPreview *preview)
{
...
static void
set_rows_and_cols (ScriviPrintPreview *preview,
		   gint	              rows,
		   gint	              cols)
{
...
static double
get_paper_width (ScriviPrintPreview *preview)
{
	return preview->priv->paper_w * preview->priv->dpi;
}

static double
get_paper_height (ScriviPrintPreview *preview)
{
...
static void
update_tile_size (ScriviPrintPreview *preview)
{
	ScriviPrintPreviewPrivate *priv;	
	gint w, h;
...
static void
set_zoom_factor (ScriviPrintPreview *preview,
		 double	            zoom)
{
	ScriviPrintPreviewPrivate *priv;	
...
static void
set_zoom_fit_to_size (ScriviPrintPreview *preview)
{
	ScriviPrintPreviewPrivate *priv;	
	double width, height;
...
static void
zoom_in (ScriviPrintPreview *preview)
{
...
static void
zoom_out (ScriviPrintPreview *preview)
{
	set_zoom_factor (preview,
...
static void
goto_page (ScriviPrintPreview *preview, gint page)
{
	gchar c[32];
...
static void
on_1x1_clicked (GtkMenuItem *i, ScriviPrintPreview *preview)
{
	set_rows_and_cols (preview, 1, 1);
}

static void
on_1x2_clicked (GtkMenuItem *i, ScriviPrintPreview *preview)
{
	set_rows_and_cols (preview, 1, 2);
}

static void
on_2x1_clicked (GtkMenuItem *i, ScriviPrintPreview *preview)
{
	set_rows_and_cols (preview, 2, 1);
}

static void
on_2x2_clicked (GtkMenuItem *i, ScriviPrintPreview *preview)
{
...
static void
create_bar (ScriviPrintPreview *preview)
{
	ScriviPrintPreviewPrivate *priv;
	GtkWidget *toolbar;
	GtkToolItem *i;
...
static gint
get_first_page_displayed (ScriviPrintPreview *preview)
{
	ScriviPrintPreviewPrivate *priv;
...
static gint
get_page_at_coords (ScriviPrintPreview *preview,
		    gint               x,
		    gint               y)
{
	ScriviPrintPreviewPrivate *priv;
...
static void
create_preview_layout (ScriviPrintPreview *preview)
{
	ScriviPrintPreviewPrivate *priv;
	AtkObject *atko;
...
static void
scrivi_print_preview_init (ScriviPrintPreview *preview)
{
	ScriviPrintPreviewPrivate *priv;
	
	priv = G_TYPE_INSTANCE_GET_PRIVATE (preview, 
					    SCRIVI_TYPE_PRINT_PREVIEW,
					    ScriviPrintPreviewPrivate);
    ...
    gtk_style_context_add_class (context, "scrivi-print-preview");
...
static double
get_screen_dpi (ScriviPrintPreview *preview)
{
	GdkScreen *screen;
	double dpi;
...
static void
set_n_pages (ScriviPrintPreview *preview,
	     gint               n_pages)
{
	gchar *str;
...
static void
update_paper_size (ScriviPrintPreview *preview,
...
static cairo_surface_t *
create_preview_surface (ScriviPrintPreview *preview,
			double	  *dpi_x,
			double	  *dpi_y)
{
    GtkPageSetup *page_setup;
...
GtkWidget *
scrivi_print_preview_new (GtkPrintOperation        *op,
			 GtkPrintOperationPreview *gtk_preview,
			 GtkPrintContext          *context)
{
	ScriviPrintPreview *preview;
	...
	preview = g_object_new (SCRIVI_TYPE_PRINT_PREVIEW, NULL);
