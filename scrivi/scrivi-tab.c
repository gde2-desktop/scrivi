...
#include "scrivi-io-error-message-area.h"
#include "scrivi-print-job.h"
#include "scrivi-print-preview.h"
#include "scrivi-progress-message-area.h"
#include "scrivi-debug.h"
#include "scrivi-prefs-manager-app.h"
#include "scrivi-enum-types.h"

#define SCRIVI_TAB_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_TAB, ScriviTabPrivate))

#define SCRIVI_TAB_KEY "SCRIVI_TAB_KEY"

#if GTK_CHECK_VERSION (3, 0, 0)
#define gdk_cursor_unref(cursor) g_object_unref (cursor)
#endif

struct _ScriviTabPrivate
{
	...
	gint                    tmp_line_pos;
	const ScriviEncoding    *tmp_encoding;
...
#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE(ScriviTab, scrivi_tab, GTK_TYPE_BOX)
#else
G_DEFINE_TYPE(ScriviTab, scrivi_tab, GTK_TYPE_VBOX)
#endif
...
static gboolean scrivi_tab_auto_save (ScriviTab *tab);

static void
install_auto_save_timeout (ScriviTab *tab)
{
	gint timeout;

	scrivi_debug (DEBUG_TAB);
	...
	g_return_if_fail (tab->priv->state != SCRIVI_TAB_STATE_LOADING);
	g_return_if_fail (tab->priv->state != SCRIVI_TAB_STATE_SAVING);
	g_return_if_fail (tab->priv->state != SCRIVI_TAB_STATE_REVERTING);
	g_return_if_fail (tab->priv->state != SCRIVI_TAB_STATE_LOADING_ERROR);
	g_return_if_fail (tab->priv->state != SCRIVI_TAB_STATE_SAVING_ERROR);
	g_return_if_fail (tab->priv->state != SCRIVI_TAB_STATE_SAVING_ERROR);
	g_return_if_fail (tab->priv->state != SCRIVI_TAB_STATE_REVERTING_ERROR);

	/* Add a new timeout */
	timeout = g_timeout_add_seconds (tab->priv->auto_save_interval * 60,
					 (GSourceFunc) scrivi_tab_auto_save,
					 tab);
...
static gboolean
install_auto_save_timeout_if_needed (ScriviTab *tab)
{
	ScriviDocument *doc;
	...
	g_return_val_if_fail ((tab->priv->state == SCRIVI_TAB_STATE_NORMAL) ||
			      (tab->priv->state == SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW) ||
			      (tab->priv->state == SCRIVI_TAB_STATE_CLOSING), FALSE);

	if (tab->priv->state == SCRIVI_TAB_STATE_CLOSING)
		return FALSE;
...
static void
remove_auto_save_timeout (ScriviTab *tab)
{
	scrivi_debug (DEBUG_TAB);
...
static void
scrivi_tab_get_property (GObject    *object,
		        guint       prop_id,
		        GValue     *value,
		        GParamSpec *pspec)
{
	ScriviTab *tab = SCRIVI_TAB (object);

	switch (prop_id)
	{
		case PROP_NAME:
			g_value_take_string (value,
					     _scrivi_tab_get_name (tab));
			break;
		case PROP_STATE:
			g_value_set_enum (value,
					  scrivi_tab_get_state (tab));
			break;			
		case PROP_AUTO_SAVE:
			g_value_set_boolean (value,
					     scrivi_tab_get_auto_save_enabled (tab));
			break;
		case PROP_AUTO_SAVE_INTERVAL:
			g_value_set_int (value,
					 scrivi_tab_get_auto_save_interval (tab));
			break;
		default:
...
static void
scrivi_tab_set_property (GObject      *object,
		        guint         prop_id,
		        const GValue *value,
		        GParamSpec   *pspec)
{
	...
	{
		case PROP_AUTO_SAVE:
			scrivi_tab_set_auto_save_enabled (tab,
							 g_value_get_boolean (value));
			break;
		case PROP_AUTO_SAVE_INTERVAL:
			scrivi_tab_set_auto_save_interval (tab,
...
static void
scrivi_tab_finalize (GObject *object)
{
	ScriviTab *tab = SCRIVI_TAB (object);
	...
	G_OBJECT_CLASS (scrivi_tab_parent_class)->finalize (object);
}

static void 
scrivi_tab_class_init (ScriviTabClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = scrivi_tab_finalize;
	object_class->get_property = scrivi_tab_get_property;
	object_class->set_property = scrivi_tab_set_property;
	...
	g_object_class_install_property (object_class,
					 ...
					 g_param_spec_enum ("state",
							    "State",
							    "The tab's state",
							    SCRIVI_TYPE_TAB_STATE,
							    SCRIVI_TAB_STATE_NORMAL,
							    G_PARAM_READABLE |
							    G_PARAM_STATIC_STRINGS));
	...
	g_type_class_add_private (object_class, sizeof (ScriviTabPrivate));
}

/**
 * scrivi_tab_get_state:
 * @tab: a #ScriviTab
 *
 * Gets the #ScriviTabState of @tab.
 *
 * Returns: the #ScriviTabState of @tab
 */
ScriviTabState
scrivi_tab_get_state (ScriviTab *tab)
{
	g_return_val_if_fail (SCRIVI_IS_TAB (tab), SCRIVI_TAB_STATE_NORMAL);
			       ...
			       ScriviTabState  state)
{
	...
	if ((state == SCRIVI_TAB_STATE_LOADING)          ||
	    (state == SCRIVI_TAB_STATE_REVERTING)        ||
		     ...
		     ScriviTabState  state)
{
	...
	if ((state == SCRIVI_TAB_STATE_LOADING_ERROR) || /* FIXME: add other states if needed */
...
static void 
document_uri_notify_handler (ScriviDocument *document,
			     GParamSpec    *pspec,
			     ScriviTab      *tab)
{
	scrivi_debug (DEBUG_TAB);
...
static void 
document_shortname_notify_handler (ScriviDocument *document,
				   GParamSpec    *pspec,
				   ScriviTab      *tab)
{
	scrivi_debug (DEBUG_TAB);
...
static void
set_message_area (ScriviTab  *tab,
		  GtkWidget *message_area)
{
...
static void
remove_tab (ScriviTab *tab)
{
	ScriviNotebook *notebook;

	notebook = SCRIVI_NOTEBOOK (gtk_widget_get_parent (GTK_WIDGET (tab)));

	scrivi_notebook_remove_tab (notebook, tab);
}
					...
					ScriviTab         *tab)
{
	...
	g_return_if_fail (SCRIVI_IS_VIEW (view));
	...
	switch (response_id)
	{
		case GTK_RESPONSE_OK:
			encoding = scrivi_conversion_error_message_area_get_encoding (
			...
			scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_LOADING);

			g_return_if_fail (tab->priv->auto_save_timeout <= 0);

			scrivi_document_load (doc,
					     uri,
		...
		case GTK_RESPONSE_YES:
			...
			set_message_area (tab, NULL);
			_scrivi_document_set_readonly (doc, FALSE);
			break;
		case GTK_RESPONSE_NO:
			...
			break;
		default:
			_scrivi_recent_remove (SCRIVI_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (tab))), uri);
...
load_cancelled (GtkWidget        *area,
                ...
                ScriviTab         *tab)
{
	g_return_if_fail (SCRIVI_IS_PROGRESS_MESSAGE_AREA (tab->priv->message_area));
	
	g_object_ref (tab);
	scrivi_document_load_cancel (scrivi_tab_get_document (tab));
	g_object_unref (tab);	
}

static gboolean
scroll_to_cursor (ScriviTab *tab)
{
	scrivi_view_scroll_to_cursor (SCRIVI_VIEW (tab->priv->view));
	tab->priv->idle_scroll = 0;
	return FALSE;
}
...
static void
show_loading_message_area (ScriviTab *tab)
{
	GtkWidget *area;
	...
	if (len > MAX_MSG_LENGTH)
	{
		...
		str = scrivi_utils_str_middle_truncate (name, MAX_MSG_LENGTH);
		g_free (name);
		name = str;
	}
	else
	...
	if (tab->priv->state == SCRIVI_TAB_STATE_REVERTING)
	{
		if (dirname != NULL)
		{
		...
		area = scrivi_progress_message_area_new (GTK_STOCK_REVERT_TO_SAVED,
							msg,
							TRUE);
	}
	else
	{
		...
		area = scrivi_progress_message_area_new (GTK_STOCK_OPEN,
							msg,
							TRUE);
	}
...
static void
show_saving_message_area (ScriviTab *tab)
{
	GtkWidget *area;
	ScriviDocument *doc = NULL;
	gchar *short_name;
	...
	short_name = scrivi_document_get_short_name_for_display (doc);
	...
	if (len > MAX_MSG_LENGTH)
	{
		from = scrivi_utils_str_middle_truncate (short_name, 
							MAX_MSG_LENGTH);
		g_free (short_name);
	}
	else
	{
		...
		to = scrivi_utils_uri_for_display (tab->priv->tmp_save_uri);

		str = scrivi_utils_str_middle_truncate (to, 
						       MAX (20, MAX_MSG_LENGTH - len));
		g_free (to);
	...
	area = scrivi_progress_message_area_new (GTK_STOCK_SAVE,
						msg,
						FALSE);
...
static void
message_area_set_progress (ScriviTab *tab,
			   goffset   size,
			   goffset   total_size)
{
	if (tab->priv->message_area == NULL)
	...
	scrivi_debug_message (DEBUG_TAB, "%" G_GUINT64_FORMAT "/%" G_GUINT64_FORMAT, size, total_size);
	...
	{
		if (size != 0)
			scrivi_progress_message_area_pulse (
					SCRIVI_PROGRESS_MESSAGE_AREA (tab->priv->message_area));	
		else
			scrivi_progress_message_area_set_fraction (
				SCRIVI_PROGRESS_MESSAGE_AREA (tab->priv->message_area),
				0);
	}
	else
...
static void
document_loading (ScriviDocument *document,
		  goffset        size,
		  goffset        total_size,
		  ScriviTab      *tab)
{
	...
	g_return_if_fail ((tab->priv->state == SCRIVI_TAB_STATE_LOADING) ||
		 	  (tab->priv->state == SCRIVI_TAB_STATE_REVERTING));
...
static void
document_loaded (ScriviDocument *document,
		 const GError  *error,
		 ScriviTab      *tab)
{
	GtkWidget *emsg;
	...
	location = scrivi_document_get_location (document);
	uri = scrivi_document_get_uri (document);

	/* if the error is CONVERSION FALLBACK don't treat it as a normal error */
	if (error != NULL &&
	    (error->domain != SCRIVI_DOCUMENT_ERROR || error->code != SCRIVI_DOCUMENT_ERROR_CONVERSION_FALLBACK))
	{
		if (tab->priv->state == SCRIVI_TAB_STATE_LOADING)
			scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_LOADING_ERROR);
		else
			scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_REVERTING_ERROR);

		encoding = scrivi_document_get_encoding (document);
		...
		else
		{
			...
			if (tab->priv->state == SCRIVI_TAB_STATE_LOADING_ERROR)
			{
				emsg = scrivi_io_loading_error_message_area_new (uri,
										tab->priv->tmp_encoding,
										error);
			...
			else
			{
				...
				emsg = scrivi_unrecoverable_reverting_error_message_area_new (uri,
											     error);
	...
	else
	{
		...
		mime = scrivi_document_get_mime_type (document);
		_scrivi_recent_add (SCRIVI_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (tab))),
				   uri,
				   mime);
		g_free (mime);

		if (error &&
		    error->domain == SCRIVI_DOCUMENT_ERROR &&
		    error->code == SCRIVI_DOCUMENT_ERROR_CONVERSION_FALLBACK)
		{
			GtkWidget *emsg;

			_scrivi_document_set_readonly (document, TRUE);
		...
		all_documents = scrivi_app_get_documents (scrivi_app_get_default ());

		for (l = all_documents; l != NULL; l = g_list_next (l))
		{
			ScriviDocument *d = SCRIVI_DOCUMENT (l->data);
			
			if (d != document)
			{
				GFile *loc;

				loc = scrivi_document_get_location (d);
				...
				if ((loc != NULL) &&
			    	    g_file_equal (location, loc))
			    	{
			    		...
			    		w = scrivi_file_already_open_warning_message_area_new (uri);
		...
		scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_NORMAL);
...
static void
document_saving (ScriviDocument    *document,
		 goffset  size,
		 goffset  total_size,
		 ScriviTab         *tab)
{
						  ...
						  ScriviTab         *tab)
{
	...
	if (tab->priv->print_preview != NULL)
		scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW);
	else
		scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_NORMAL);
				       ...
				       ScriviTab         *tab)
{
	if (response_id == GTK_RESPONSE_YES)
	{
		...
		scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_SAVING);

		/* don't bug the user again with this... */
		tab->priv->save_flags |= SCRIVI_DOCUMENT_SAVE_IGNORE_BACKUP;

		g_return_if_fail (tab->priv->auto_save_timeout <= 0);
		
		/* Force saving */
		scrivi_document_save (doc, tab->priv->save_flags);
	}
	else
	{
						 ...
						 ScriviTab         *tab)
{
	if (response_id == GTK_RESPONSE_YES)
	{
		...
		/* Force saving */
		scrivi_document_save (doc, tab->priv->save_flags | SCRIVI_DOCUMENT_SAVE_IGNORE_MTIME);
	}
	else
	{		
		unrecoverable_saving_error_message_area_response (message_area,
						...
						ScriviTab         *tab)
{
	...
	if (response_id == GTK_RESPONSE_OK)
	{
		...
		scrivi_debug_message (DEBUG_TAB, "Force saving with URI '%s'", tab->priv->tmp_save_uri);
			 
		g_return_if_fail (tab->priv->auto_save_timeout <= 0);
		
		scrivi_document_save_as (doc,
...
static void
document_saved (ScriviDocument *document,
		const GError  *error,
		ScriviTab      *tab)
{
	GtkWidget *emsg;
	...
	if (error != NULL)
	{
		scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_SAVING_ERROR);
		
		if (error->domain == SCRIVI_DOCUMENT_ERROR &&
		    error->code == SCRIVI_DOCUMENT_ERROR_EXTERNALLY_MODIFIED)
		{
			/* This error is recoverable */
			emsg = scrivi_externally_modified_saving_error_message_area_new (
							tab->priv->tmp_save_uri, 
							error);
		...
		else if ((error->domain == SCRIVI_DOCUMENT_ERROR &&
			  error->code == SCRIVI_DOCUMENT_ERROR_CANT_CREATE_BACKUP) ||
		         (error->domain == G_IO_ERROR &&
			  error->code == G_IO_ERROR_CANT_CREATE_BACKUP))
		{
			/* This error is recoverable */
			emsg = scrivi_no_backup_saving_error_message_area_new (
							tab->priv->tmp_save_uri, 
							error);
			g_return_if_fail (emsg != NULL);
			  ...
			  error->code != G_IO_ERROR_PARTIAL_INPUT))
		{
			...
			_scrivi_recent_remove  (SCRIVI_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (tab))),
					       tab->priv->tmp_save_uri);

			emsg = scrivi_unrecoverable_saving_error_message_area_new (tab->priv->tmp_save_uri, 
								  error);
			g_return_if_fail (emsg != NULL);
		...
		else
		{
			...
			emsg = scrivi_conversion_error_while_saving_message_area_new (
									tab->priv->tmp_save_uri,
									tab->priv->tmp_encoding,
									error);
	...
	{
		gchar *mime = scrivi_document_get_mime_type (document);
...
static void
display_externally_modified_notification (ScriviTab *tab)
{
	GtkWidget *message_area;
	ScriviDocument *doc;
	gchar *uri;
	...
	message_area = scrivi_externally_modified_message_area_new (uri, document_modified);
	g_free (uri);
                 ...
                 ScriviTab      *tab)
{
	...
	g_return_val_if_fail (SCRIVI_IS_TAB (tab), FALSE);

	/* we try to detect file changes only in the normal state */
	if (tab->priv->state != SCRIVI_TAB_STATE_NORMAL)
	{
		return FALSE;
	}
	...
	if (!scrivi_document_is_local (doc))
	{
		return FALSE;
	}

	if (_scrivi_document_check_externally_modified (doc))
	{
		scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION);
...
tab_mount_operation_factory (ScriviDocument *doc,
			     gpointer userdata)
{
	ScriviTab *tab = SCRIVI_TAB (userdata);
	GtkWidget *window;
...
static void
scrivi_tab_init (ScriviTab *tab)
{
	GtkWidget *sw;
	ScriviDocument *doc;
	ScriviLockdownMask lockdown;

	tab->priv = SCRIVI_TAB_GET_PRIVATE (tab);

	tab->priv->state = SCRIVI_TAB_STATE_NORMAL;
	...
	tab->priv->auto_save = scrivi_prefs_manager_get_auto_save () &&
			       !(lockdown & SCRIVI_LOCKDOWN_SAVE_TO_DISK);
	tab->priv->auto_save = (tab->priv->auto_save != FALSE);

	tab->priv->auto_save_interval = scrivi_prefs_manager_get_auto_save_interval ();
	if (tab->priv->auto_save_interval <= 0)
		tab->priv->auto_save_interval = GPM_DEFAULT_AUTO_SAVE_INTERVAL;

	/* Create the view */
	doc = scrivi_document_new ();
	g_object_set_data (G_OBJECT (doc), SCRIVI_TAB_KEY, tab);

	_scrivi_document_set_mount_operation_factory (doc,
						     tab_mount_operation_factory,
						     tab);

	tab->priv->view = scrivi_view_new (doc);
	g_object_unref (doc);
	gtk_widget_show (tab->priv->view);
	g_object_set_data (G_OBJECT (tab->priv->view), SCRIVI_TAB_KEY, tab);
...
GtkWidget *
_scrivi_tab_new (void)
{
	return GTK_WIDGET (g_object_new (SCRIVI_TYPE_TAB, NULL));
}

/* Whether create is TRUE, creates a new empty document if location does 
   not refer to an existing file */
GtkWidget *
_scrivi_tab_new_from_uri (const gchar         *uri,
			 const ScriviEncoding *encoding,
			 gint                 line_pos,			
			 gboolean             create)
{
 ...
 * @tab: a #ScriviTab
 *
 * Gets the #ScriviView inside @tab.
 *
 * Returns: the #ScriviView inside @tab
 */
ScriviView *
scrivi_tab_get_view (ScriviTab *tab)
{
	return SCRIVI_VIEW (tab->priv->view);
}

/**
 * scrivi_tab_get_document:
 * @tab: a #ScriviTab
 *
 * Gets the #ScriviDocument associated to @tab.
 *
 * Returns: the #ScriviDocument associated to @tab
 */
ScriviDocument *
scrivi_tab_get_document (ScriviTab *tab)
{
	return SCRIVI_DOCUMENT (gtk_text_view_get_buffer (
...
gchar *
_scrivi_tab_get_name (ScriviTab *tab)
{
	ScriviDocument *doc;
	gchar *name;
	gchar *docname;
	...
	docname = scrivi_utils_str_middle_truncate (name, MAX_DOC_NAME_LENGTH);
...
gchar *
_scrivi_tab_get_tooltips	(ScriviTab *tab)
{
	ScriviDocument *doc;
	gchar *tip;
	gchar *uri;
	...
	switch (tab->priv->state)
	{
		...
		case SCRIVI_TAB_STATE_SAVING_ERROR:
			...
			break;			
		default:
			content_type = scrivi_document_get_content_type (doc);
			mime_type = scrivi_document_get_mime_type (doc);
			...
			enc = scrivi_document_get_encoding (doc);

			if (enc == NULL)
				encoding = g_strdup (_("Unicode (UTF-8)"));
			else
				encoding = scrivi_encoding_to_string (enc);
...
GdkPixbuf *
_scrivi_tab_get_icon (ScriviTab *tab)
{
	GdkPixbuf *pixbuf;
	GtkIconTheme *theme;
	GdkScreen *screen;
	...
	switch (tab->priv->state)
	{
		...
		case SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW:		
			pixbuf = get_stock_icon (theme, 
		...
		case SCRIVI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION:
			pixbuf = get_stock_icon (theme,
		...
		default:
		{
			...
			location = scrivi_document_get_location (doc);
...
/**
 * scrivi_tab_get_from_document:
 * @doc: a #ScriviDocument
 *
 * Gets the #ScriviTab associated with @doc.
 *
 * Returns: the #ScriviTab associated with @doc
 */
ScriviTab *
scrivi_tab_get_from_document (ScriviDocument *doc)
{
	gpointer res;
	
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), NULL);
	
	res = g_object_get_data (G_OBJECT (doc), SCRIVI_TAB_KEY);
	
	return (res != NULL) ? SCRIVI_TAB (res) : NULL;
}

void
_scrivi_tab_load (ScriviTab            *tab,
		 const gchar         *uri,
		 const ScriviEncoding *encoding,
		 gint                 line_pos,
		 gboolean             create)
{
	...
	g_return_if_fail (SCRIVI_IS_TAB (tab));
	g_return_if_fail (tab->priv->state == SCRIVI_TAB_STATE_NORMAL);
...
void
_scrivi_tab_revert (ScriviTab *tab)
{
	ScriviDocument *doc;
	gchar *uri;
	...
	g_return_if_fail (SCRIVI_IS_TAB (tab));
	g_return_if_fail ((tab->priv->state == SCRIVI_TAB_STATE_NORMAL) ||
			  (tab->priv->state == SCRIVI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION));

	if (tab->priv->state == SCRIVI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION)
	{
		set_message_area (tab, NULL);
	}
	...
	scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_REVERTING);

	uri = scrivi_document_get_uri (doc);
	g_return_if_fail (uri != NULL);

	tab->priv->tmp_line_pos = 0;
	tab->priv->tmp_encoding = scrivi_document_get_encoding (doc);
...
void
_scrivi_tab_save (ScriviTab *tab)
{
	ScriviDocument *doc;
	ScriviDocumentSaveFlags save_flags;
	...
	if (tab->priv->state == SCRIVI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION)
	{
		...
		set_message_area (tab, NULL);
		save_flags = tab->priv->save_flags | SCRIVI_DOCUMENT_SAVE_IGNORE_MTIME;
	}
	else
	{
	...
	tab->priv->tmp_save_uri = scrivi_document_get_uri (doc);
	tab->priv->tmp_encoding = scrivi_document_get_encoding (doc); 

	if (tab->priv->auto_save_timeout > 0)
		remove_auto_save_timeout (tab);
		
	scrivi_document_save (doc, save_flags);
}

static gboolean
scrivi_tab_auto_save (ScriviTab *tab)
{
	ScriviDocument *doc;
	...
	g_return_val_if_fail (!scrivi_document_is_untitled (doc), FALSE);
	g_return_val_if_fail (!scrivi_document_get_readonly (doc), FALSE);
	...
	if (!gtk_text_buffer_get_modified (GTK_TEXT_BUFFER(doc)))
	{
		scrivi_debug_message (DEBUG_TAB, "Document not modified");
	    ...
	    (tab->priv->state != SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW))
	{
		...
		scrivi_debug_message (DEBUG_TAB, "Retry after 30 seconds");
	...
	scrivi_document_save (doc, tab->priv->save_flags | SCRIVI_DOCUMENT_SAVE_PRESERVE_BACKUP);
	
	scrivi_debug_message (DEBUG_TAB, "Done");
...
void
_scrivi_tab_save_as (ScriviTab                 *tab,
                    const gchar              *uri,
                    const ScriviEncoding      *encoding,
                    ScriviDocumentNewlineType  newline_type)
{
	...
	scrivi_document_set_newline_type (doc, newline_type);
	scrivi_document_save_as (doc, uri, encoding, tab->priv->save_flags);
}

#define SCRIVI_PAGE_SETUP_KEY "scrivi-page-setup-key"
#define SCRIVI_PRINT_SETTINGS_KEY "scrivi-print-settings-key"

static GtkPageSetup *
get_page_setup (ScriviTab *tab)
{
	gpointer data;
	ScriviDocument *doc;
	...
	data = g_object_get_data (G_OBJECT (doc),
				  SCRIVI_PAGE_SETUP_KEY);

	if (data == NULL)
	{
		return _scrivi_app_get_default_page_setup (scrivi_app_get_default());
	}
	else
	{
...
get_print_settings (ScriviTab *tab)
{
	gpointer data;
	ScriviDocument *doc;
	...
	data = g_object_get_data (G_OBJECT (doc),
				  SCRIVI_PRINT_SETTINGS_KEY);

	if (data == NULL)
	{
		settings = _scrivi_app_get_default_print_settings (scrivi_app_get_default());
	}
	else
	{
...
static void
printing_cb (ScriviPrintJob       *job,
	     ...
	     ScriviTab            *tab)
{
	...
	scrivi_progress_message_area_set_text (SCRIVI_PROGRESS_MESSAGE_AREA (tab->priv->message_area),
					      scrivi_print_job_get_status_string (job));

	scrivi_progress_message_area_set_fraction (SCRIVI_PROGRESS_MESSAGE_AREA (tab->priv->message_area),
						  scrivi_print_job_get_progress (job));
}

static void
store_print_settings (ScriviTab      *tab,
		      ScriviPrintJob *job)
{
	ScriviDocument *doc;
	GtkPrintSettings *settings;
	...
	settings = scrivi_print_job_get_print_settings (job);
	...
	_scrivi_app_set_default_print_settings (scrivi_app_get_default (),
					       settings);

	page_setup = scrivi_print_job_get_page_setup (job);
	...
	/* make it the default */
	_scrivi_app_set_default_page_setup (scrivi_app_get_default (),
					   page_setup);
}

static void
done_printing_cb (ScriviPrintJob       *job,
		  ...
		  ScriviTab            *tab)
{
	...
	g_return_if_fail (tab->priv->state == SCRIVI_TAB_STATE_PRINT_PREVIEWING ||
			  tab->priv->state == SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW ||
			  tab->priv->state == SCRIVI_TAB_STATE_PRINTING);

	if (tab->priv->state == SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW)
	{
	...
	if (result ==  SCRIVI_PRINT_JOB_RESULT_OK)
	{
	...
	if (tab->priv->print_preview != NULL)
	{
		...
		g_return_if_fail (tab->priv->state == SCRIVI_TAB_STATE_PRINTING);
	}
#endif
...
static void
show_preview_cb (ScriviPrintJob       *job,
		 ScriviPrintPreview   *preview,
		 ScriviTab            *tab)
{
//	g_return_if_fail (tab->priv->state == SCRIVI_TAB_STATE_PRINT_PREVIEWING);
...
static void
set_print_preview (ScriviTab  *tab,
		   GtkWidget *print_preview)
{
...
static void
preview_finished_cb (GtkSourcePrintJob *pjob, ScriviTab *tab)
{
	Gde2PrintJob *gjob;
	...
	preview = scrivi_print_job_preview_new (gjob);	
 	g_object_unref (gjob);
                 ...
                 ScriviTab         *tab)
{
	...
	scrivi_print_job_cancel (tab->priv->print_job);

	g_debug ("print_cancelled");
}

static void
show_printing_message_area (ScriviTab *tab, gboolean preview)
{
	GtkWidget *area;

	if (preview)
		area = scrivi_progress_message_area_new (GTK_STOCK_PRINT_PREVIEW,
							"",
							TRUE);
	else
		area = scrivi_progress_message_area_new (GTK_STOCK_PRINT,
							"",
							TRUE);
...
static void
scrivi_tab_print_or_print_preview (ScriviTab                *tab,
				  GtkPrintOperationAction  print_action)
{
	...
	tab->priv->print_job = scrivi_print_job_new (view);
	...
	if (is_preview)
		scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_PRINT_PREVIEWING);
	else
		scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_PRINTING);

	setup = get_page_setup (tab);
	settings = get_print_settings (tab);

	res = scrivi_print_job_print (tab->priv->print_job,
				     print_action,
				     setup,
				     settings,
...
void 
_scrivi_tab_print (ScriviTab     *tab)
{
	g_return_if_fail (SCRIVI_IS_TAB (tab));
	...
	scrivi_tab_print_or_print_preview (tab,
					  GTK_PRINT_OPERATION_ACTION_PRINT_DIALOG);
}

void
_scrivi_tab_print_preview (ScriviTab     *tab)
{
	g_return_if_fail (SCRIVI_IS_TAB (tab));
...
void 
_scrivi_tab_mark_for_closing (ScriviTab *tab)
{
	g_return_if_fail (SCRIVI_IS_TAB (tab));
	g_return_if_fail (tab->priv->state == SCRIVI_TAB_STATE_NORMAL);
	
	scrivi_tab_set_state (tab, SCRIVI_TAB_STATE_CLOSING);
}

gboolean
_scrivi_tab_can_close (ScriviTab *tab)
{
	ScriviDocument *doc;
	ScriviTabState  ts;
	...
	if ((ts == SCRIVI_TAB_STATE_LOADING)       ||
	    (ts == SCRIVI_TAB_STATE_LOADING_ERROR) ||
	    (ts == SCRIVI_TAB_STATE_REVERTING)     ||
	    (ts == SCRIVI_TAB_STATE_REVERTING_ERROR)) /* CHECK: I'm not sure this is the right behavior for REVERTING ERROR */
		return TRUE;

	/* Do not close tab with saving errors */
	if (ts == SCRIVI_TAB_STATE_SAVING_ERROR)
		return FALSE;
	...
	return (!gtk_text_buffer_get_modified (GTK_TEXT_BUFFER (doc)) &&
		!scrivi_document_get_deleted (doc));
}

/**
 * scrivi_tab_get_auto_save_enabled:
 * @tab: a #ScriviTab
 * 
 ...
 **/
gboolean
scrivi_tab_get_auto_save_enabled	(ScriviTab *tab)
{
	scrivi_debug (DEBUG_TAB);
...
/**
 * scrivi_tab_set_auto_save_enabled:
 * @tab: a #ScriviTab
 ...
 **/
void
scrivi_tab_set_auto_save_enabled	(ScriviTab *tab, 
				 gboolean  enable)
{
	ScriviDocument *doc = NULL;
	ScriviLockdownMask lockdown;
	...
	lockdown = scrivi_app_get_lockdown (scrivi_app_get_default());
	if (lockdown & SCRIVI_LOCKDOWN_SAVE_TO_DISK)
		enable = FALSE;
	...
	tab->priv->auto_save = enable;
 	...
 	{
 		if ((tab->priv->state != SCRIVI_TAB_STATE_LOADING) &&
		    (tab->priv->state != SCRIVI_TAB_STATE_SAVING) &&
		    (tab->priv->state != SCRIVI_TAB_STATE_REVERTING) &&
		    (tab->priv->state != SCRIVI_TAB_STATE_LOADING_ERROR) &&
		    (tab->priv->state != SCRIVI_TAB_STATE_SAVING_ERROR) &&
		    (tab->priv->state != SCRIVI_TAB_STATE_REVERTING_ERROR))
		{
			install_auto_save_timeout (tab);
		}
 	...
 	g_return_if_fail ((!enable && (tab->priv->auto_save_timeout <= 0)) ||  
 			  scrivi_document_is_untitled (doc) || scrivi_document_get_readonly (doc)); 
}

/**
 * scrivi_tab_get_auto_save_interval:
 * @tab: a #ScriviTab
 * 
 ...
 **/
gint 
scrivi_tab_get_auto_save_interval (ScriviTab *tab)
{
	scrivi_debug (DEBUG_TAB);

	g_return_val_if_fail (SCRIVI_IS_TAB (tab), 0);

	return tab->priv->auto_save_interval;
}

/**
 * scrivi_tab_set_auto_save_interval:
 * @tab: a #ScriviTab
 ...
 **/
void 
scrivi_tab_set_auto_save_interval (ScriviTab *tab, 
				  gint      interval)
{
	ScriviDocument *doc = NULL;
	...
	{
		g_return_if_fail (!scrivi_document_is_untitled (doc));
		g_return_if_fail (!scrivi_document_get_readonly (doc));
...
void
scrivi_tab_set_info_bar (ScriviTab  *tab,
                        GtkWidget *info_bar)
{
	g_return_if_fail (SCRIVI_IS_TAB (tab));
