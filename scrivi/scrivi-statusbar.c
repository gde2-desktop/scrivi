...
#define SCRIVI_STATUSBAR_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object),\
					    SCRIVI_TYPE_STATUSBAR,\
					    ScriviStatusbarPrivate))

#if GTK_CHECK_VERSION (3, 0, 0)
#define gtk_hbox_new(X,Y) gtk_box_new(GTK_ORIENTATION_HORIZONTAL,Y)
#endif

struct _ScriviStatusbarPrivate
{
...
G_DEFINE_TYPE(ScriviStatusbar, scrivi_statusbar, GTK_TYPE_STATUSBAR)
...
static void
scrivi_statusbar_dispose (GObject *object)
{
	ScriviStatusbar *statusbar = SCRIVI_STATUSBAR (object);
	...
	G_OBJECT_CLASS (scrivi_statusbar_parent_class)->dispose (object);
}

static void
scrivi_statusbar_class_init (ScriviStatusbarClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->dispose = scrivi_statusbar_dispose;

	g_type_class_add_private (object_class, sizeof (ScriviStatusbarPrivate));
}

#define CURSOR_POSITION_LABEL_WIDTH_CHARS 18

static void
scrivi_statusbar_init (ScriviStatusbar *statusbar)
{
	GtkWidget *hbox;
	GtkWidget *error_image;

	statusbar->priv = SCRIVI_STATUSBAR_GET_PRIVATE (statusbar);
...
/**
 * scrivi_statusbar_new:
 *
 * Creates a new #ScriviStatusbar.
 *
 * Return value: the new #ScriviStatusbar object
 **/
GtkWidget *
scrivi_statusbar_new (void)
{
	return GTK_WIDGET (g_object_new (SCRIVI_TYPE_STATUSBAR, NULL));
}

/**
 * scrivi_statusbar_set_overwrite:
 * @statusbar: a #ScriviStatusbar
 * @overwrite: if the overwrite mode is set
 *
 * Sets the overwrite mode on the statusbar.
 **/
void
scrivi_statusbar_set_overwrite (ScriviStatusbar *statusbar,
                               gboolean        overwrite)
{
	gchar *msg;

	g_return_if_fail (SCRIVI_IS_STATUSBAR (statusbar));
...
void
scrivi_statusbar_clear_overwrite (ScriviStatusbar *statusbar)
{
	g_return_if_fail (SCRIVI_IS_STATUSBAR (statusbar));
...
/**
 * scrivi_statusbar_cursor_position:
 * @statusbar: an #ScriviStatusbar
 * @line: line position
 * @col: column position
 *
 ...
 **/
void
scrivi_statusbar_set_cursor_position (ScriviStatusbar *statusbar,
				     gint            line,
				     gint            col)
{
	gchar *msg = NULL;
...
static gboolean
remove_message_timeout (ScriviStatusbar *statusbar)
{
...
/**
 * scrivi_statusbar_flash_message:
 * @statusbar: a #ScriviStatusbar
 ...
 */
void
scrivi_statusbar_flash_message (ScriviStatusbar *statusbar,
			       guint           context_id,
			       const gchar    *format, ...)
{
...
void
scrivi_statusbar_set_window_state (ScriviStatusbar   *statusbar,
				  ...
				  gint              num_of_errors)
{
	...
	if (state & SCRIVI_WINDOW_STATE_LOADING)
	{
		gtk_widget_show (statusbar->priv->state_frame);
		gtk_widget_show (statusbar->priv->load_image);
	}

	if (state & SCRIVI_WINDOW_STATE_PRINTING)
	{
		gtk_widget_show (statusbar->priv->state_frame);
		gtk_widget_show (statusbar->priv->print_image);
	}

	if (state & SCRIVI_WINDOW_STATE_ERROR)
	{
	 	gchar *tip;
