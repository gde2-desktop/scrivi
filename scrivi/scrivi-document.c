...
#ifdef MAXPATHLEN
#define SCRIVI_MAX_PATH_LEN  MAXPATHLEN
#elif defined (PATH_MAX)
#define SCRIVI_MAX_PATH_LEN  PATH_MAX
#else
#define SCRIVI_MAX_PATH_LEN  2048
#endif

#define SCRIVI_DOCUMENT_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_DOCUMENT, ScriviDocumentPrivate))

static void	scrivi_document_load_real	(ScriviDocument          *doc,
						 const gchar            *uri,
						 const ScriviEncoding    *encoding,
						 gint                    line_pos,
						 gboolean                create);
static void	scrivi_document_save_real	(ScriviDocument          *doc,
						 const gchar            *uri,
						 const ScriviEncoding    *encoding,
						 ScriviDocumentSaveFlags  flags);
static void	to_search_region_range 		(ScriviDocument *doc,
						 GtkTextIter   *start, 
						 GtkTextIter   *end);
static void 	insert_text_cb		 	(ScriviDocument *doc, 
						 GtkTextIter   *pos,
						 const gchar   *text, 
						 gint           length);
						 
static void	delete_range_cb 		(ScriviDocument *doc, 
						 GtkTextIter   *start,
						 GtkTextIter   *end);
			     
struct _ScriviDocumentPrivate
{
	gchar	    *uri;
	...
	const ScriviEncoding *requested_encoding;
	...
	ScriviTextRegion *to_search_region;
	GtkTextTag      *found_tag;

	/* Mount operation factory */
	ScriviMountOperationFactory  mount_operation_factory;
...
#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE(ScriviDocument, scrivi_document, GTK_SOURCE_TYPE_BUFFER)
#else
G_DEFINE_TYPE(ScriviDocument, scrivi_document, GTK_TYPE_SOURCE_BUFFER)
#endif

GQuark
scrivi_document_error_quark (void)
{
	static GQuark quark = 0;

	if (G_UNLIKELY (quark == 0))
		quark = g_quark_from_static_string ("scrivi_io_load_error");
...
static void
scrivi_document_dispose (GObject *object)
{
	ScriviDocument *doc = SCRIVI_DOCUMENT (object); 

	scrivi_debug (DEBUG_DOCUMENT);
	...
	if ((!doc->priv->dispose_has_run) && (doc->priv->uri != NULL))
	{
		...
		if (language == NULL)
			scrivi_document_set_metadata (doc, SCRIVI_METADATA_ATTRIBUTE_POSITION,
						     position, NULL);
		else
			scrivi_document_set_metadata (doc, SCRIVI_METADATA_ATTRIBUTE_POSITION,
						     position, SCRIVI_METADATA_ATTRIBUTE_LANGUAGE,
						     language, NULL);
		g_free (position);
	}
	...
	G_OBJECT_CLASS (scrivi_document_parent_class)->dispose (object);
}

static void
scrivi_document_finalize (GObject *object)
{
	ScriviDocument *doc = SCRIVI_DOCUMENT (object); 
	...
	if (doc->priv->to_search_region != NULL)
	{
		...
		scrivi_text_region_destroy (doc->priv->to_search_region, FALSE);
	}

	G_OBJECT_CLASS (scrivi_document_parent_class)->finalize (object);
}

static void
scrivi_document_get_property (GObject    *object,
			     guint       prop_id,
			     GValue     *value,
			     GParamSpec *pspec)
{
	...
	switch (prop_id)
	{
		case PROP_URI:
			...
			break;
		case PROP_SHORTNAME:
			g_value_take_string (value, scrivi_document_get_short_name_for_display (doc));
			break;
		case PROP_CONTENT_TYPE:
			g_value_take_string (value, scrivi_document_get_content_type (doc));
			break;
		case PROP_MIME_TYPE:
			g_value_take_string (value, scrivi_document_get_mime_type (doc));
			break;
		case PROP_READ_ONLY:
		...
		case PROP_CAN_SEARCH_AGAIN:
			g_value_set_boolean (value, scrivi_document_get_can_search_again (doc));
			break;
		case PROP_ENABLE_SEARCH_HIGHLIGHTING:
			g_value_set_boolean (value, scrivi_document_get_enable_search_highlighting (doc));
			break;
...
static void
scrivi_document_set_property (GObject      *object,
			     guint         prop_id,
			     const GValue *value,
			     GParamSpec   *pspec)
{
	...
	{
		case PROP_ENABLE_SEARCH_HIGHLIGHTING:
			scrivi_document_set_enable_search_highlighting (doc,
								       g_value_get_boolean (value));
			break;
		case PROP_NEWLINE_TYPE:
			scrivi_document_set_newline_type (doc,
							 g_value_get_enum (value));
			break;
		case PROP_SHORTNAME:
			scrivi_document_set_short_name_for_display (doc,
			                                           g_value_get_string (value));
			break;
		case PROP_CONTENT_TYPE:
			scrivi_document_set_content_type (doc,
			                                 g_value_get_string (value));
			break;
		default:
			G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
...
static void
emit_cursor_moved (ScriviDocument *doc)
{
...
static void
scrivi_document_mark_set (GtkTextBuffer     *buffer,
                         const GtkTextIter *iter,
                         GtkTextMark       *mark)
{
	ScriviDocument *doc = SCRIVI_DOCUMENT (buffer);

	if (GTK_TEXT_BUFFER_CLASS (scrivi_document_parent_class)->mark_set)
		GTK_TEXT_BUFFER_CLASS (scrivi_document_parent_class)->mark_set (buffer,
									       iter,
									       mark);
...
static void
scrivi_document_changed (GtkTextBuffer *buffer)
{
	emit_cursor_moved (SCRIVI_DOCUMENT (buffer));

	GTK_TEXT_BUFFER_CLASS (scrivi_document_parent_class)->changed (buffer);
}

static void 
scrivi_document_class_init (ScriviDocumentClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkTextBufferClass *buf_class = GTK_TEXT_BUFFER_CLASS (klass);

	object_class->dispose = scrivi_document_dispose;
	object_class->finalize = scrivi_document_finalize;
	object_class->get_property = scrivi_document_get_property;
	object_class->set_property = scrivi_document_set_property;

	buf_class->mark_set = scrivi_document_mark_set;
	buf_class->changed = scrivi_document_changed;

	klass->load = scrivi_document_load_real;
	klass->save = scrivi_document_save_real;
	...
	g_object_class_install_property (object_class, PROP_ENCODING,
					 g_param_spec_boxed ("encoding",
							     "Encoding",
							     "The ScriviEncoding used for the document",
	...
	document_signals[CURSOR_MOVED] =
   		g_signal_new ("cursor-moved",
			      ...
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviDocumentClass, cursor_moved),
			      NULL, NULL,
	...
	/**
	 * ScriviDocument::load:
	 * @document: the #ScriviDocument.
	 * @uri: the uri where to load the document from.
	 * @encoding: the #ScriviEncoding to encode the document.
	...
	document_signals[LOAD] =
		g_signal_new ("load",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviDocumentClass, load),
			      NULL, NULL,
			      scrivi_marshal_VOID__STRING_BOXED_INT_BOOLEAN,
			      G_TYPE_NONE,
			      4,
			      G_TYPE_STRING,
			      /* we rely on the fact that the ScriviEncoding pointer stays
			       * the same forever */
			      SCRIVI_TYPE_ENCODING | G_SIGNAL_TYPE_STATIC_SCOPE,
			      G_TYPE_INT,
			      G_TYPE_BOOLEAN);
	...
	document_signals[LOADING] =
		g_signal_new ("loading",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviDocumentClass, loading),
			      NULL, NULL,
			      scrivi_marshal_VOID__UINT64_UINT64,
			      G_TYPE_NONE,
			      2,
			      G_TYPE_UINT64,
			      G_TYPE_UINT64);
	...
	document_signals[LOADED] =
   		g_signal_new ("loaded",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviDocumentClass, loaded),
			      NULL, NULL,
	 ...
	 * @encoding: the #ScriviEncoding used to save the document.
	 * @flags: the #ScriviDocumentSaveFlags for the save operation.
	 *
	...
	document_signals[SAVE] =
		g_signal_new ("save",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviDocumentClass, save),
			      NULL, NULL,
			      scrivi_marshal_VOID__STRING_BOXED_FLAGS,
			      G_TYPE_NONE,
			      3,
			      G_TYPE_STRING,
			      /* we rely on the fact that the ScriviEncoding pointer stays
	...
	document_signals[SAVING] =
   		g_signal_new ("saving",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviDocumentClass, saving),
			      NULL, NULL,
			      scrivi_marshal_VOID__UINT64_UINT64,
			      G_TYPE_NONE,
			      2,
	...
	document_signals[SAVED] =
   		g_signal_new ("saved",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviDocumentClass, saved),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__POINTER,
			      G_TYPE_NONE,
			      1,
	...
	document_signals[SEARCH_HIGHLIGHT_UPDATED] =
	    	g_signal_new ("search_highlight_updated",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviDocumentClass, search_highlight_updated),
			      NULL, NULL,
			      scrivi_marshal_VOID__BOXED_BOXED,
			      G_TYPE_NONE, 
			      2, 
			      GTK_TYPE_TEXT_ITER | G_SIGNAL_TYPE_STATIC_SCOPE,
			      GTK_TYPE_TEXT_ITER | G_SIGNAL_TYPE_STATIC_SCOPE);

	g_type_class_add_private (object_class, sizeof(ScriviDocumentPrivate));
}

static void
set_language (ScriviDocument     *doc, 
              GtkSourceLanguage *lang,
              gboolean           set_by_user)
{
	...
	{
		scrivi_document_set_metadata (doc, SCRIVI_METADATA_ATTRIBUTE_LANGUAGE,
...
static void
set_encoding (ScriviDocument       *doc, 
	      const ScriviEncoding *encoding,
	      gboolean             set_by_user)
{
	...
	if (set_by_user)
	{
		...
		charset = scrivi_encoding_get_charset (encoding);

		scrivi_document_set_metadata (doc, SCRIVI_METADATA_ATTRIBUTE_ENCODING,
					     charset, NULL);
	}
...
get_default_style_scheme (void)
{
	...
	manager = scrivi_get_style_scheme_manager ();
	scheme_id = scrivi_prefs_manager_get_source_style_scheme ();
...
static void
on_uri_changed (ScriviDocument *doc,
		GParamSpec    *pspec,
		gpointer       useless)
{
...
guess_language (ScriviDocument *doc,
		const gchar   *content_type)
{
	gchar *data;
	GtkSourceLanguage *language = NULL;

	data = scrivi_document_get_metadata (doc, SCRIVI_METADATA_ATTRIBUTE_LANGUAGE);

	if (data != NULL)
	{
		scrivi_debug_message (DEBUG_DOCUMENT, "Language from metadata: %s", data);
	...
	else
	{
		...
		scrivi_debug_message (DEBUG_DOCUMENT, "Sniffing Language");
...
static void
on_content_type_changed (ScriviDocument *doc,
			 GParamSpec    *pspec,
			 gpointer       useless)
{
	if (!doc->priv->language_set_by_user)
	{
		...
		scrivi_debug_message (DEBUG_DOCUMENT, "Language: %s",
...
static void
scrivi_document_init (ScriviDocument *doc)
{
	GtkSourceStyleScheme *style_scheme;

	scrivi_debug (DEBUG_DOCUMENT);

	doc->priv = SCRIVI_DOCUMENT_GET_PRIVATE (doc);
	...
	doc->priv->encoding = scrivi_encoding_get_utf8 ();

	doc->priv->newline_type = SCRIVI_DOCUMENT_NEWLINE_TYPE_DEFAULT;

	gtk_source_buffer_set_max_undo_levels (GTK_SOURCE_BUFFER (doc), 
					       scrivi_prefs_manager_get_undo_actions_limit ());

	gtk_source_buffer_set_highlight_matching_brackets (GTK_SOURCE_BUFFER (doc), 
							   scrivi_prefs_manager_get_bracket_matching ());

	scrivi_document_set_enable_search_highlighting (doc,
						       scrivi_prefs_manager_get_enable_search_highlighting ());
...
ScriviDocument *
scrivi_document_new (void)
{
	scrivi_debug (DEBUG_DOCUMENT);

	return SCRIVI_DOCUMENT (g_object_new (SCRIVI_TYPE_DOCUMENT, NULL));
}

static void
set_content_type_no_guess (ScriviDocument *doc,
			   const gchar   *content_type)
{
	scrivi_debug (DEBUG_DOCUMENT);
...
static void
set_content_type (ScriviDocument *doc,
		  const gchar   *content_type)
{
	scrivi_debug (DEBUG_DOCUMENT);
...
/**
 * scrivi_document_set_content_type:
 * @doc:
 * @content_type: (allow-none):
 */
void
scrivi_document_set_content_type (ScriviDocument *doc,
                                 const gchar   *content_type)
{
...
static void
set_uri (ScriviDocument *doc,
	 const gchar   *uri)
{
	scrivi_debug (DEBUG_DOCUMENT);

	g_return_if_fail ((uri == NULL) || scrivi_utils_is_valid_uri (uri));
...
GFile *
scrivi_document_get_location (ScriviDocument *doc)
{
...
gchar *
scrivi_document_get_uri (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), NULL);
...
void
scrivi_document_set_uri (ScriviDocument *doc,
			const gchar   *uri)
{
	g_return_if_fail (SCRIVI_IS_DOCUMENT (doc));
...
/**
 * scrivi_document_get_uri_for_display:
 * @doc:
 *
 * Note: this never returns %NULL.
 **/
gchar *
scrivi_document_get_uri_for_display (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), g_strdup (""));

	if (doc->priv->uri == NULL)
		return g_strdup_printf (_("Unsaved Document %d"),
					doc->priv->untitled_number);
	else
		return scrivi_utils_uri_for_display (doc->priv->uri);
}

/**
 * scrivi_document_get_short_name_for_display:
 * @doc:
 *
 * Note: this never returns %NULL.
 **/
gchar *
scrivi_document_get_short_name_for_display (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), g_strdup (""));
	...
	if (doc->priv->short_name != NULL)
		...
		return scrivi_utils_basename_for_display (doc->priv->uri);
}

/**
 * scrivi_document_set_short_name_for_display:
 * @doc:
 * @short_name: (allow-none):
 */
void
scrivi_document_set_short_name_for_display (ScriviDocument *doc,
...
gchar *
scrivi_document_get_content_type (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), NULL);
...
/**
 * scrivi_document_get_mime_type:
 * @doc:
 *
 * Note: this never returns %NULL.
 **/
gchar *
scrivi_document_get_mime_type (ScriviDocument *doc)
{
	gchar *mime_type = NULL;

	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), g_strdup ("text/plain"));
...
static gboolean
set_readonly (ScriviDocument *doc,
	      gboolean       readonly)
{
	scrivi_debug (DEBUG_DOCUMENT);
 ...
 */
void
_scrivi_document_set_readonly (ScriviDocument *doc,
			      gboolean       readonly)
{
	scrivi_debug (DEBUG_DOCUMENT);
...
gboolean
scrivi_document_get_readonly (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), TRUE);

	return doc->priv->readonly;
}

gboolean
_scrivi_document_check_externally_modified (ScriviDocument *doc)
{
	GFile *gfile;
	GFileInfo *info;

	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), FALSE);
	...
	if (info != NULL)
	{
		...
		if (g_file_info_has_attribute (info, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE))
		{
			...
			_scrivi_document_set_readonly (doc, read_only);
		}
...
static void
reset_temp_loading_data (ScriviDocument       *doc)
{
...
static void
document_loader_loaded (ScriviDocumentLoader *loader,
			const GError        *error,
			ScriviDocument       *doc)
{
	/* load was successful */
	     ...
	     error->code == SCRIVI_DOCUMENT_ERROR_CONVERSION_FALLBACK))
	{
		...
		info = scrivi_document_loader_get_info (loader);
		...
		set_encoding (doc, 
			      scrivi_document_loader_get_encoding (loader),
			      (doc->priv->requested_encoding != NULL));

		set_content_type (doc, content_type);

		scrivi_document_set_newline_type (doc,
		                                 scrivi_document_loader_get_newline_type (loader));
		...
		else if (scrivi_prefs_manager_get_restore_cursor_position ())
		{
			gchar *pos;
			gint offset;

			pos = scrivi_document_get_metadata (doc, SCRIVI_METADATA_ATTRIBUTE_POSITION);
	...
	/* special case creating a named new doc */
	         ...
	         (scrivi_utils_uri_has_file_scheme (doc->priv->uri)))
	{
...
static void
document_loader_loading (ScriviDocumentLoader *loader,
			 gboolean             completed,
			 const GError        *error,
			 ScriviDocument       *doc)
{
	...
	else
	{
		...
		read = scrivi_document_loader_get_bytes_read (loader);
...
static void
scrivi_document_load_real (ScriviDocument       *doc,
			  const gchar         *uri,
			  const ScriviEncoding *encoding,
			  gint                 line_pos,
			  gboolean             create)
{
	...
	scrivi_debug_message (DEBUG_DOCUMENT, "load_real: uri = %s", uri);

	/* create a loader. It will be destroyed when loading is completed */
	doc->priv->loader = scrivi_document_loader_new (doc, uri, encoding);
	...
	scrivi_document_loader_load (doc->priv->loader);
}

/**
 * scrivi_document_load:
 * @doc: the #ScriviDocument.
 * @uri: the uri where to load the document from.
 * @encoding: the #ScriviEncoding to encode the document.
 * @line_pos: the line to show.
 * @create: whether the document should be created if it doesn't exist.
 ...
 */
void
scrivi_document_load (ScriviDocument       *doc,
		     const gchar         *uri,
		     const ScriviEncoding *encoding,
		     gint                 line_pos,
		     gboolean             create)
...
/**
 * scrivi_document_load_cancel:
 * @doc: the #ScriviDocument.
 *
 * Cancel load of a document.
 */
gboolean
scrivi_document_load_cancel (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), FALSE);
	...
	return scrivi_document_loader_cancel (doc->priv->loader);
}

static void
document_saver_saving (ScriviDocumentSaver *saver,
		       gboolean            completed,
		       const GError       *error,
		       ScriviDocument      *doc)
{
	...
	if (completed)
	{
		...
		if (error == NULL)
		{
			...
			uri = scrivi_document_saver_get_uri (saver);
			set_uri (doc, uri);

			info = scrivi_document_saver_get_info (saver);
	...
	else
	{
		...
		size = scrivi_document_saver_get_file_size (saver);
		written = scrivi_document_saver_get_bytes_written (saver);

		scrivi_debug_message (DEBUG_DOCUMENT, "save progress: %" G_GINT64_FORMAT " of %" G_GINT64_FORMAT, written, size);
...
static void
scrivi_document_save_real (ScriviDocument          *doc,
			  const gchar            *uri,
			  const ScriviEncoding    *encoding,
			  ScriviDocumentSaveFlags  flags)
{
	...
	doc->priv->saver = scrivi_document_saver_new (doc, uri, encoding,
	...
	scrivi_document_saver_save (doc->priv->saver,
				   &doc->priv->mtime);
}

/**
 * scrivi_document_save:
 * @doc: the #ScriviDocument.
 * @flags: optionnal #ScriviDocumentSaveFlags.
 *
 * Save the document to its previous location. This results in the "save"
 * signal to be emitted.
 */
void
scrivi_document_save (ScriviDocument          *doc,
		     ScriviDocumentSaveFlags  flags)
{
	g_return_if_fail (SCRIVI_IS_DOCUMENT (doc));
	g_return_if_fail (doc->priv->uri != NULL);
 ...
 */
void
scrivi_document_save_as (ScriviDocument          *doc,
			const gchar            *uri,
			const ScriviEncoding    *encoding,
			ScriviDocumentSaveFlags  flags)
{
	...
	g_signal_emit (doc,
		       ...
		       uri,
		       encoding,
		       flags | SCRIVI_DOCUMENT_SAVE_IGNORE_MTIME);
}

gboolean
scrivi_document_insert_file (ScriviDocument       *doc,
			    GtkTextIter         *iter,
			    const gchar         *uri,
			    const ScriviEncoding *encoding)
{
...
gboolean	 
scrivi_document_is_untouched (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), TRUE);
...
gboolean 
scrivi_document_is_untitled (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), TRUE);
...
gboolean
scrivi_document_is_local (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), FALSE);
	...
	return scrivi_utils_uri_has_file_scheme (doc->priv->uri);
}

gboolean
scrivi_document_get_deleted (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), FALSE);

	return doc->priv->uri && !scrivi_utils_uri_exists (doc->priv->uri);
}
 ...
 */
gboolean
scrivi_document_goto_line (ScriviDocument *doc, 
			  gint           line)
{
	gboolean ret = TRUE;
	guint line_count;
...
gboolean
scrivi_document_goto_line_offset (ScriviDocument *doc,
				 gint           line,
				 gint           line_offset)
{
	gboolean ret = TRUE;
...
/**
 * scrivi_document_set_search_text"
 * @doc:
 * @text: (allow-none):
 * @flags:
 **/
void
scrivi_document_set_search_text (ScriviDocument *doc,
				const gchar   *text,
				guint          flags)
{
	...
	scrivi_debug_message (DEBUG_DOCUMENT, "text = %s", text);

	if (text != NULL)
	{
		if (*text != '\0')
		{
			converted_text = scrivi_utils_unescape_search_text (text);
			notify = !scrivi_document_get_can_search_again (doc);
		}
		else
		{
	...
	if (!SCRIVI_SEARCH_IS_DONT_SET_FLAGS (flags))
	{
...
/**
 * scrivi_document_get_search_text:
 * @doc:
 * @flags: (allow-none):
 */
gchar *
scrivi_document_get_search_text (ScriviDocument *doc,
				guint         *flags)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), NULL);
	...
	return scrivi_utils_escape_search_text (doc->priv->search_text);
}

gboolean
scrivi_document_get_can_search_again (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), FALSE);
...
/**
 * scrivi_document_search_forward:
 * @doc:
 * @start: (allow-none):
 * @end: (allow-none):
 ...
 **/
gboolean
scrivi_document_search_forward (ScriviDocument     *doc,
			       ...
			       GtkTextIter       *match_end)
{
	...
	{
		scrivi_debug_message (DEBUG_DOCUMENT, "doc->priv->search_text == NULL\n");
		return FALSE;
	}
	else
		scrivi_debug_message (DEBUG_DOCUMENT, "doc->priv->search_text == \"%s\"\n", doc->priv->search_text);
	...
	if (!SCRIVI_SEARCH_IS_CASE_SENSITIVE (doc->priv->search_flags))
	{
#if GTK_CHECK_VERSION (3, 0, 0)
	...
	while (!found)
	{
		...
		if (found && SCRIVI_SEARCH_IS_ENTIRE_WORD (doc->priv->search_flags))
		{
...
/**
 * scrivi_document_search_backward:
 * @doc:
 * @start: (allow-none):
 * @end: (allow-none):
 * @match_start: (allow-none):
 ...
 **/
gboolean
scrivi_document_search_backward (ScriviDocument     *doc,
				const GtkTextIter *start,
				const GtkTextIter *end,
				GtkTextIter       *match_start,
				GtkTextIter       *match_end)
...
gint 
scrivi_document_replace_all (ScriviDocument       *doc,
			    const gchar         *find, 
			    const gchar         *replace, 
			    guint                flags)
{
	...
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), 0);
	...
	if (find == NULL)
		...
		search_text = scrivi_utils_unescape_search_text (find);

	replace_text = scrivi_utils_unescape_search_text (replace);
	...
	if (!SCRIVI_SEARCH_IS_CASE_SENSITIVE (flags))
	{
#if GTK_CHECK_VERSION (3, 0, 0)
		search_flags = search_flags | GTK_TEXT_SEARCH_CASE_INSENSITIVE;
#else
	...
	search_highliting = scrivi_document_get_enable_search_highlighting (doc);
	scrivi_document_set_enable_search_highlighting (doc, FALSE);
	...
	do
	{
		...
		if (found && SCRIVI_SEARCH_IS_ENTIRE_WORD (flags))
		{
			gboolean word;
	...
	scrivi_document_set_enable_search_highlighting (doc, search_highliting);
...
/**
 * scrivi_document_set_language:
 * @doc:
 * @lang: (allow-none):
 **/
void
scrivi_document_set_language (ScriviDocument     *doc, 
			     GtkSourceLanguage *lang)
{
	g_return_if_fail (SCRIVI_IS_DOCUMENT (doc));
...
glong
_scrivi_document_get_seconds_since_last_save_or_load (ScriviDocument *doc)
{
	GTimeVal current_time;

	scrivi_debug (DEBUG_DOCUMENT);
	
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), -1);
...
static void
get_search_match_colors (ScriviDocument *doc,
...
#endif
{
 ...
 fallback:
	scrivi_debug_message (DEBUG_DOCUMENT, 
...
static void
sync_found_tag (ScriviDocument *doc,
		GParamSpec    *pspec,
		gpointer       data)
{
#if GTK_CHECK_VERSION (3, 14, 0)
...
static void
search_region (ScriviDocument *doc,
	       GtkTextIter   *start,
	       GtkTextIter   *end)
{
	GtkTextIter iter;
...
static void
to_search_region_range (ScriviDocument *doc,
			GtkTextIter   *start, 
			GtkTextIter   *end)
{
	scrivi_debug (DEBUG_DOCUMENT);
	...
	*/
	...
	scrivi_text_region_add (doc->priv->to_search_region, start, end);
...
void
_scrivi_document_search_region (ScriviDocument     *doc,
			       const GtkTextIter *start,
			       const GtkTextIter *end)
{
	ScriviTextRegion *region;
	...
	*/
	...
	region = scrivi_text_region_intersect (doc->priv->to_search_region, 
					      start,
					      end);
	if (region) 
	{
		...
		i = scrivi_text_region_subregions (region);
		scrivi_text_region_nth_subregion (region, 
						 0,
						 &start_search,
						 NULL);
		...
		scrivi_text_region_destroy (region, TRUE);
		...
		scrivi_text_region_subtract (doc->priv->to_search_region,
					    start, 
					    end);
	}
}
...
static void
insert_text_cb (ScriviDocument *doc, 
		GtkTextIter   *pos,
		const gchar   *text, 
		gint           length)
{
...
static void	
delete_range_cb (ScriviDocument *doc, 
		 GtkTextIter   *start,
		 GtkTextIter   *end)
{
	GtkTextIter d_start;
...
void
scrivi_document_set_enable_search_highlighting (ScriviDocument *doc,
					       gboolean       enable)
{
	g_return_if_fail (SCRIVI_IS_DOCUMENT (doc));
	...
	if (doc->priv->to_search_region != NULL)
	{
		...
		scrivi_text_region_destroy (doc->priv->to_search_region,
					   TRUE);
		doc->priv->to_search_region = NULL;
	}
	else
	{
		doc->priv->to_search_region = scrivi_text_region_new (GTK_TEXT_BUFFER (doc));
		if (scrivi_document_get_can_search_again (doc))
		{
...
gboolean
scrivi_document_get_enable_search_highlighting (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), FALSE);
...
void
scrivi_document_set_newline_type (ScriviDocument           *doc,
				 ScriviDocumentNewlineType newline_type)
{
	g_return_if_fail (SCRIVI_IS_DOCUMENT (doc));
...
void
_scrivi_document_set_mount_operation_factory (ScriviDocument 	       *doc,
...
GMountOperation *
_scrivi_document_create_mount_operation (ScriviDocument *doc)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), NULL);
...
#ifndef ENABLE_GVFS_METADATA
gchar *
scrivi_document_get_metadata (ScriviDocument *doc,
			     const gchar   *key)
{
	gchar *value = NULL;
	...
	{
		value = scrivi_metadata_manager_get (doc->priv->uri, key);
	}
...
void
scrivi_document_set_metadata (ScriviDocument *doc,
			     ...
			     ...)
{
	...
	for (key = first_key; key; key = va_arg (var_args, const gchar *))
	{
		...
		scrivi_metadata_manager_set (doc->priv->uri,
					    key,
					    value);
	}
