...
#define SCRIVI_SMART_CHARSET_CONVERTER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE((object), SCRIVI_TYPE_SMART_CHARSET_CONVERTER, ScriviSmartCharsetConverterPrivate))

struct _ScriviSmartCharsetConverterPrivate
{
...
static void scrivi_smart_charset_converter_iface_init    (GConverterIface *iface);

G_DEFINE_TYPE_WITH_CODE (ScriviSmartCharsetConverter, scrivi_smart_charset_converter,
			 G_TYPE_OBJECT,
			 G_IMPLEMENT_INTERFACE (G_TYPE_CONVERTER,
						scrivi_smart_charset_converter_iface_init))

static void
scrivi_smart_charset_converter_finalize (GObject *object)
{
	ScriviSmartCharsetConverter *smart = SCRIVI_SMART_CHARSET_CONVERTER (object);

	g_slist_free (smart->priv->encodings);

	scrivi_debug_message (DEBUG_UTILS, "finalizing smart charset converter");

	G_OBJECT_CLASS (scrivi_smart_charset_converter_parent_class)->finalize (object);
}

static void
scrivi_smart_charset_converter_dispose (GObject *object)
{
	ScriviSmartCharsetConverter *smart = SCRIVI_SMART_CHARSET_CONVERTER (object);
	...
	scrivi_debug_message (DEBUG_UTILS, "disposing smart charset converter");

	G_OBJECT_CLASS (scrivi_smart_charset_converter_parent_class)->dispose (object);
}

static void
scrivi_smart_charset_converter_class_init (ScriviSmartCharsetConverterClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = scrivi_smart_charset_converter_finalize;
	object_class->dispose = scrivi_smart_charset_converter_dispose;

	g_type_class_add_private (object_class, sizeof (ScriviSmartCharsetConverterPrivate));
}

static void
scrivi_smart_charset_converter_init (ScriviSmartCharsetConverter *smart)
{
	smart->priv = SCRIVI_SMART_CHARSET_CONVERTER_GET_PRIVATE (smart);
	...
	scrivi_debug_message (DEBUG_UTILS, "initializing smart charset converter");
}

static const ScriviEncoding *
get_encoding (ScriviSmartCharsetConverter *smart)
{
	...
	if (smart->priv->current_encoding != NULL)
		return (const ScriviEncoding *)smart->priv->current_encoding->data;
...
guess_encoding (ScriviSmartCharsetConverter *smart,
		const void                 *inbuf,
		gsize                       inbuf_size)
{
	...
	while (TRUE)
	{
		...
		scrivi_debug_message (DEBUG_UTILS, "trying charset: %s",
				     scrivi_encoding_get_charset (smart->priv->current_encoding->data));

		if (enc == scrivi_encoding_get_utf8 ())
		{
			gsize remainder;
			const gchar *end;
		...
		conv = g_charset_converter_new ("UTF-8",
						scrivi_encoding_get_charset (enc),
						NULL);
...
scrivi_smart_charset_converter_convert (GConverter *converter,
				       const void *inbuf,
				       gsize       inbuf_size,
				       void       *outbuf,
				       gsize       outbuf_size,
				       ...
				       GError    **error)
{
	ScriviSmartCharsetConverter *smart = SCRIVI_SMART_CHARSET_CONVERTER (converter);
	    ...
	    !smart->priv->is_utf8)
	{
		...
		if (smart->priv->charset_conv == NULL &&
		    !smart->priv->is_utf8)
		{
			/* FIXME: Add a different domain when we kill scrivi_convert */
			g_set_error_literal (error, SCRIVI_DOCUMENT_ERROR,
...
static void
scrivi_smart_charset_converter_reset (GConverter *converter)
{
	ScriviSmartCharsetConverter *smart = SCRIVI_SMART_CHARSET_CONVERTER (converter);
...
static void
scrivi_smart_charset_converter_iface_init (GConverterIface *iface)
{
	iface->convert = scrivi_smart_charset_converter_convert;
	iface->reset = scrivi_smart_charset_converter_reset;
}

ScriviSmartCharsetConverter *
scrivi_smart_charset_converter_new (GSList *candidate_encodings)
{
	ScriviSmartCharsetConverter *smart;

	g_return_val_if_fail (candidate_encodings != NULL, NULL);

	smart = g_object_new (SCRIVI_TYPE_SMART_CHARSET_CONVERTER, NULL);
...
scrivi_smart_charset_converter_get_guessed (ScriviSmartCharsetConverter *smart)
{
	g_return_val_if_fail (SCRIVI_IS_SMART_CHARSET_CONVERTER (smart), NULL);
...
guint
scrivi_smart_charset_converter_get_num_fallbacks (ScriviSmartCharsetConverter *smart)
{
	g_return_val_if_fail (SCRIVI_IS_SMART_CHARSET_CONVERTER (smart), FALSE);
