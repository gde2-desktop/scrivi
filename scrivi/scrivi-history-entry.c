...
#define SCRIVI_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT 10

#define SCRIVI_HISTORY_ENTRY_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						SCRIVI_TYPE_HISTORY_ENTRY, \
						ScriviHistoryEntryPrivate))

struct _ScriviHistoryEntryPrivate
{
	gchar              *history_id;
...
G_DEFINE_TYPE(ScriviHistoryEntry, scrivi_history_entry, GTK_TYPE_COMBO_BOX_TEXT)

static void
scrivi_history_entry_set_property (GObject      *object,
				  guint         prop_id,
				  const GValue *value,
				  GParamSpec   *spec)
{
	...
	g_return_if_fail (SCRIVI_IS_HISTORY_ENTRY (object));

	entry = SCRIVI_HISTORY_ENTRY (object);
	...
	case PROP_HISTORY_LENGTH:
		scrivi_history_entry_set_history_length (entry,
...
static void
scrivi_history_entry_get_property (GObject    *object,
				  guint       prop_id,
				  GValue     *value,
				  GParamSpec *spec)
{
	...
	priv = SCRIVI_HISTORY_ENTRY (object)->priv;
...
static void
scrivi_history_entry_dispose (GObject *object)
{
	scrivi_history_entry_set_enable_completion (SCRIVI_HISTORY_ENTRY (object),
						   FALSE);

	G_OBJECT_CLASS (scrivi_history_entry_parent_class)->dispose (object);
}

static void
scrivi_history_entry_finalize (GObject *object)
{
	ScriviHistoryEntryPrivate *priv;
	...
	G_OBJECT_CLASS (scrivi_history_entry_parent_class)->finalize (object);
}

static void 
scrivi_history_entry_class_init (ScriviHistoryEntryClass *klass)
{
	GObjectClass   *object_class = G_OBJECT_CLASS (klass);
	
	object_class->set_property = scrivi_history_entry_set_property;
	object_class->get_property = scrivi_history_entry_get_property;
	object_class->finalize = scrivi_history_entry_finalize;
	object_class->dispose = scrivi_history_entry_dispose;
	...
	g_object_class_install_property (object_class,
					 ...
					 g_param_spec_uint ("history-length",
							    ...
							    0,
							    G_MAXUINT,
							    SCRIVI_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT,
							    G_PARAM_READWRITE |
							    G_PARAM_STATIC_STRINGS));

	/* TODO: Add enable-completion property */

	g_type_class_add_private (object_class, sizeof(ScriviHistoryEntryPrivate));
}

static GtkListStore *
get_history_store (ScriviHistoryEntry *entry)
{
	GtkTreeModel *store;
...
static GSList *
get_history_list (ScriviHistoryEntry *entry)
{
	GtkListStore *store;
	GtkTreeIter iter;
	gboolean valid;
...
static void
scrivi_history_entry_save_history (ScriviHistoryEntry *entry)
{
	GSList *settings_items;

	g_return_if_fail (SCRIVI_IS_HISTORY_ENTRY (entry));

	settings_items = get_history_list (entry);

	scrivi_prefs_manager_set_gslist (entry->priv->settings,
...
static void
insert_history_item (ScriviHistoryEntry *entry,
		     const gchar       *text,
		     gboolean           prepend)
{
	GtkListStore *store;
	...
	scrivi_history_entry_save_history (entry);
}

void
scrivi_history_entry_prepend_text (ScriviHistoryEntry *entry,
				  const gchar       *text)
{
	g_return_if_fail (SCRIVI_IS_HISTORY_ENTRY (entry));
...
void
scrivi_history_entry_append_text (ScriviHistoryEntry *entry,
				 const gchar       *text)
{
	g_return_if_fail (SCRIVI_IS_HISTORY_ENTRY (entry));
	g_return_if_fail (text != NULL);
...
static void
scrivi_history_entry_load_history (ScriviHistoryEntry *entry)
{
	...
	settings_items = scrivi_prefs_manager_get_gslist (entry->priv->settings,
					     entry->priv->history_id);
...
void
scrivi_history_entry_clear (ScriviHistoryEntry *entry)
{
	GtkListStore *store;
...
static void
scrivi_history_entry_init (ScriviHistoryEntry *entry)
{
	ScriviHistoryEntryPrivate *priv;

	priv = SCRIVI_HISTORY_ENTRY_GET_PRIVATE (entry);
	entry->priv = priv;

	priv->history_id = NULL;
	priv->history_length = SCRIVI_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT;

	priv->completion = NULL;

	priv->settings = g_settings_new (SCRIVI_SCHEMA);
}

void
scrivi_history_entry_set_history_length (ScriviHistoryEntry *entry,
					guint              history_length)
{
	g_return_if_fail (SCRIVI_IS_HISTORY_ENTRY (entry));
...
guint
scrivi_history_entry_get_history_length (ScriviHistoryEntry *entry)
{
	g_return_val_if_fail (SCRIVI_IS_HISTORY_ENTRY (entry), 0);

	return entry->priv->history_length;
}

gchar *
scrivi_history_entry_get_history_id (ScriviHistoryEntry *entry)
{
	g_return_val_if_fail (SCRIVI_IS_HISTORY_ENTRY (entry), NULL);

	return g_strdup (entry->priv->history_id);
}

void
scrivi_history_entry_set_enable_completion (ScriviHistoryEntry *entry,
					   gboolean           enable)
{
	g_return_if_fail (SCRIVI_IS_HISTORY_ENTRY (entry));
	...
	if (enable)
	{
		...
		gtk_entry_set_completion (GTK_ENTRY (scrivi_history_entry_get_entry(entry)), 
...
gboolean
scrivi_history_entry_get_enable_completion (ScriviHistoryEntry *entry)
{
	g_return_val_if_fail (SCRIVI_IS_HISTORY_ENTRY (entry), FALSE);
	
	return entry->priv->completion != NULL;
}

GtkWidget *
scrivi_history_entry_new (const gchar *history_id,
			 gboolean     enable_completion)
{
	 ...
	 * data in the history through scrivi_history_entry_
	 * functions.
	 */

	store = gtk_list_store_new (1, G_TYPE_STRING);

	ret = g_object_new (SCRIVI_TYPE_HISTORY_ENTRY,
	 ...
	 */
	scrivi_history_entry_load_history (SCRIVI_HISTORY_ENTRY (ret));

	scrivi_history_entry_set_enable_completion (SCRIVI_HISTORY_ENTRY (ret),
						   enable_completion);
...
void
scrivi_history_entry_set_escape_func (ScriviHistoryEntry           *entry,
				     ScriviHistoryEntryEscapeFunc  escape_func)
{
	GList *cells;
