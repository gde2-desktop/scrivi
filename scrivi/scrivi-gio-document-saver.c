...
#include "scrivi-document-input-stream.h"
...
#define SCRIVI_GIO_DOCUMENT_SAVER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
							  SCRIVI_TYPE_GIO_DOCUMENT_SAVER, \
							  ScriviGioDocumentSaverPrivate))

static void	     scrivi_gio_document_saver_save		    (ScriviDocumentSaver *saver,
								     GTimeVal           *old_mtime);
static goffset	     scrivi_gio_document_saver_get_file_size	    (ScriviDocumentSaver *saver);
static goffset	     scrivi_gio_document_saver_get_bytes_written	    (ScriviDocumentSaver *saver);


static void 	    check_modified_async 			    (AsyncData          *async);

struct _ScriviGioDocumentSaverPrivate
{
	GTimeVal		  old_mtime;
...
G_DEFINE_TYPE(ScriviGioDocumentSaver, scrivi_gio_document_saver, SCRIVI_TYPE_DOCUMENT_SAVER)

static void
scrivi_gio_document_saver_dispose (GObject *object)
{
	ScriviGioDocumentSaverPrivate *priv = SCRIVI_GIO_DOCUMENT_SAVER (object)->priv;
	...
	G_OBJECT_CLASS (scrivi_gio_document_saver_parent_class)->dispose (object);
}

static AsyncData *
async_data_new (ScriviGioDocumentSaver *gvsaver)
{
	AsyncData *async;
...
static void 
scrivi_gio_document_saver_class_init (ScriviGioDocumentSaverClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	ScriviDocumentSaverClass *saver_class = SCRIVI_DOCUMENT_SAVER_CLASS (klass);

	object_class->dispose = scrivi_gio_document_saver_dispose;

	saver_class->save = scrivi_gio_document_saver_save;
	saver_class->get_file_size = scrivi_gio_document_saver_get_file_size;
	saver_class->get_bytes_written = scrivi_gio_document_saver_get_bytes_written;

	g_type_class_add_private (object_class, sizeof(ScriviGioDocumentSaverPrivate));
}

static void
scrivi_gio_document_saver_init (ScriviGioDocumentSaver *gvsaver)
{
	gvsaver->priv = SCRIVI_GIO_DOCUMENT_SAVER_GET_PRIVATE (gvsaver);
...
static void
remote_save_completed_or_failed (ScriviGioDocumentSaver *gvsaver, 
				 AsyncData 	       *async)
{
	scrivi_document_saver_saving (SCRIVI_DOCUMENT_SAVER (gvsaver),
				     TRUE,
				     gvsaver->priv->error);
 ...
 *
 * Bug 615110 - write file ignore encoding errors (scrivi)
...
cancel_output_stream (AsyncData *async)
{
	...
	scrivi_debug_message (DEBUG_SAVER, "Cancel output stream");
                               ...
                               GError    *error)
{
	...
	scrivi_debug_message (DEBUG_SAVER, "Cancel output stream and fail");
		    ...
		    AsyncData    *async)
{
	...
	scrivi_debug_message (DEBUG_SAVER, "Finished query info on file");
	info = g_file_query_info_finish (source, res, &error);

	if (info != NULL)
	{
		if (SCRIVI_DOCUMENT_SAVER (saver)->info != NULL)
			g_object_unref (SCRIVI_DOCUMENT_SAVER (saver)->info);

		SCRIVI_DOCUMENT_SAVER (saver)->info = info;
	}
	else
	{
		scrivi_debug_message (DEBUG_SAVER, "Query info failed: %s", error->message);
			       ...
			       AsyncData     *async)
{
	...
	scrivi_debug_message (DEBUG_SAVER, "Finished closing stream");
	
	if (!g_output_stream_close_finish (stream, res, &error))
	{
		scrivi_debug_message (DEBUG_SAVER, "Closing stream error: %s", error->message);
	 ...
	 */
	scrivi_debug_message (DEBUG_SAVER, "Query info on file");
...
write_complete (AsyncData *async)
{
	...
	scrivi_debug_message (DEBUG_SAVER, "Close input stream");
	if (!g_input_stream_close (async->saver->priv->input,
				   async->cancellable, &error))
	{
		scrivi_debug_message (DEBUG_SAVER, "Closing input stream error: %s", error->message);
		cancel_output_stream_and_fail (async, error);
		return;
	}

	/* now we close the output stream */
	scrivi_debug_message (DEBUG_SAVER, "Close output stream");
	g_output_stream_close_async (async->saver->priv->stream,
				     G_PRIORITY_HIGH,
				     async->cancellable,
...
async_write_cb (GOutputStream *stream,
		...
		AsyncData     *async)
{
	ScriviGioDocumentSaver *gvsaver;
	...
	scrivi_debug_message (DEBUG_SAVER, "Written: %" G_GSSIZE_FORMAT, bytes_written);

	if (bytes_written == -1)
	{
		scrivi_debug_message (DEBUG_SAVER, "Write error: %s", error->message);
		cancel_output_stream_and_fail (async, error);
		return;
	}
...
{
	ScriviGioDocumentSaver *gvsaver;
	ScriviDocumentInputStream *dstream;
	GError *error = NULL;
	...
	dstream = SCRIVI_DOCUMENT_INPUT_STREAM (gvsaver->priv->input);
	gvsaver->priv->bytes_written = scrivi_document_input_stream_tell (dstream);
			      ...
			      AsyncData    *async)
{
	...
	gvsaver = async->saver;
	saver = SCRIVI_DOCUMENT_SAVER (gvsaver);
	file_stream = g_file_replace_finish (source, res, &error);
	
	/* handle any error that might occur */
	if (!file_stream)
	{
		scrivi_debug_message (DEBUG_SAVER, "Opening file failed: %s", error->message);
		async_failed (async, error);
		return;
	}

	/* FIXME: manage converter error? */
	scrivi_debug_message (DEBUG_SAVER, "Encoding charset: %s",
			     scrivi_encoding_get_charset (saver->encoding));

	if (saver->encoding != scrivi_encoding_get_utf8 ())
	{
		converter = g_charset_converter_new (scrivi_encoding_get_charset (saver->encoding),
						     "UTF-8",
						     NULL);
	...
	gvsaver->priv->input = scrivi_document_input_stream_new (GTK_TEXT_BUFFER (saver->document),
								saver->newline_type);

	gvsaver->priv->size = scrivi_document_input_stream_get_total_size (SCRIVI_DOCUMENT_INPUT_STREAM (gvsaver->priv->input));
...
begin_write (AsyncData *async)
{
	...
	scrivi_debug_message (DEBUG_SAVER, "Start replacing file contents");
	...
	backup = (saver->keep_backup && scrivi_document_is_local (saver->document));

	scrivi_debug_message (DEBUG_SAVER, "File contents size: %" G_GINT64_FORMAT, gvsaver->priv->size);
	scrivi_debug_message (DEBUG_SAVER, "Calling replace_async");
	scrivi_debug_message (DEBUG_SAVER, backup ? "Keep backup" : "Discard backup");
...
recover_not_mounted (AsyncData *async)
{
	...
	scrivi_debug (DEBUG_LOADER);

	doc = scrivi_document_saver_get_document (SCRIVI_DOCUMENT_SAVER (async->saver));
	mount_operation = _scrivi_document_create_mount_operation (doc);
			     ...
			     AsyncData    *async)
{
	...
	if (info == NULL)
	{
		...
		{
			scrivi_debug_message (DEBUG_SAVER, "Error getting modification: %s", error->message);
				       ...
				       G_FILE_ATTRIBUTE_TIME_MODIFIED))
	{
		...
		if ((old_mtime.tv_sec > 0 || old_mtime.tv_usec > 0) &&
		    ...
		    (SCRIVI_DOCUMENT_SAVER (gvsaver)->flags & SCRIVI_DOCUMENT_SAVE_IGNORE_MTIME) == 0)
		{
			scrivi_debug_message (DEBUG_SAVER, "File is externally modified");
			g_set_error (&gvsaver->priv->error,
				     SCRIVI_DOCUMENT_ERROR,
...
{
	scrivi_debug_message (DEBUG_SAVER, "Check externally modified");
...
static gboolean
save_remote_file_real (ScriviGioDocumentSaver *gvsaver)
{
	AsyncData *async;

	scrivi_debug_message (DEBUG_SAVER, "Starting gio save");
...
static void
scrivi_gio_document_saver_save (ScriviDocumentSaver *saver,
			       GTimeVal           *old_mtime)
{
	ScriviGioDocumentSaver *gvsaver = SCRIVI_GIO_DOCUMENT_SAVER (saver);

	gvsaver->priv->old_mtime = *old_mtime;
	gvsaver->priv->gfile = g_file_new_for_uri (saver->uri);

	/* saving start */
	scrivi_document_saver_saving (saver, FALSE, NULL);
...
static goffset
scrivi_gio_document_saver_get_file_size (ScriviDocumentSaver *saver)
{
	return SCRIVI_GIO_DOCUMENT_SAVER (saver)->priv->size;
}

static goffset
scrivi_gio_document_saver_get_bytes_written (ScriviDocumentSaver *saver)
{
	return SCRIVI_GIO_DOCUMENT_SAVER (saver)->priv->bytes_written;
}
