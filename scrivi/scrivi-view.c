...
#define SCRIVI_VIEW_SCROLL_MARGIN 0.02
#define SCRIVI_VIEW_SEARCH_DIALOG_TIMEOUT (30*1000) /* 30 seconds */

#define MIN_SEARCH_COMPLETION_KEY_LEN	3

#define SCRIVI_VIEW_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_VIEW, ScriviViewPrivate))
...
static void	scrivi_view_dispose		(GObject          *object);
static void	scrivi_view_finalize		(GObject          *object);
static gint     scrivi_view_focus_out		(GtkWidget        *widget,
						 GdkEventFocus    *event);
static gboolean scrivi_view_drag_motion		(GtkWidget        *widget,
						 GdkDragContext   *context,
						 gint              x,
						 gint              y,
						 guint             timestamp);
static void     scrivi_view_drag_data_received   (GtkWidget        *widget,
						 GdkDragContext   *context,
						 gint              x,
						 gint              y,
						 ...
						 guint             info,
						 guint             timestamp);
static gboolean scrivi_view_drag_drop		(GtkWidget        *widget,
	      					 GdkDragContext   *context,
	      					 gint              x,
	      					 gint              y,
	      					 guint             timestamp);
static gboolean	scrivi_view_button_press_event	(GtkWidget        *widget,
						 GdkEventButton   *event);

static gboolean start_interactive_search	(ScriviView        *view);
static gboolean start_interactive_goto_line	(ScriviView        *view);
static gboolean reset_searched_text		(ScriviView        *view);

static void	hide_search_window 		(ScriviView        *view,
						 gboolean          cancel);

#if GTK_CHECK_VERSION (3, 0, 0)
static gboolean	scrivi_view_draw		 	(GtkWidget        *widget,
						 cairo_t          *cr);
#else
static gint	scrivi_view_expose	 	(GtkWidget        *widget,
						 GdkEventExpose   *event);
#endif
static void 	search_highlight_updated_cb	(ScriviDocument    *doc,
						 GtkTextIter      *start,
						 GtkTextIter      *end,
						 ScriviView        *view);

static void	scrivi_view_delete_from_cursor 	(GtkTextView     *text_view,
						 GtkDeleteType    type,
						 gint             count);

#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE(ScriviView, scrivi_view, GTK_SOURCE_TYPE_VIEW)
#else
G_DEFINE_TYPE(ScriviView, scrivi_view, GTK_TYPE_SOURCE_VIEW)
#endif
...
typedef enum
{
	SCRIVI_SEARCH_ENTRY_NORMAL,
	SCRIVI_SEARCH_ENTRY_NOT_FOUND
} ScriviSearchEntryState;

static void
document_read_only_notify_handler (ScriviDocument *document, 
			           GParamSpec    *pspec,
				   ScriviView     *view)
{
	scrivi_debug (DEBUG_VIEW);

	gtk_text_view_set_editable (GTK_TEXT_VIEW (view), 
				    !scrivi_document_get_readonly (document));
}

static void
scrivi_view_class_init (ScriviViewClass *klass)
{
	...
	object_class->dispose = scrivi_view_dispose;
	object_class->finalize = scrivi_view_finalize;

	widget_class->focus_out_event = scrivi_view_focus_out;
#if GTK_CHECK_VERSION (3, 0, 0)
	widget_class->draw = scrivi_view_draw;
#else
	widget_class->expose_event = scrivi_view_expose;
#endif
	 ...
	 */
	widget_class->drag_motion = scrivi_view_drag_motion;
	widget_class->drag_data_received = scrivi_view_drag_data_received;
	widget_class->drag_drop = scrivi_view_drag_drop;
	widget_class->button_press_event = scrivi_view_button_press_event;
	...
	text_view_class->delete_from_cursor = scrivi_view_delete_from_cursor;
	
	view_signals[START_INTERACTIVE_SEARCH] =
    		g_signal_new ("start_interactive_search",
		  	      G_TYPE_FROM_CLASS (object_class),
		  	      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		  	      G_STRUCT_OFFSET (ScriviViewClass, start_interactive_search),
			      NULL, NULL,
			      scrivi_marshal_BOOLEAN__NONE,
			      G_TYPE_BOOLEAN, 0);	
	...
	view_signals[START_INTERACTIVE_GOTO_LINE] =
    		g_signal_new ("start_interactive_goto_line",
		  	      G_TYPE_FROM_CLASS (object_class),
		  	      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		  	      G_STRUCT_OFFSET (ScriviViewClass, start_interactive_goto_line),
			      NULL, NULL,
			      scrivi_marshal_BOOLEAN__NONE,
			      G_TYPE_BOOLEAN, 0);
	...
	view_signals[RESET_SEARCHED_TEXT] =
    		g_signal_new ("reset_searched_text",
		  	      G_TYPE_FROM_CLASS (object_class),
		  	      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		  	      G_STRUCT_OFFSET (ScriviViewClass, reset_searched_text),
			      NULL, NULL,
			      scrivi_marshal_BOOLEAN__NONE,
			      G_TYPE_BOOLEAN, 0);		

	/* A new signal DROP_URIS has been added to allow plugins to intercept
	 * the default dnd behaviour of 'text/uri-list'. ScriviView now handles
	 ...
	 * will not prevent scrivi from loading the uri
	 */
	view_signals[DROP_URIS] =
    		g_signal_new ("drop_uris",
		  	      G_TYPE_FROM_CLASS (object_class),
		  	      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
		  	      G_STRUCT_OFFSET (ScriviViewClass, drop_uris),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__BOXED,
			      G_TYPE_NONE, 1, G_TYPE_STRV);
			      
	g_type_class_add_private (klass, sizeof (ScriviViewPrivate));
...
static void
current_buffer_removed (ScriviView *view)
{
...
static void
on_notify_buffer_cb (ScriviView  *view,
		     GParamSpec *arg1,
		     gpointer    userdata)
{
	...
	if (buffer == NULL || !SCRIVI_IS_DOCUMENT (buffer))
		return;
	...
	gtk_text_view_set_editable (GTK_TEXT_VIEW (view), 
				    !scrivi_document_get_readonly (SCRIVI_DOCUMENT (buffer)));
...
static void 
scrivi_view_init (ScriviView *view)
{
	GtkTargetList *tl;
	
	scrivi_debug (DEBUG_VIEW);
	
	view->priv = SCRIVI_VIEW_GET_PRIVATE (view);
	...
	if (!scrivi_prefs_manager_get_use_default_font ())
	{
		...
		editor_font = scrivi_prefs_manager_get_editor_font ();

		scrivi_view_set_font (view, FALSE, editor_font);

		g_free (editor_font);
	}
	else
	{
		scrivi_view_set_font (view, TRUE, NULL);
	}

	g_object_set (G_OBJECT (view), 
		      "wrap_mode", scrivi_prefs_manager_get_wrap_mode (),
		      "show_line_numbers", scrivi_prefs_manager_get_display_line_numbers (),
		      "auto_indent", scrivi_prefs_manager_get_auto_indent (),
		      "tab_width", scrivi_prefs_manager_get_tabs_size (),
		      "insert_spaces_instead_of_tabs", scrivi_prefs_manager_get_insert_spaces (),
		      "show_right_margin", scrivi_prefs_manager_get_display_right_margin (), 
		      "right_margin_position", scrivi_prefs_manager_get_right_margin_position (),
		      "highlight_current_line", scrivi_prefs_manager_get_highlight_current_line (),
		      "smart_home_end", scrivi_prefs_manager_get_smart_home_end (),
		      "indent_on_tab", TRUE,
		      NULL);
...
static void
scrivi_view_dispose (GObject *object)
{
	ScriviView *view;

	view = SCRIVI_VIEW (object);
	...
	(* G_OBJECT_CLASS (scrivi_view_parent_class)->dispose) (object);
}

static void
scrivi_view_finalize (GObject *object)
{
	ScriviView *view;
	...
	(* G_OBJECT_CLASS (scrivi_view_parent_class)->finalize) (object);
}

static gint
scrivi_view_focus_out (GtkWidget *widget, GdkEventFocus *event)
{
	ScriviView *view = SCRIVI_VIEW (widget);
	...
	(* GTK_WIDGET_CLASS (scrivi_view_parent_class)->focus_out_event) (widget, event);
 ...
 * 
 * Creates a new #ScriviView object displaying the @doc document. 
 * @doc cannot be %NULL.
 *
 * Return value: a new #ScriviView
 **/
GtkWidget *
scrivi_view_new (ScriviDocument *doc)
{
	GtkWidget *view;

	scrivi_debug_message (DEBUG_VIEW, "START");

	g_return_val_if_fail (SCRIVI_IS_DOCUMENT (doc), NULL);

	view = GTK_WIDGET (g_object_new (SCRIVI_TYPE_VIEW, "buffer", doc, NULL));

	scrivi_debug_message (DEBUG_VIEW, "END: %d", G_OBJECT (view)->ref_count);
...
void
scrivi_view_cut_clipboard (ScriviView *view)
{
	GtkTextBuffer *buffer;
	...
	/* FIXME: what is default editability of a buffer? */
  	gtk_text_buffer_cut_clipboard (buffer,
  				       clipboard,
				       !scrivi_document_get_readonly (
				       		SCRIVI_DOCUMENT (buffer)));
  	
	gtk_text_view_scroll_to_mark (GTK_TEXT_VIEW (view),
				      gtk_text_buffer_get_insert (buffer),
				      SCRIVI_VIEW_SCROLL_MARGIN,
				      FALSE,
				      0.0,
				      0.0);
}
...
void
scrivi_view_copy_clipboard (ScriviView *view)
{
	GtkTextBuffer *buffer;
	GtkClipboard *clipboard;
...
void
scrivi_view_paste_clipboard (ScriviView *view)
{
  	GtkTextBuffer *buffer;
	GtkClipboard *clipboard;
 ...
 **/
void
scrivi_view_delete_selection (ScriviView *view)
{
...
/**
 * scrivi_view_select_all:
 * @view: a #ScriviView
 * 
 * Selects all the text displayed in the @view.
 **/
void
scrivi_view_select_all (ScriviView *view)
{
	GtkTextBuffer *buffer = NULL;
...
/**
 * scrivi_view_scroll_to_cursor:
 * @view: a #ScriviView
 * 
 * Scrolls the @view to the cursor position.
 **/
void
scrivi_view_scroll_to_cursor (ScriviView *view)
{
	GtkTextBuffer* buffer = NULL;
 ...
 **/
void
scrivi_view_set_font (ScriviView   *view, 
		     gboolean     def, 
...
add_search_completion_entry (const gchar *str)
{
	...
	text = scrivi_utils_unescape_search_text (str);
...
set_entry_state (GtkWidget             *entry,
                 ScriviSearchEntryState  state)
{
#if GTK_CHECK_VERSION (3, 0 ,0)
	GtkStyleContext *context = gtk_widget_get_style_context (GTK_WIDGET (entry));
#endif

	if (state == SCRIVI_SEARCH_ENTRY_NOT_FOUND)
	{
#if GTK_CHECK_VERSION (3, 0 ,0)
...
static void
hide_search_window (ScriviView *view, gboolean cancel)
{
...
static gboolean
search_entry_flush_timeout (ScriviView *view)
{
#if !GTK_CHECK_VERSION (3, 0, 0)
	GDK_THREADS_ENTER ();
#endif
...
static void
update_search_window_position (ScriviView *view)
{
	gint x, y;
	gint view_x, view_y;
	      ...
	      gboolean   search_backward)
{
	...
	if (view->priv->typeselect_flush_timeout != 0)
	{
		...
		view->priv->typeselect_flush_timeout =
			g_timeout_add (SCRIVI_VIEW_SEARCH_DIALOG_TIMEOUT,
			       ...
			       ScriviView   *view)
{
	...
	if (event->keyval == GDK_KEY_Escape)
	{
		if (view->priv->search_mode == SEARCH)
		{
			...
			doc = SCRIVI_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)));
			scrivi_document_set_search_text (doc, 
...
match_entire_word_menu_item_toggled (GtkCheckMenuItem *checkmenuitem,
				     ScriviView        *view)
{
	SCRIVI_SEARCH_SET_ENTIRE_WORD (view->priv->search_flags,
...
match_case_menu_item_toggled (GtkCheckMenuItem *checkmenuitem,
			      ScriviView        *view)
{
	SCRIVI_SEARCH_SET_CASE_SENSITIVE (view->priv->search_flags,
					 gtk_check_menu_item_get_active (checkmenuitem));
}
...
parse_escapes_menu_item_toggled (GtkCheckMenuItem *checkmenuitem,
			         ScriviView        *view)
{
	SCRIVI_SEARCH_SET_PARSE_ESCAPES (view->priv->search_flags,
					gtk_check_menu_item_get_active (checkmenuitem));
}
...
{
	ScriviView *view = (ScriviView *)data;
			     ...
			     ScriviView *view)
{
	...
	gtk_menu_shell_prepend (GTK_MENU_SHELL (menu), menu_item);
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menu_item),
					SCRIVI_SEARCH_IS_ENTIRE_WORD (view->priv->search_flags));
	gtk_widget_show (menu_item);
	...
	gtk_menu_shell_prepend (GTK_MENU_SHELL (menu), menu_item);
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menu_item),
					SCRIVI_SEARCH_IS_CASE_SENSITIVE (view->priv->search_flags));
	gtk_widget_show (menu_item);
	...
	gtk_menu_shell_prepend (GTK_MENU_SHELL (menu), menu_item);
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (menu_item),
					SCRIVI_SEARCH_IS_PARSE_ESCAPES (view->priv->search_flags));
	gtk_widget_show (menu_item);
}
			  ...
			  ScriviView   *view)
{
	...
	else
	{
		...
		scrivi_debug_message (DEBUG_SEARCH, "Text: %s", text);

		/* To avoid recursive behavior */
		if (insert_text)
			return;

		escaped_text = scrivi_utils_escape_search_text (text);

		scrivi_debug_message (DEBUG_SEARCH, "Escaped Text: %s", escaped_text);
...
static void
customize_for_search_mode (ScriviView *view)
{
	if (view->priv->search_mode == SEARCH)
	{
		 ...
		 gpointer            data)
{
	...
	gboolean ret = FALSE;
	GtkTreeModel *model;
	ScriviViewPrivate *priv = (ScriviViewPrivate *)data;
	...
	if (SCRIVI_SEARCH_IS_CASE_SENSITIVE (priv->search_flags))
	{		
...
static void
ensure_search_window (ScriviView *view)
{  
	GtkWidget          *frame;
	GtkWidget          *vbox;
	GtkWidget          *toplevel;
...
static void
init_search_entry (ScriviView *view)
{
	GtkTextBuffer *buffer;
	...
	else
	{
		...
		old_find_text = scrivi_document_get_search_text (SCRIVI_DOCUMENT (buffer), 
								&old_find_flags);
	     ...
	     ScriviView *view)
{
	...
	if (view->priv->search_mode == SEARCH)
	{
		...
		search_text = scrivi_document_get_search_text (doc, &search_flags);
	...
	else
	{
		if (*entry_text != '\0')
		{
			...
			moved = scrivi_document_goto_line (doc, line);
			moved_offset = scrivi_document_goto_line_offset (doc, line,
									line_offset);
...
static gboolean
start_interactive_search_real (ScriviView *view)
{	
	GtkTextBuffer *buffer;
...
static gboolean
reset_searched_text (ScriviView *view)
{		
	ScriviDocument *doc;

	doc = SCRIVI_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)));
	
	scrivi_document_set_search_text (doc, "", SCRIVI_SEARCH_DONT_SET_FLAGS);
	
	return TRUE;
}

static gboolean
start_interactive_search (ScriviView *view)
{		
...
static gboolean 
start_interactive_goto_line (ScriviView *view)
{
...
#if GTK_CHECK_VERSION (3, 0, 0)
static gboolean
scrivi_view_draw (GtkWidget      *widget,
                 cairo_t        *cr)
#else
static gint
scrivi_view_expose (GtkWidget      *widget,
                   GdkEventExpose *event)
#endif
{
	...
	doc = SCRIVI_DOCUMENT (gtk_text_view_get_buffer (text_view));
#if GTK_CHECK_VERSION (3, 0, 0)
...
#endif
	    scrivi_document_get_enable_search_highlighting (doc))
	{
		...
		_scrivi_document_search_region (doc,
					       &iter1,
					       &iter2);
	}

#if GTK_CHECK_VERSION (3, 0, 0)
	return GTK_WIDGET_CLASS (scrivi_view_parent_class)->draw (widget, cr);
#else
	return (* GTK_WIDGET_CLASS (scrivi_view_parent_class)->expose_event)(widget, event);
#endif
}
...
static gboolean
scrivi_view_drag_motion (GtkWidget      *widget,
			GdkDragContext *context,
			gint            x,
			gint            y,
			guint           timestamp)
{
	...
	result = GTK_WIDGET_CLASS (scrivi_view_parent_class)->drag_motion (widget, context, x, y, timestamp);
...
static void
scrivi_view_drag_data_received (GtkWidget        *widget,
		       	       GdkDragContext   *context,
			       gint              x,
			       gint              y,
			       GtkSelectionData *selection_data,
			       ...
			       guint             timestamp)
{
	...
	{
		GTK_WIDGET_CLASS (scrivi_view_parent_class)->drag_data_received (widget, context, x, y, selection_data, info, timestamp);
	}
}

static gboolean
scrivi_view_drag_drop (GtkWidget      *widget,
		      GdkDragContext *context,
		      gint            x,
		      gint            y,
		      guint           timestamp)
{
	...
	{
		/* Chain up */
		result = GTK_WIDGET_CLASS (scrivi_view_parent_class)->drag_drop (widget, context, x, y, timestamp);
	}
			   ...
			   ScriviView *view)
{
	...
	scrivi_prefs_manager_set_display_line_numbers (show);
}
...
static gboolean
scrivi_view_button_press_event (GtkWidget *widget, GdkEventButton *event)
{
	...
	return GTK_WIDGET_CLASS (scrivi_view_parent_class)->button_press_event (widget, event);
}

static void 	
search_highlight_updated_cb (ScriviDocument *doc,
			     GtkTextIter   *start,
			     GtkTextIter   *end,
			     ScriviView     *view)
{
	...
	g_return_if_fail (scrivi_document_get_enable_search_highlighting (
				SCRIVI_DOCUMENT (gtk_text_view_get_buffer (text_view))));
...
static void
scrivi_view_delete_from_cursor (GtkTextView   *text_view,
			       GtkDeleteType  type,
			       gint           count)
{
	...
	switch (type)
	{
		case GTK_DELETE_PARAGRAPHS:
			...
			break;
		default:
			GTK_TEXT_VIEW_CLASS (scrivi_view_parent_class)->delete_from_cursor(text_view, type, count);
			break;
	}
}
