...
#include "scrivi-language-manager.h"
#include "scrivi-prefs-manager-app.h"
#include "scrivi-panel.h"
#include "scrivi-documents-panel.h"
...
#define SCRIVI_UIFILE "scrivi-ui.xml"
#define TAB_WIDTH_DATA "ScriviWindowTabWidthData"
#define LANGUAGE_DATA "ScriviWindowLanguageData"
#define FULLSCREEN_ANIMATION_SPEED 4

#define SCRIVI_WINDOW_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object),\
					 SCRIVI_TYPE_WINDOW,                    \
					 ScriviWindowPrivate))
...
G_DEFINE_TYPE(ScriviWindow, scrivi_window, GTK_TYPE_WINDOW)

static void	recent_manager_changed	(GtkRecentManager *manager,
					 ScriviWindow *window);

static void
scrivi_window_get_property (GObject    *object,
			   guint       prop_id,
			   GValue     *value,
			   GParamSpec *pspec)
{
	ScriviWindow *window = SCRIVI_WINDOW (object);

	switch (prop_id)
	{
		case PROP_STATE:
			g_value_set_enum (value,
					  scrivi_window_get_state (window));
			break;			
		default:
...
static void
save_panes_state (ScriviWindow *window)
{
	gint pane_page;

	scrivi_debug (DEBUG_WINDOW);

	if (scrivi_prefs_manager_window_size_can_set ())
		scrivi_prefs_manager_set_window_size (window->priv->width,
						     window->priv->height);

	if (scrivi_prefs_manager_window_state_can_set ())
		scrivi_prefs_manager_set_window_state (window->priv->window_state);

	if ((window->priv->side_panel_size > 0) &&
	    scrivi_prefs_manager_side_panel_size_can_set ())
		scrivi_prefs_manager_set_side_panel_size	(
					window->priv->side_panel_size);

	pane_page = _scrivi_panel_get_active_item_id (SCRIVI_PANEL (window->priv->side_panel));
	if (pane_page != 0 &&
	    scrivi_prefs_manager_side_panel_active_page_can_set ())
		scrivi_prefs_manager_set_side_panel_active_page (pane_page);

	if ((window->priv->bottom_panel_size > 0) && 
	    scrivi_prefs_manager_bottom_panel_size_can_set ())
		scrivi_prefs_manager_set_bottom_panel_size (
					window->priv->bottom_panel_size);

	pane_page = _scrivi_panel_get_active_item_id (SCRIVI_PANEL (window->priv->bottom_panel));
	if (pane_page != 0 &&
	    scrivi_prefs_manager_bottom_panel_active_page_can_set ())
		scrivi_prefs_manager_set_bottom_panel_active_page (pane_page);
}

static void
scrivi_window_dispose (GObject *object)
{
	ScriviWindow *window;

	scrivi_debug (DEBUG_WINDOW);

	window = SCRIVI_WINDOW (object);
	 ...
	 */
	scrivi_plugins_engine_garbage_collect (scrivi_plugins_engine_get_default ());
	...
	if (!window->priv->dispose_has_run)
	{
		...
		scrivi_plugins_engine_deactivate_plugins (scrivi_plugins_engine_get_default (),
					                  window);
	...
	G_OBJECT_CLASS (scrivi_window_parent_class)->dispose (object);
}

static void
scrivi_window_finalize (GObject *object)
{
	ScriviWindow *window; 
	...
	G_OBJECT_CLASS (scrivi_window_parent_class)->finalize (object);
}

static gboolean
scrivi_window_window_state_event (GtkWidget           *widget,
				 GdkEventWindowState *event)
{
	ScriviWindow *window = SCRIVI_WINDOW (widget);
...
static gboolean 
scrivi_window_configure_event (GtkWidget         *widget,
			      GdkEventConfigure *event)
{
	...
	return GTK_WIDGET_CLASS (scrivi_window_parent_class)->configure_event (widget, event);
}
 ...
 */
static gboolean
scrivi_window_key_press_event (GtkWidget   *widget,
			      GdkEventKey *event)
{
	...
	if (grand_parent_class == NULL)
		grand_parent_class = g_type_class_peek_parent (scrivi_window_parent_class);
...
static void
scrivi_window_tab_removed (ScriviWindow *window,
			  ScriviTab    *tab) 
{
	scrivi_plugins_engine_garbage_collect (scrivi_plugins_engine_get_default ());
}
...
static void
scrivi_window_class_init (ScriviWindowClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

	klass->tab_removed = scrivi_window_tab_removed;

	object_class->dispose = scrivi_window_dispose;
	object_class->finalize = scrivi_window_finalize;
	object_class->get_property = scrivi_window_get_property;

	widget_class->window_state_event = scrivi_window_window_state_event;
	widget_class->configure_event = scrivi_window_configure_event;
	widget_class->key_press_event = scrivi_window_key_press_event;

	signals[TAB_ADDED] =
		g_signal_new ("tab_added",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviWindowClass, tab_added),
			      NULL, NULL,
	...
	signals[TAB_REMOVED] =
		g_signal_new ("tab_removed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviWindowClass, tab_removed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	signals[TABS_REORDERED] =
		g_signal_new ("tabs_reordered",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviWindowClass, tabs_reordered),
			      NULL, NULL,
	...
	signals[ACTIVE_TAB_CHANGED] =
		g_signal_new ("active_tab_changed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviWindowClass, active_tab_changed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	signals[ACTIVE_TAB_STATE_CHANGED] =
		g_signal_new ("active_tab_state_changed",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviWindowClass, active_tab_state_changed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__VOID,
			      G_TYPE_NONE,
			      0);			      			      
	...
	g_object_class_install_property (object_class,
					 ...
					 g_param_spec_flags ("state",
							     "State",
							     "The window's state",
							     SCRIVI_TYPE_WINDOW_STATE,
							     SCRIVI_WINDOW_STATE_NORMAL,
							     G_PARAM_READABLE |
							     G_PARAM_STATIC_STRINGS));

	g_type_class_add_private (object_class, sizeof(ScriviWindowPrivate));
}
...
static void
apply_toolbar_style (ScriviWindow *window,
		     GtkWidget *toolbar)
{
	switch (window->priv->toolbar_style)
	{
		case SCRIVI_TOOLBAR_SYSTEM:
			scrivi_debug_message (DEBUG_WINDOW, "SCRIVI: SYSTEM");
			gtk_toolbar_unset_style (
					GTK_TOOLBAR (toolbar));
			break;

		case SCRIVI_TOOLBAR_ICONS:
			scrivi_debug_message (DEBUG_WINDOW, "SCRIVI: ICONS");
		...
		case SCRIVI_TOOLBAR_ICONS_AND_TEXT:
			scrivi_debug_message (DEBUG_WINDOW, "SCRIVI: ICONS_AND_TEXT");
			gtk_toolbar_set_style (
					GTK_TOOLBAR (toolbar),
					GTK_TOOLBAR_BOTH);
			break;
		...
		case SCRIVI_TOOLBAR_ICONS_BOTH_HORIZ:
			scrivi_debug_message (DEBUG_WINDOW, "SCRIVI: ICONS_BOTH_HORIZ");
			gtk_toolbar_set_style (
					GTK_TOOLBAR (toolbar),
...
static gboolean
set_toolbar_style (ScriviWindow *window,
		   ScriviWindow *origin)
{
	gboolean visible;
	ScriviToolbarSetting style;
	GtkAction *action;
	
	if (origin == NULL)
		visible = scrivi_prefs_manager_get_toolbar_visible ();
	else
	...
	/* Set style */
	if (origin == NULL)
		style = scrivi_prefs_manager_get_toolbar_buttons_style ();
	else
...
static void
update_next_prev_doc_sensitivity (ScriviWindow *window,
				  ScriviTab    *tab)
{
	gint	     tab_number;
...
static void
update_next_prev_doc_sensitivity_per_window (ScriviWindow *window)
{
	ScriviTab  *tab;
	GtkAction *action;
			     ...
			     ScriviWindow      *window)
{
	...
	if (window->priv->active_tab != NULL)
	{
		...
		state = scrivi_tab_get_state (window->priv->active_tab);
		state_normal = (state == SCRIVI_TAB_STATE_NORMAL);
...
static void
set_paste_sensitivity_according_to_clipboard (ScriviWindow  *window,
...
static void
set_sensitivity_according_to_tab (ScriviWindow *window,
				  ScriviTab    *tab)
{
	ScriviDocument *doc;
	ScriviView     *view;
	...
	g_return_if_fail (SCRIVI_TAB (tab));

	scrivi_debug (DEBUG_WINDOW);

	lockdown = scrivi_app_get_lockdown (scrivi_app_get_default ());
	...
	gtk_action_set_sensitive (action,
				  (state_normal ||
				   (state == SCRIVI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION) ||
	...
	gtk_action_set_sensitive (action,
				  (state_normal ||
				   (state == SCRIVI_TAB_STATE_SAVING_ERROR) ||
				   (state == SCRIVI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION) ||
				   (state == SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW)) &&
				  !(lockdown & SCRIVI_LOCKDOWN_SAVE_TO_DISK));
	...
	gtk_action_set_sensitive (action,
				  (state != SCRIVI_TAB_STATE_CLOSING) &&
				  (state != SCRIVI_TAB_STATE_SAVING) &&
				  (state != SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW) &&
				  (state != SCRIVI_TAB_STATE_PRINTING) &&
				  (state != SCRIVI_TAB_STATE_PRINT_PREVIEWING) &&
				  (state != SCRIVI_TAB_STATE_SAVING_ERROR));
	...
	b = scrivi_document_get_can_search_again (doc);
	action = gtk_action_group_get_action (window->priv->action_group,
					      "SearchFindNext");
	gtk_action_set_sensitive (action,
				  (state_normal ||
				   state == SCRIVI_TAB_STATE_EXTERNALLY_MODIFIED_NOTIFICATION) && b);
	...
	gtk_action_set_sensitive (action, 
				  (state != SCRIVI_TAB_STATE_CLOSING) &&
				  scrivi_prefs_manager_get_enable_syntax_highlighting ());

	update_next_prev_doc_sensitivity (window, tab);

	scrivi_plugins_engine_update_plugins_ui (scrivi_plugins_engine_get_default (),
						 window);
}
		  ...
		  ScriviWindow     *window)
{
	...
	scrivi_document_set_language (doc, lang);
}
			   ...
			   ScriviWindow       *window)
{
	...
	if (section_action == NULL)
	{
		...
		section_name = scrivi_utils_escape_underscores (section, -1);
	...
	escaped_lang_name = scrivi_utils_escape_underscores (lang_name, -1);
...
static void
create_languages_menu (ScriviWindow *window)
{
	...
	languages = scrivi_language_manager_list_languages_sorted (
						scrivi_get_language_manager (),
						FALSE);
...
static void
update_languages_menu (ScriviWindow *window)
{
	ScriviDocument *doc;
	GList *actions;
	GList *l;
	...
	lang = scrivi_document_get_language (doc);
	if (lang != NULL)
		lang_id = gtk_source_language_get_id (lang);
	else
...
void
_scrivi_recent_add (ScriviWindow *window,
		   const gchar *uri,
		   const gchar *mime)
{
	GtkRecentManager *recent_manager;
...
void
_scrivi_recent_remove (ScriviWindow *window,
		      const gchar *uri)
{
	GtkRecentManager *recent_manager;
		  ...
		  ScriviWindow *window)
{
	...
	if (scrivi_commands_load_uris (window, uris, NULL, 0) != 1)
	{
		_scrivi_recent_remove (window, uri);
	}
...
static void	update_recent_files_menu (ScriviWindow *window);
 ...
 */
static void
update_recent_files_menu (ScriviWindow *window)
{
	ScriviWindowPrivate *p = window->priv;
	GtkRecentManager *recent_manager;
	gint max_recents;
	...
	max_recents = scrivi_prefs_manager_get_max_recents ();
	...
	for (l = items; l != NULL; l = l->next)
	{
		...
		if (!gtk_recent_info_has_group (info, "scrivi"))
			continue;
	...
	for (l = filtered_items; l != NULL; l = l->next)
	{
		...
		escaped = scrivi_utils_escape_underscores (display_name, -1);
		if (i >= 10)
		...
		uri = scrivi_utils_uri_for_display (gtk_recent_info_get_uri (info));
		ruri = scrivi_utils_replace_home_dir_with_tilde (uri);
		g_free (uri);
			    ...
			    ScriviWindow *window)
{
	...
	if (scrivi_prefs_manager_toolbar_visible_can_set ())
		scrivi_prefs_manager_set_toolbar_visible (visible);
...
static GtkWidget *
setup_toolbar_open_button (ScriviWindow *window,
			   GtkWidget *toolbar)
{
	GtkRecentManager *recent_manager;
	...
	gtk_recent_chooser_set_limit (GTK_RECENT_CHOOSER (toolbar_recent_menu),
				      scrivi_prefs_manager_get_max_recents ());

	filter = gtk_recent_filter_new ();
	gtk_recent_filter_add_group (filter, "scrivi");
...
static void
create_menu_bar_and_toolbar (ScriviWindow *window, 
			     GtkWidget   *main_box)
{
	...
	action_group = gtk_action_group_new ("ScriviWindowAlwaysSensitiveActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_actions (action_group,
				      scrivi_always_sensitive_menu_entries,
				      G_N_ELEMENTS (scrivi_always_sensitive_menu_entries),
				      window);
	gtk_action_group_add_toggle_actions (action_group,
					     scrivi_always_sensitive_toggle_menu_entries,
					     G_N_ELEMENTS (scrivi_always_sensitive_toggle_menu_entries),
					     window);
	...
	action_group = gtk_action_group_new ("ScriviWindowActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_actions (action_group,
				      scrivi_menu_entries,
				      G_N_ELEMENTS (scrivi_menu_entries),
				      window);
	gtk_ui_manager_insert_action_group (manager, action_group, 0);
	g_object_unref (action_group);
	...
	action_group = gtk_action_group_new ("ScriviQuitWindowActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_actions (action_group,
				      scrivi_quit_menu_entries,
				      G_N_ELEMENTS (scrivi_quit_menu_entries),
				      window);
	...
	action_group = gtk_action_group_new ("ScriviCloseWindowActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_actions (action_group,
	                              scrivi_close_menu_entries,
	                              G_N_ELEMENTS (scrivi_close_menu_entries),
	                              window);
	...
	action_group = gtk_action_group_new ("ScriviWindowPanesActions");
	gtk_action_group_set_translation_domain (action_group, NULL);
	gtk_action_group_add_toggle_actions (action_group,
					     scrivi_panes_toggle_menu_entries,
					     G_N_ELEMENTS (scrivi_panes_toggle_menu_entries),
					     window);
	...
	ui_file = scrivi_dirs_get_ui_file (SCRIVI_UIFILE);
...
static gchar *
get_menu_tip_for_tab (ScriviTab *tab)
{
	ScriviDocument *doc;
	gchar *uri;
	gchar *ruri;
	...
	uri = scrivi_document_get_uri_for_display (doc);
	ruri = scrivi_utils_replace_home_dir_with_tilde (uri);
	g_free (uri);
...
static void
update_documents_list_menu (ScriviWindow *window)
{
	ScriviWindowPrivate *p = window->priv;
	GList *actions, *l;
	gint n, i;
	...
	for (i = 0; i < n; i++)
	{
		...
		tab_name = _scrivi_tab_get_name (SCRIVI_TAB (tab));
		name = scrivi_utils_escape_underscores (tab_name, -1);
		tip =  get_menu_tip_for_tab (SCRIVI_TAB (tab));
		...
		if (SCRIVI_TAB (tab) == p->active_tab)
			gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action), TRUE);
...
static gboolean
set_statusbar_style (ScriviWindow *window,
		     ScriviWindow *origin)
{
	GtkAction *action;
	...
	if (origin == NULL)
		visible = scrivi_prefs_manager_get_statusbar_visible ();
	else
			      ...
			      ScriviWindow *window)
{
	...
	if (scrivi_prefs_manager_statusbar_visible_can_set ())
		scrivi_prefs_manager_set_statusbar_visible (visible);
...
static void
tab_width_combo_changed (ScriviStatusComboBox *combo,
			 GtkMenuItem         *item,
			 ScriviWindow         *window)
{
	ScriviView *view;
...
static void
language_combo_changed (ScriviStatusComboBox *combo,
			GtkMenuItem         *item,
			ScriviWindow         *window)
{
	ScriviDocument *doc;
	...
	scrivi_document_set_language (doc, language);
...
static void
fill_tab_width_combo (ScriviWindow *window)
{
	...
	ScriviStatusComboBox *combo = SCRIVI_STATUS_COMBO_BOX (window->priv->tab_width_combo);
	guint i = 0;
	GtkWidget *item;
	...
	while (defs[i].label != NULL)
	{
		...
		scrivi_status_combo_box_add_item (combo,
						 GTK_MENU_ITEM (item),
						 defs[i].label);
	...
	scrivi_status_combo_box_add_item (combo, GTK_MENU_ITEM (item), NULL);
	gtk_widget_show (item);
...
static void
fill_language_combo (ScriviWindow *window)
{
	...
	manager = scrivi_get_language_manager ();
	languages = scrivi_language_manager_list_languages_sorted (manager, FALSE);
	...
	scrivi_status_combo_box_add_item (SCRIVI_STATUS_COMBO_BOX (window->priv->language_combo),
...
static void
create_statusbar (ScriviWindow *window, 
		  GtkWidget   *main_box)
{
	scrivi_debug (DEBUG_WINDOW);

	window->priv->statusbar = scrivi_statusbar_new ();
	...
	window->priv->tab_width_combo = scrivi_status_combo_box_new (_("Tab Width"));
	...
	window->priv->language_combo = scrivi_status_combo_box_new (NULL);
...
static ScriviWindow *
clone_window (ScriviWindow *origin)
{
	ScriviWindow *window;
	GdkScreen *screen;
	ScriviApp  *app;
	...
	window = scrivi_app_create_window (app, screen);
	...
	panel_page = _scrivi_panel_get_active_item_id (SCRIVI_PANEL (origin->priv->side_panel));
	_scrivi_panel_set_active_item_by_id (SCRIVI_PANEL (window->priv->side_panel),
					    panel_page);

	panel_page = _scrivi_panel_get_active_item_id (SCRIVI_PANEL (origin->priv->bottom_panel));
	_scrivi_panel_set_active_item_by_id (SCRIVI_PANEL (window->priv->bottom_panel),
					    panel_page);
				  ...
				  ScriviWindow   *window)
{
	...
	scrivi_debug (DEBUG_WINDOW);
 	...
 	if (buffer != GTK_TEXT_BUFFER (scrivi_window_get_active_document (window)))
 		return;
	...
	scrivi_statusbar_set_cursor_position (
				SCRIVI_STATUSBAR (window->priv->statusbar),
				row + 1,
				col + 1);
}
...
update_overwrite_mode_statusbar (GtkTextView *view, 
				 ScriviWindow *window)
{
	if (view != GTK_TEXT_VIEW (scrivi_window_get_active_view (window)))
		return;
	...
	/* Note that we have to use !gtk_text_view_get_overwrite since we
	   ...
	   G_SIGNAL_RUN_LAST
	*/
	scrivi_statusbar_set_overwrite (
			SCRIVI_STATUSBAR (window->priv->statusbar),
...
static void 
set_title (ScriviWindow *window)
{
	ScriviDocument *doc = NULL;
	gchar *name;
	gchar *dirname = NULL;
	gchar *title = NULL;
	gint len;
	...
	if (window->priv->active_tab == NULL)
	{
		gtk_window_set_title (GTK_WINDOW (window), "Scrivi");
		return;
	}

	doc = scrivi_tab_get_document (window->priv->active_tab);
	g_return_if_fail (doc != NULL);

	name = scrivi_document_get_short_name_for_display (doc);
	...
	if (len > MAX_TITLE_LENGTH)
	{
		...
		tmp = scrivi_utils_str_middle_truncate (name,
						       MAX_TITLE_LENGTH);
		g_free (name);
		name = tmp;
	}
	else
	{
		...
		if (file != NULL)
		{
			...
			str = scrivi_utils_location_get_dirname_for_display (file);
			g_object_unref (file);
			 ...
			 */
			dirname = scrivi_utils_str_middle_truncate (str, 
	...
	if (scrivi_document_get_readonly (doc)) 
	{
		if (dirname != NULL)
			title = g_strdup_printf ("%s [%s] (%s) - Scrivi",
						 name, 
						 _("Read-Only"), 
						 dirname);
		else
			title = g_strdup_printf ("%s [%s] - Scrivi",
						 name, 
						 _("Read-Only"));
	} 
	else 
	{
		if (dirname != NULL)
			...
			title = g_strdup_printf ("%s - Scrivi",
						 name);
	}
...
static void
set_tab_width_item_blocked (ScriviWindow *window,
			    GtkMenuItem *item)
{
	...
	scrivi_status_combo_box_set_item (SCRIVI_STATUS_COMBO_BOX (window->priv->tab_width_combo),
					 item);
...
spaces_instead_of_tabs_changed (GObject     *object,
		 		...
		 		ScriviWindow *window)
{
	ScriviView *view = SCRIVI_VIEW (object);
	gboolean active = gtk_source_view_get_insert_spaces_instead_of_tabs (
			GTK_SOURCE_VIEW (view));
	GList *children = scrivi_status_combo_box_get_items (
			SCRIVI_STATUS_COMBO_BOX (window->priv->tab_width_combo));
		   ...
		   ScriviWindow *window)
{
	...
	items = scrivi_status_combo_box_get_items (combo);
	...
	for (item = items; item; item = item->next)
	{
		...
		if (GTK_IS_SEPARATOR_MENU_ITEM (item->next->data))
		{
			if (!found)
			{
				...
				scrivi_status_combo_box_set_item_text (combo, 
		  ...
		  ScriviWindow *window)
{
	GList *items;
	GList *item;
	ScriviStatusComboBox *combo = SCRIVI_STATUS_COMBO_BOX (window->priv->language_combo);
	...
	for (item = items; item; item = item->next)
	{
		    					       ...
		    					       new_id) == 0))
		{
			...
			scrivi_status_combo_box_set_item (SCRIVI_STATUS_COMBO_BOX (window->priv->language_combo),
					 		 GTK_MENU_ITEM (item->data));
		      ...
		      ScriviWindow     *window)
{
	...
	tab = SCRIVI_TAB (gtk_notebook_get_nth_page (book, page_num));
	...
	if (window->priv->active_tab)
	{
		...
		{
			g_signal_handler_disconnect (scrivi_tab_get_view (window->priv->active_tab), 
	...
	/* sync the statusbar */
	update_cursor_position_statusbar (GTK_TEXT_BUFFER (scrivi_tab_get_document (tab)),
					  window);
	scrivi_statusbar_set_overwrite (SCRIVI_STATUSBAR (window->priv->statusbar),
	...
	window->priv->language_changed_id = g_signal_connect (scrivi_tab_get_document (tab),
							      "notify::language",
	...
	language_changed (G_OBJECT (scrivi_tab_get_document (tab)), NULL, window);
...
static void
set_sensitivity_according_to_window_state (ScriviWindow *window)
{
	GtkAction *action;
	ScriviLockdownMask lockdown;
	...
	gtk_action_group_set_sensitive (window->priv->quit_action_group,
				  !(window->priv->state & SCRIVI_WINDOW_STATE_SAVING) &&
				  !(window->priv->state & SCRIVI_WINDOW_STATE_PRINTING));
	...
	action = gtk_action_group_get_action (window->priv->always_sensitive_action_group,
					      "FileNew");
	gtk_action_set_sensitive (action, 
				  !(window->priv->state & SCRIVI_WINDOW_STATE_SAVING_SESSION));
	...
	scrivi_notebook_set_close_buttons_sensitive (SCRIVI_NOTEBOOK (window->priv->notebook),
						    !(window->priv->state & SCRIVI_WINDOW_STATE_SAVING_SESSION));
						    
	scrivi_notebook_set_tab_drag_and_drop_enabled (SCRIVI_NOTEBOOK (window->priv->notebook),
						      !(window->priv->state & SCRIVI_WINDOW_STATE_SAVING_SESSION));

	if ((window->priv->state & SCRIVI_WINDOW_STATE_SAVING_SESSION) != 0)
	{
...
update_tab_autosave (GtkWidget *widget,
		     gpointer   data)
{
	ScriviTab *tab = SCRIVI_TAB (widget);
	gboolean *enabled = (gboolean *) data;

	scrivi_tab_set_auto_save_enabled (tab, *enabled);
}

void
_scrivi_window_set_lockdown (ScriviWindow       *window,
			    ScriviLockdownMask  lockdown)
{
	ScriviTab *tab;
	GtkAction *action;
	...
	autosave = scrivi_prefs_manager_get_auto_save ();
...
static void
analyze_tab_state (ScriviTab    *tab, 
		   ScriviWindow *window)
{
	ScriviTabState ts;
	...
	switch (ts)
	{
		case SCRIVI_TAB_STATE_LOADING:
		case SCRIVI_TAB_STATE_REVERTING:
			window->priv->state |= SCRIVI_WINDOW_STATE_LOADING;
			break;
		
		case SCRIVI_TAB_STATE_SAVING:
			window->priv->state |= SCRIVI_WINDOW_STATE_SAVING;
			break;
			
		case SCRIVI_TAB_STATE_PRINTING:
		case SCRIVI_TAB_STATE_PRINT_PREVIEWING:
			window->priv->state |= SCRIVI_WINDOW_STATE_PRINTING;
			break;
	
		case SCRIVI_TAB_STATE_LOADING_ERROR:
		case SCRIVI_TAB_STATE_REVERTING_ERROR:
		case SCRIVI_TAB_STATE_SAVING_ERROR:
		case SCRIVI_TAB_STATE_GENERIC_ERROR:
			window->priv->state |= SCRIVI_WINDOW_STATE_ERROR;
...
static void
update_window_state (ScriviWindow *window)
{
	ScriviWindowState old_ws;
	gint old_num_of_errors;
	
	scrivi_debug_message (DEBUG_WINDOW, "Old state: %x", window->priv->state);
	
	old_ws = window->priv->state;
	old_num_of_errors = window->priv->num_tabs_with_error;
	
	window->priv->state = old_ws & SCRIVI_WINDOW_STATE_SAVING_SESSION;
	...
	scrivi_debug_message (DEBUG_WINDOW, "New state: %x", window->priv->state);		
		
	if (old_ws != window->priv->state)
	{
		set_sensitivity_according_to_window_state (window);

		scrivi_statusbar_set_window_state (SCRIVI_STATUSBAR (window->priv->statusbar),
						  window->priv->state,
						  window->priv->num_tabs_with_error);
	   ...
	   ScriviWindow *window)
{
	...
	tab_name = _scrivi_tab_get_name (tab);
	escaped_name = scrivi_utils_escape_underscores (tab_name, -1);
...
get_drop_window (GtkWidget *widget)
{
	...
	g_return_val_if_fail (SCRIVI_IS_WINDOW (target_window), NULL);

	if ((SCRIVI_WINDOW(target_window)->priv->state & SCRIVI_WINDOW_STATE_SAVING_SESSION) != 0)
		return NULL;
	
	return SCRIVI_WINDOW (target_window);
}

static void
load_uris_from_drop (ScriviWindow  *window,
		     gchar       **uri_list)
{
	GSList *uris = NULL;
	gint i;
	...
	scrivi_commands_load_uris (window,
				  uris,
				  NULL,
				  0);
...
/* Handle drops on the ScriviWindow */
static void
		       ...
		       gpointer          data)
{
	...
	{
		uri_list = scrivi_utils_drop_get_uris(selection_data);
		load_uris_from_drop (window, uri_list);
		g_strfreev (uri_list);
	}
}

/* Handle drops on the ScriviView */
static void
...
static void
fullscreen_controls_show (ScriviWindow *window)
{
	GdkScreen *screen;
...
{
	ScriviWindow *window = SCRIVI_WINDOW (data);
	GdkScreen *screen;
	GdkRectangle fs_rect;
	gint x, y;
...
static void
show_hide_fullscreen_toolbar (ScriviWindow *window,
			      gboolean     show,
			      gint         height)
{
...
static void
fullscreen_controls_build (ScriviWindow *window)
{
	ScriviWindowPrivate *priv = window->priv;
	GtkWidget *toolbar;
	GtkWidget *toolbar_recent_menu;
	GtkAction *action;
...
static void
can_search_again (ScriviDocument *doc,
		  GParamSpec    *pspec,
		  ScriviWindow   *window)
{
	gboolean sensitive;
	...
	sensitive = scrivi_document_get_can_search_again (doc);
...
static void
selection_changed (ScriviDocument *doc,
		   GParamSpec    *pspec,
		   ScriviWindow   *window)
{
	ScriviTab *tab;
...
static void
sync_languages_menu (ScriviDocument *doc,
		     GParamSpec    *pspec,
		     ScriviWindow   *window)
{
	update_languages_menu (window);
...
static void
readonly_changed (ScriviDocument *doc,
		  GParamSpec    *pspec,
		  ScriviWindow   *window)
{
...
static void
editable_changed (ScriviView  *view,
                  GParamSpec  *arg1,
                  ScriviWindow *window)
{
	scrivi_plugins_engine_update_plugins_ui (scrivi_plugins_engine_get_default (),
...
static void
update_sensitivity_according_to_open_tabs (ScriviWindow *window)
{
	GtkAction *action;
...
static void
notebook_tab_added (ScriviNotebook *notebook,
		    ScriviTab      *tab,
		    ScriviWindow   *window)
{
	ScriviView *view;
	...
	g_return_if_fail ((window->priv->state & SCRIVI_WINDOW_STATE_SAVING_SESSION) == 0);
...
static void
notebook_tab_removed (ScriviNotebook *notebook,
		      ScriviTab      *tab,
		      ScriviWindow   *window)
{
	ScriviView     *view;
	...
	if (window->priv->tab_width_id && tab == scrivi_window_get_active_tab (window))
	{
		g_signal_handler_disconnect (view, window->priv->tab_width_id);
		window->priv->tab_width_id = 0;
	}
	
	if (window->priv->spaces_instead_of_tabs_id && tab == scrivi_window_get_active_tab (window))
	{
		g_signal_handler_disconnect (view, window->priv->spaces_instead_of_tabs_id);
		window->priv->spaces_instead_of_tabs_id = 0;
	}
	
	if (window->priv->language_changed_id && tab == scrivi_window_get_active_tab (window))
	{
	...
	if (window->priv->num_tabs == 0)
	{
		...
		scrivi_statusbar_clear_overwrite (
				SCRIVI_STATUSBAR (window->priv->statusbar));
...
static void
notebook_tabs_reordered (ScriviNotebook *notebook,
			 ScriviWindow   *window)
{
	update_documents_list_menu (window);
	update_next_prev_doc_sensitivity_per_window (window);
...
static void
notebook_tab_detached (ScriviNotebook *notebook,
		       ScriviTab      *tab,
		       ScriviWindow   *window)
{
	ScriviWindow *new_window;
	
	new_window = clone_window (window);
		
	scrivi_notebook_move_tab (notebook,
				 SCRIVI_NOTEBOOK (_scrivi_window_get_notebook (new_window)),
				 tab, 0);
...
// CHECK do we need this?
	...
	else
	{
		...
		tab = GTK_WIDGET (scrivi_window_get_active_tab (window));
		g_return_val_if_fail (tab != NULL, FALSE);

		tab_label = gtk_notebook_get_tab_label (notebook, tab);

		gtk_menu_popup (GTK_MENU (menu), NULL, NULL,
				scrivi_utils_menu_position_under_widget, tab_label,
			     ...
			     ScriviWindow    *window)
{
	...
	else if (GDK_BUTTON_PRESS == event->type && 2 == event->button)
	{
		ScriviTab *tab;
		tab = scrivi_window_get_active_tab (window);
		notebook_tab_close_request (SCRIVI_NOTEBOOK (notebook), tab, GTK_WINDOW (window));
		return FALSE;
	}
		     ...
		     ScriviWindow *window)
{
	...
	if (SCRIVI_IS_NOTEBOOK (gtk_window_get_focus (GTK_WINDOW (window))))
	{
			 ...
			 ScriviWindow *window)
{
	...
	scrivi_debug_message (DEBUG_WINDOW,
			       ...
			       ScriviWindow *window)
{
	...
	if (scrivi_prefs_manager_side_pane_visible_can_set ())
		scrivi_prefs_manager_set_side_pane_visible (visible);
	...
	if (!visible && window->priv->active_tab != NULL)
		gtk_widget_grab_focus (GTK_WIDGET (
				scrivi_tab_get_view (SCRIVI_TAB (window->priv->active_tab))));
}

static void
create_side_panel (ScriviWindow *window)
{
	GtkWidget *documents_panel;

	scrivi_debug (DEBUG_WINDOW);

	window->priv->side_panel = scrivi_panel_new (GTK_ORIENTATION_VERTICAL);
	...
	documents_panel = scrivi_documents_panel_new (window);
	scrivi_panel_add_item_with_stock_icon (SCRIVI_PANEL (window->priv->side_panel),
					      documents_panel,
					      _("Documents"),
					      GTK_STOCK_FILE);
}
...
static void
bottom_panel_visibility_changed (ScriviPanel  *bottom_panel,
				 ScriviWindow *window)
{
	gboolean visible;
	GtkAction *action;
	...
	if (scrivi_prefs_manager_bottom_panel_visible_can_set ())
		scrivi_prefs_manager_set_bottom_panel_visible (visible);
...
static void
bottom_panel_item_removed (ScriviPanel  *panel,
			   GtkWidget   *item,
			   ScriviWindow *window)
{
	if (scrivi_panel_get_n_items (panel) == 0)
	{
		GtkAction *action;
...
static void
create_bottom_panel (ScriviWindow *window) 
{
	scrivi_debug (DEBUG_WINDOW);

	window->priv->bottom_panel = scrivi_panel_new (GTK_ORIENTATION_HORIZONTAL);
...
static void
init_panels_visibility (ScriviWindow *window)
{
	gint active_page;
	...
	/* side pane */
	active_page = scrivi_prefs_manager_get_side_panel_active_page ();
	_scrivi_panel_set_active_item_by_id (SCRIVI_PANEL (window->priv->side_panel),
					    active_page);

	if (scrivi_prefs_manager_get_side_pane_visible ())
	{
		gtk_widget_show (window->priv->side_panel);
	}

	/* bottom pane, it can be empty */
	if (scrivi_panel_get_n_items (SCRIVI_PANEL (window->priv->bottom_panel)) > 0)
	{
		active_page = scrivi_prefs_manager_get_bottom_panel_active_page ();
		_scrivi_panel_set_active_item_by_id (SCRIVI_PANEL (window->priv->bottom_panel),
						    active_page);

		if (scrivi_prefs_manager_get_bottom_panel_visible ())
		{
			gtk_widget_show (window->priv->bottom_panel);
		}
	}
...
static void
check_window_is_active (ScriviWindow *window,
			GParamSpec *property,
			gpointer useless)
{
...
static void
connect_notebook_signals (ScriviWindow *window,
			  GtkWidget   *notebook)
{
...
static void
add_notebook (ScriviWindow *window,
	      GtkWidget   *notebook)
{
...
static void
scrivi_window_init (ScriviWindow *window)
{
	GtkWidget *main_box;
	GtkTargetList *tl;
	...
	window->priv = SCRIVI_WINDOW_GET_PRIVATE (window);
	window->priv->active_tab = NULL;
	window->priv->num_tabs = 0;
	window->priv->removing_tabs = FALSE;
	window->priv->state = SCRIVI_WINDOW_STATE_NORMAL;
	window->priv->dispose_has_run = FALSE;
	window->priv->fullscreen_controls = NULL;
	window->priv->fullscreen_animation_timeout_id = 0;

	window->priv->message_bus = scrivi_message_bus_new ();
    ...
    gtk_style_context_add_class (context, "scrivi-window");
#endif
	...
	/* Add the main area */
	scrivi_debug_message (DEBUG_WINDOW, "Add main area");
#if GTK_CHECK_VERSION(3, 0, 0)
	...
	scrivi_debug_message (DEBUG_WINDOW, "Create scrivi notebook");
	window->priv->notebook = scrivi_notebook_new ();
	...
	window->priv->side_panel_size = scrivi_prefs_manager_get_side_panel_size ();
	window->priv->bottom_panel_size = scrivi_prefs_manager_get_bottom_panel_size ();
	...
	scrivi_debug_message (DEBUG_WINDOW, "Update plugins ui");
	
	scrivi_plugins_engine_activate_plugins (scrivi_plugins_engine_get_default (),
					        window);
	...
	scrivi_debug_message (DEBUG_WINDOW, "END");
}

/**
 * scrivi_window_get_active_view:
 * @window: a #ScriviWindow
 *
 * Gets the active #ScriviView.
 *
 * Returns: the active #ScriviView
 */
ScriviView *
scrivi_window_get_active_view (ScriviWindow *window)
{
	ScriviView *view;
	...
	view = scrivi_tab_get_view (SCRIVI_TAB (window->priv->active_tab));

	return view;
}

/**
 * scrivi_window_get_active_document:
 * @window: a #ScriviWindow
 *
 * Gets the active #ScriviDocument.
 * 
 * Returns: the active #ScriviDocument
 */
ScriviDocument *
scrivi_window_get_active_document (ScriviWindow *window)
{
	ScriviView *view;
	...
	return SCRIVI_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)));
}

GtkWidget *
_scrivi_window_get_notebook (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);
...
/**
 * scrivi_window_create_tab:
 * @window: a #ScriviWindow
 * @jump_to: %TRUE to set the new #ScriviTab as active
 *
 * Creates a new #ScriviTab and adds the new tab to the #ScriviNotebook.
 * In case @jump_to is %TRUE the #ScriviNotebook switches to that new #ScriviTab.
 *
 * Returns: a new #ScriviTab
 */
ScriviTab *
scrivi_window_create_tab (ScriviWindow *window,
			 gboolean     jump_to)
{
	ScriviTab *tab;
	...
	tab = SCRIVI_TAB (_scrivi_tab_new ());	
	gtk_widget_show (GTK_WIDGET (tab));	

	scrivi_notebook_add_tab (SCRIVI_NOTEBOOK (window->priv->notebook),
				tab,
				-1,
				jump_to);
...
/**
 * scrivi_window_create_tab_from_uri:
 * @window: a #ScriviWindow
 * @uri: the uri of the document
 * @encoding: a #ScriviEncoding
 * @line_pos: the line position to visualize
 * @create: %TRUE to create a new document in case @uri does exist
 * @jump_to: %TRUE to set the new #ScriviTab as active
 *
 * Creates a new #ScriviTab loading the document specified by @uri.
 * In case @jump_to is %TRUE the #ScriviNotebook swithes to that new #ScriviTab.
 ...
 */
ScriviTab *
scrivi_window_create_tab_from_uri (ScriviWindow         *window,
				  const gchar         *uri,
				  ...
				  gboolean             jump_to)
{
	...
	tab = _scrivi_tab_new_from_uri (uri,
				       encoding,
				       line_pos,
				       create);	
	if (tab == NULL)
	...
	return SCRIVI_TAB (tab);
}				  

/**
 * scrivi_window_get_active_tab:
 * @window: a ScriviWindow
 *
 * Gets the active #ScriviTab in the @window.
 *
 * Returns: the active #ScriviTab in the @window.
 */
ScriviTab *
scrivi_window_get_active_tab (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);
	
	return (window->priv->active_tab == NULL) ? 
				NULL : SCRIVI_TAB (window->priv->active_tab);
}

static void
add_document (ScriviTab *tab, GList **res)
{
	ScriviDocument *doc;
...
static void
add_view (ScriviTab *tab, GList **res)
{
	ScriviView *view;
...
/**
 * scrivi_window_get_views:
 * @window: a #ScriviWindow
 *
 * Gets a list with all the views in the window. This list must be freed.
 *
 * Returns: (element-type Scrivi.View) (transfer container): a newly allocated
 * list with all the views in the window
 */
GList *
scrivi_window_get_views (ScriviWindow *window)
{
	GList *res = NULL;
...
/**
 * scrivi_window_close_tab:
 * @window: a #ScriviWindow
 * @tab: the #ScriviTab to close
 *
 * Closes the @tab.
 */
void
scrivi_window_close_tab (ScriviWindow *window,
			ScriviTab    *tab)
{
	g_return_if_fail (SCRIVI_IS_WINDOW (window));
	g_return_if_fail (SCRIVI_IS_TAB (tab));
	g_return_if_fail ((scrivi_tab_get_state (tab) != SCRIVI_TAB_STATE_SAVING) &&
			  (scrivi_tab_get_state (tab) != SCRIVI_TAB_STATE_SHOWING_PRINT_PREVIEW));
	
	scrivi_notebook_remove_tab (SCRIVI_NOTEBOOK (window->priv->notebook),
				   tab);
}

/**
 * scrivi_window_close_all_tabs:
 * @window: a #ScriviWindow
 *
 * Closes all opened tabs.
 */
void
scrivi_window_close_all_tabs (ScriviWindow *window)
{
	g_return_if_fail (SCRIVI_IS_WINDOW (window));
	g_return_if_fail (!(window->priv->state & SCRIVI_WINDOW_STATE_SAVING) &&
			  !(window->priv->state & SCRIVI_WINDOW_STATE_SAVING_SESSION));

	window->priv->removing_tabs = TRUE;

	scrivi_notebook_remove_all_tabs (SCRIVI_NOTEBOOK (window->priv->notebook));

	window->priv->removing_tabs = FALSE;
}

/**
 * scrivi_window_close_tabs:
 * @window: a #ScriviWindow
 * @tabs: (element-type Scrivi.Tab): a list of #ScriviTab
 *
 * Closes all tabs specified by @tabs.
 */
void
scrivi_window_close_tabs (ScriviWindow *window,
			 const GList *tabs)
{
	g_return_if_fail (SCRIVI_IS_WINDOW (window));
	g_return_if_fail (!(window->priv->state & SCRIVI_WINDOW_STATE_SAVING) &&
...
ScriviWindow *
_scrivi_window_move_tab_to_new_window (ScriviWindow *window,
				      ScriviTab    *tab)
{
	ScriviWindow *new_window;

	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);
	g_return_val_if_fail (SCRIVI_IS_TAB (tab), NULL);
	...
	scrivi_notebook_move_tab (SCRIVI_NOTEBOOK (window->priv->notebook),
				 SCRIVI_NOTEBOOK (new_window->priv->notebook),
				 tab,
				 -1);
 ...
 */
void
scrivi_window_set_active_tab (ScriviWindow *window,
			     ScriviTab    *tab)
{
	gint page_num;
 ...
 */
GtkWindowGroup *
scrivi_window_get_group (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);
...
gboolean
_scrivi_window_is_removing_tabs (ScriviWindow *window)
{
 ...
 */
GtkUIManager *
scrivi_window_get_ui_manager (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);
...
/**
 * scrivi_window_get_side_panel:
 * @window: a #ScriviWindow
 *
 * Gets the side #ScriviPanel of the @window.
 *
 * Returns: the side #ScriviPanel.
 */
ScriviPanel *
scrivi_window_get_side_panel (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);

	return SCRIVI_PANEL (window->priv->side_panel);
}

/**
 * scrivi_window_get_bottom_panel:
 * @window: a #ScriviWindow
 *
 * Gets the bottom #ScriviPanel of the @window.
 *
 * Returns: the bottom #ScriviPanel.
 */
ScriviPanel *
scrivi_window_get_bottom_panel (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);

	return SCRIVI_PANEL (window->priv->bottom_panel);
}

/**
 * scrivi_window_get_statusbar:
 * @window: a #ScriviWindow
 *
 * Gets the #ScriviStatusbar of the @window.
 *
 * Returns: the #ScriviStatusbar of the @window.
 */
GtkWidget *
scrivi_window_get_statusbar (ScriviWindow *window)
{
...
/**
 * scrivi_window_get_state:
 * @window: a #ScriviWindow
 *
 * Retrieves the state of the @window.
 *
 * Returns: the current #ScriviWindowState of the @window.
 */
ScriviWindowState
scrivi_window_get_state (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), SCRIVI_WINDOW_STATE_NORMAL);

	return window->priv->state;
}

GFile *
_scrivi_window_get_default_location (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);
...
void
_scrivi_window_set_default_location (ScriviWindow *window,
				    GFile       *location)
{
	GFile *dir;
...
/**
 * scrivi_window_get_unsaved_documents:
 * @window: a #ScriviWindow
 *
 * Gets the list of documents that need to be saved before closing the window.
 *
 * Returns: (element-type Scrivi.Document) (transfer container): a list of
 * #ScriviDocument that need to be saved before closing the window
 */
GList *
scrivi_window_get_unsaved_documents (ScriviWindow *window)
{
...
void 
_scrivi_window_set_saving_session_state (ScriviWindow *window,
					gboolean     saving_session)
{
	ScriviWindowState old_state;
...
void
_scrivi_window_fullscreen (ScriviWindow *window)
{
	g_return_if_fail (SCRIVI_IS_WINDOW (window));
...
void
_scrivi_window_unfullscreen (ScriviWindow *window)
{
	gboolean visible;
	GtkAction *action;
...
gboolean
_scrivi_window_is_fullscreen (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), FALSE);
...
/**
 * scrivi_window_get_tab_from_location:
 * @window: a #ScriviWindow
 * @location: a #GFile
 *
 * Gets the #ScriviTab that matches with the given @location.
 *
 * Returns: the #ScriviTab that matches with the given @location.
 */
ScriviTab *
scrivi_window_get_tab_from_location (ScriviWindow *window,
				    GFile       *location)
{
	GList *tabs;
	GList *l;
	...
	for (l = tabs; l != NULL; l = g_list_next (l))
	{
		...
		t = SCRIVI_TAB (l->data);
		d = scrivi_tab_get_document (t);

		f = scrivi_document_get_location (d);
...
/**
 * scrivi_window_get_message_bus:
 * @window: a #ScriviWindow
 *
 * Gets the #ScriviMessageBus associated with @window. The returned reference
 * is owned by the window and should not be unreffed.
 *
 * Return value: (transfer none): the #ScriviMessageBus associated with @window
 */
ScriviMessageBus	*
scrivi_window_get_message_bus (ScriviWindow *window)
{
	g_return_val_if_fail (SCRIVI_IS_WINDOW (window), NULL);
...
/**
 * scrivi_window_get_tab_from_uri:
 * @window: a #ScriviWindow
 * @uri: the uri to get the #ScriviTab
 *
 * Gets the #ScriviTab that matches @uri.
 *
 * Returns: the #ScriviTab associated with @uri.
 *
 * Deprecated: 2.24: Use scrivi_window_get_tab_from_location() instead.
 */
ScriviTab *
scrivi_window_get_tab_from_uri (ScriviWindow *window,
			       const gchar *uri)
{
	GFile *f;
	ScriviTab *tab;
	...
	tab = scrivi_window_get_tab_from_location (window, f);
	g_object_unref (f);
