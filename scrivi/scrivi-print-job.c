 ...
 *
 * $Id: scrivi-print.c 6022 2007-12-09 14:38:57Z pborelli $
 */
...
#define SCRIVI_PRINT_JOB_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
					    SCRIVI_TYPE_PRINT_JOB, \
					    ScriviPrintJobPrivate))

struct _ScriviPrintJobPrivate
{
	ScriviView                *view;
	ScriviDocument            *doc;
...
G_DEFINE_TYPE (ScriviPrintJob, scrivi_print_job, G_TYPE_OBJECT)

static void
set_view (ScriviPrintJob *job, ScriviView *view)
{
	job->priv->view = view;
	job->priv->doc = SCRIVI_DOCUMENT (gtk_text_view_get_buffer (GTK_TEXT_VIEW (view)));
}

static void 
scrivi_print_job_get_property (GObject    *object,
			      guint       prop_id,
			      GValue     *value,
			      GParamSpec *pspec)
{
	ScriviPrintJob *job = SCRIVI_PRINT_JOB (object);
...
static void 
scrivi_print_job_set_property (GObject      *object,
			      guint         prop_id,
			      const GValue *value,
			      GParamSpec   *pspec)
{
...
static void
scrivi_print_job_finalize (GObject *object)
{
	ScriviPrintJob *job = SCRIVI_PRINT_JOB (object);
	...
	G_OBJECT_CLASS (scrivi_print_job_parent_class)->finalize (object);
}

static void 
scrivi_print_job_class_init (ScriviPrintJobClass *klass)
{
	GObjectClass *object_class;

	object_class = G_OBJECT_CLASS (klass);

	object_class->get_property = scrivi_print_job_get_property;
	object_class->set_property = scrivi_print_job_set_property;
	object_class->finalize = scrivi_print_job_finalize;

	g_object_class_install_property (object_class,
					 PROP_VIEW,
					 g_param_spec_object ("view",
							      "Scrivi View",
							      "Scrivi View to print",
							      SCRIVI_TYPE_VIEW,
							      G_PARAM_READWRITE |
	...
	print_job_signals[PRINTING] =
		g_signal_new ("printing",
			      ...
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviPrintJobClass, printing),
			      NULL, NULL,
	...
	print_job_signals[SHOW_PREVIEW] =
		g_signal_new ("show-preview",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviPrintJobClass, show_preview),
			      NULL, NULL,
	...
	print_job_signals[DONE] =
		g_signal_new ("done",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (ScriviPrintJobClass, done),
			      NULL, NULL,
			      scrivi_marshal_VOID__UINT_POINTER,
			      G_TYPE_NONE,
			      2,
			      G_TYPE_UINT,
			      G_TYPE_POINTER);
			      			      
	g_type_class_add_private (object_class, sizeof (ScriviPrintJobPrivate));
}
				  ...
				  ScriviPrintJob   *job)
{
	if (gtk_toggle_button_get_active (button))
	{
		gtk_widget_set_sensitive (job->priv->line_numbers_hbox, 
					  scrivi_prefs_manager_print_line_numbers_can_set ());
	}
	else
	{
...
{
	if (scrivi_prefs_manager_print_font_body_can_set ())
	{
		gchar *font;

		font = scrivi_prefs_manager_get_default_print_font_body ();
	...
	if (scrivi_prefs_manager_print_font_header_can_set ())
	{
		gchar *font;

		font = scrivi_prefs_manager_get_default_print_font_header ();
	...
	if (scrivi_prefs_manager_print_font_numbers_can_set ())
	{
		gchar *font;

		font = scrivi_prefs_manager_get_default_print_font_numbers ();
			 ...
			 ScriviPrintJob     *job)
{
	...
	file = scrivi_dirs_get_ui_file ("scrivi-print-preferences.ui");
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (job->priv->syntax_checkbutton),
				      scrivi_prefs_manager_get_print_syntax_hl ());
	gtk_widget_set_sensitive (job->priv->syntax_checkbutton,
				  scrivi_prefs_manager_print_syntax_hl_can_set ());

	/* Print page headers */
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (job->priv->page_header_checkbutton),
				      scrivi_prefs_manager_get_print_header ());
	gtk_widget_set_sensitive (job->priv->page_header_checkbutton,
				  scrivi_prefs_manager_print_header_can_set ());

	/* Line numbers */
	line_numbers =  scrivi_prefs_manager_get_print_line_numbers ();
	can_set = scrivi_prefs_manager_print_line_numbers_can_set ();
	...
	/* Text wrapping */
	wrap_mode = scrivi_prefs_manager_get_print_wrap_mode ();
	...
	can_set = scrivi_prefs_manager_print_wrap_mode_can_set ();
...
custom_widget_apply_cb (GtkPrintOperation *operation,
			...
			ScriviPrintJob     *job)
{
	scrivi_prefs_manager_set_print_syntax_hl (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->syntax_checkbutton)));

	scrivi_prefs_manager_set_print_header (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->page_header_checkbutton)));

	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->line_numbers_checkbutton)))
	{
		scrivi_prefs_manager_set_print_line_numbers (
			MAX (1, gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (job->priv->line_numbers_spinbutton))));
	}
	else
	{
		scrivi_prefs_manager_set_print_line_numbers (0);
	}

	if (!gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->text_wrapping_checkbutton)))
	{
		scrivi_prefs_manager_set_print_wrap_mode (GTK_WRAP_NONE);
	}
	else
	{
		if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (job->priv->do_not_split_checkbutton)))
		{
			scrivi_prefs_manager_set_print_wrap_mode (GTK_WRAP_WORD);
		}
		else
		{
			scrivi_prefs_manager_set_print_wrap_mode (GTK_WRAP_CHAR);
		}	
	}

	scrivi_prefs_manager_set_print_font_body (gtk_font_button_get_font_name (GTK_FONT_BUTTON (job->priv->body_fontbutton)));
	scrivi_prefs_manager_set_print_font_header (gtk_font_button_get_font_name (GTK_FONT_BUTTON (job->priv->headers_fontbutton)));
	scrivi_prefs_manager_set_print_font_numbers (gtk_font_button_get_font_name (GTK_FONT_BUTTON (job->priv->numbers_fontbutton)));
}

static void
create_compositor (ScriviPrintJob *job)
{
	...
	print_font_body = scrivi_prefs_manager_get_print_font_body ();
	print_font_header = scrivi_prefs_manager_get_print_font_header ();
	print_font_numbers = scrivi_prefs_manager_get_print_font_numbers ();
	...
	job->priv->compositor = GTK_SOURCE_PRINT_COMPOSITOR (
					...
					g_object_new (GTK_TYPE_SOURCE_PRINT_COMPOSITOR,
						     ...
						     "highlight-syntax", gtk_source_buffer_get_highlight_syntax (GTK_SOURCE_BUFFER (job->priv->doc)) &&
					   				 scrivi_prefs_manager_get_print_syntax_hl (),
						     "wrap-mode", scrivi_prefs_manager_get_print_wrap_mode (),
						     "print-line-numbers", scrivi_prefs_manager_get_print_line_numbers (),
						     "print-header", scrivi_prefs_manager_get_print_header (),
						     "print-footer", FALSE,
	...
	if (scrivi_prefs_manager_get_print_header ())
	{
		gchar *doc_name;
		gchar *name_to_display;
		gchar *left;

		doc_name = scrivi_document_get_uri_for_display (job->priv->doc);
		name_to_display = scrivi_utils_str_middle_truncate (doc_name, 60);
	        ...
	        ScriviPrintJob     *job)
{
	...
	job->priv->status = SCRIVI_PRINT_JOB_STATUS_PAGINATING;
...
preview_cb (GtkPrintOperation        *op,
	    ...
	    ScriviPrintJob            *job)
{
	job->priv->preview = scrivi_print_preview_new (op, gtk_preview, context);
	      ...
	      ScriviPrintJob     *job)
{
	...
	if (!job->priv->is_preview)
	{
		...
		job->priv->status = SCRIVI_PRINT_JOB_STATUS_DRAWING;
...
{
	GError *error = NULL;
	ScriviPrintJobResult print_result;

	switch (result) 
	{
		case GTK_PRINT_OPERATION_RESULT_CANCEL:
			print_result = SCRIVI_PRINT_JOB_RESULT_CANCEL;
			break;

		case GTK_PRINT_OPERATION_RESULT_APPLY:
			print_result = SCRIVI_PRINT_JOB_RESULT_OK;
			break;

		case GTK_PRINT_OPERATION_RESULT_ERROR:
			print_result = SCRIVI_PRINT_JOB_RESULT_ERROR;
...
/* Note that scrivi_print_job_print can can only be called once on a given ScriviPrintJob */
GtkPrintOperationResult	 
scrivi_print_job_print (ScriviPrintJob            *job,
		       ...
		       GError                  **error)
{
	...
	job_name = scrivi_document_get_short_name_for_display (priv->doc);
...
static void
scrivi_print_job_init (ScriviPrintJob *job)
{
	job->priv = SCRIVI_PRINT_JOB_GET_PRIVATE (job);
	
	job->priv->status = SCRIVI_PRINT_JOB_STATUS_INIT;
	
	job->priv->status_string = g_strdup (_("Preparing..."));
}

ScriviPrintJob *
scrivi_print_job_new (ScriviView *view)
{
	ScriviPrintJob *job;
	
	g_return_val_if_fail (SCRIVI_IS_VIEW (view), NULL);
	
	job = SCRIVI_PRINT_JOB (g_object_new (SCRIVI_TYPE_PRINT_JOB,
					     "view", view,
					      NULL));
...
void
scrivi_print_job_cancel (ScriviPrintJob *job)
{
	g_return_if_fail (SCRIVI_IS_PRINT_JOB (job));

	gtk_print_operation_cancel (job->priv->operation);
}

const gchar *
scrivi_print_job_get_status_string (ScriviPrintJob *job)
{
	g_return_val_if_fail (SCRIVI_IS_PRINT_JOB (job), NULL);
...
gdouble
scrivi_print_job_get_progress (ScriviPrintJob *job)
{
	g_return_val_if_fail (SCRIVI_IS_PRINT_JOB (job), 0.0);

	return job->priv->progress;
}

GtkPrintSettings *
scrivi_print_job_get_print_settings (ScriviPrintJob *job)
{
	g_return_val_if_fail (SCRIVI_IS_PRINT_JOB (job), NULL);
...
GtkPageSetup *
scrivi_print_job_get_page_setup (ScriviPrintJob *job)
{
	g_return_val_if_fail (SCRIVI_IS_PRINT_JOB (job), NULL);
