...
#define SCRIVI_PLUGIN_MANAGER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_PLUGIN_MANAGER, ScriviPluginManagerPrivate))

struct _ScriviPluginManagerPrivate
{
	GtkWidget	*tree;
...
G_DEFINE_TYPE(ScriviPluginManager, scrivi_plugin_manager, GTK_TYPE_BOX)
#else
G_DEFINE_TYPE(ScriviPluginManager, scrivi_plugin_manager, GTK_TYPE_VBOX)
#endif

static ScriviPluginInfo *plugin_manager_get_selected_plugin (ScriviPluginManager *pm); 
static void plugin_manager_toggle_active (ScriviPluginManager *pm, GtkTreeIter *iter, GtkTreeModel *model);
static void scrivi_plugin_manager_finalize (GObject *object);

static void 
scrivi_plugin_manager_class_init (ScriviPluginManagerClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = scrivi_plugin_manager_finalize;

	g_type_class_add_private (object_class, sizeof (ScriviPluginManagerPrivate));
}

static void
about_button_cb (GtkWidget          *button,
		 ScriviPluginManager *pm)
{
	...
	pm->priv->about = g_object_new (GTK_TYPE_ABOUT_DIALOG,
		"program-name", scrivi_plugin_info_get_name (info),
		"copyright", scrivi_plugin_info_get_copyright (info),
		"authors", scrivi_plugin_info_get_authors (info),
		"comments", scrivi_plugin_info_get_description (info),
		"website", scrivi_plugin_info_get_website (info),
		"logo-icon-name", scrivi_plugin_info_get_icon_name (info),
		"version", scrivi_plugin_info_get_version (info),
		NULL);
		     ...
		     ScriviPluginManager *pm)
{
	...
	scrivi_debug_message (DEBUG_PLUGINS, "Configuring: %s\n", 
			     scrivi_plugin_info_get_name (info));

	toplevel = GTK_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET(pm)));

	scrivi_plugins_engine_configure_plugin (pm->priv->engine,
					       info, toplevel);

	scrivi_debug_message (DEBUG_PLUGINS, "Done");	
}
				  ...
				  gpointer           data)
{
	...
	text = g_markup_printf_escaped ("<b>%s</b>\n%s",
					scrivi_plugin_info_get_name (info),
					scrivi_plugin_info_get_description (info));
	g_object_set (G_OBJECT (cell),
		      "markup", text,
		      "sensitive", scrivi_plugin_info_is_available (info),
		      NULL);
				  ...
				  gpointer           data)
{
	...
	g_object_set (G_OBJECT (cell),
		      "icon-name", scrivi_plugin_info_get_icon_name (info),
		      "sensitive", scrivi_plugin_info_is_available (info),
		      NULL);
}
		   ...
		   gpointer     data)
{
	...
	gtk_widget_set_sensitive (GTK_WIDGET (pm->priv->configure_button),
				  (info != NULL) && 
				   scrivi_plugin_info_is_configurable (info));
}
...
static void
plugin_manager_populate_lists (ScriviPluginManager *pm)
{
	const GList *plugins;
	GtkListStore *model;
	GtkTreeIter iter;
	...
	plugins = scrivi_plugins_engine_get_plugin_list (pm->priv->engine);
	...
	{
		ScriviPluginInfo *info;
		info = (ScriviPluginInfo *)plugins->data;

		gtk_list_store_append (model, &iter);
		gtk_list_store_set (model, &iter,
				    ACTIVE_COLUMN, scrivi_plugin_info_is_active (info),
				    AVAILABLE_COLUMN, scrivi_plugin_info_is_available (info),
				    INFO_COLUMN, info,
				    -1);
...
static gboolean
plugin_manager_set_active (ScriviPluginManager *pm,
			   GtkTreeIter        *iter,
			   GtkTreeModel       *model,
			   gboolean            active)
{
	...
	{
		/* activate the plugin */
		if (!scrivi_plugins_engine_activate_plugin (pm->priv->engine, info)) {
			scrivi_debug_message (DEBUG_PLUGINS, "Could not activate %s.\n", 
					     scrivi_plugin_info_get_name (info));
	...
	else
	{
		...
		if (!scrivi_plugins_engine_deactivate_plugin (pm->priv->engine, info)) {
			scrivi_debug_message (DEBUG_PLUGINS, "Could not deactivate %s.\n", 
					     scrivi_plugin_info_get_name (info));
...
static void
plugin_manager_toggle_active (ScriviPluginManager *pm,
			      GtkTreeIter        *iter,
			      GtkTreeModel       *model)
{
	gboolean active;
...
plugin_manager_get_selected_plugin (ScriviPluginManager *pm)
{
	ScriviPluginInfo *info = NULL;
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
...
static void
plugin_manager_set_active_all (ScriviPluginManager *pm,
			       gboolean            active)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
		...
		gpointer      data)
{
	...
	normalized_string = g_utf8_normalize (scrivi_plugin_info_get_name (info), -1, G_NORMALIZE_ALL);
...
static GtkWidget *
create_tree_popup_menu (ScriviPluginManager *pm)
{
	GtkWidget *menu;
	GtkWidget *item;
	GtkWidget *image;
	...
	gtk_widget_set_sensitive (item, scrivi_plugin_info_is_configurable (info));
	gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);

	item = gtk_check_menu_item_new_with_mnemonic (_("A_ctivate"));
	gtk_widget_set_sensitive (item, scrivi_plugin_info_is_available (info));
...
static void
tree_popup_menu_detach (ScriviPluginManager *pm,
			GtkMenu            *menu)
{
		      ...
		      GdkEventButton     *event)
{
	...
	{
		gtk_menu_popup (GTK_MENU (pm->priv->popup_menu), NULL, NULL,
				scrivi_utils_menu_position_under_tree_view, tree,
...
model_name_sort_func (GtkTreeModel *model,
		      ...
		      gpointer      user_data)
{
	ScriviPluginInfo *info1, *info2;
	
	gtk_tree_model_get (model, iter1, INFO_COLUMN, &info1, -1);
	gtk_tree_model_get (model, iter2, INFO_COLUMN, &info2, -1);

	return g_utf8_collate (scrivi_plugin_info_get_name (info1),
			       scrivi_plugin_info_get_name (info2));
}

static void
plugin_manager_construct_tree (ScriviPluginManager *pm)
{
	GtkTreeViewColumn *column;
	GtkCellRenderer *cell;
	GtkListStore *model;
...
static void
plugin_toggled_cb (ScriviPluginsEngine *engine,
		   ScriviPluginInfo    *info,
		   ScriviPluginManager *pm)
{
	GtkTreeSelection *selection;
	...
	if (!info_found)
	{
		g_warning ("ScriviPluginManager: plugin '%s' not found in the tree model",
			   scrivi_plugin_info_get_name (info));
		return;
	}

	gtk_list_store_set (GTK_LIST_STORE (model), &iter, ACTIVE_COLUMN, scrivi_plugin_info_is_active (info), -1);
}

static void 
scrivi_plugin_manager_init (ScriviPluginManager *pm)
{
	GtkWidget *label;
	GtkWidget *viewport;
	...
	pm->priv = SCRIVI_PLUGIN_MANAGER_GET_PRIVATE (pm);
	 ...
	 */
	scrivi_plugins_engine_rescan_plugins (scrivi_plugins_engine_get_default ());
	...
	pm->priv->about_button = scrivi_gtk_button_new_with_stock_icon (_("_About Plugin"),
								       GTK_STOCK_ABOUT);
	gtk_container_add (GTK_CONTAINER (hbuttonbox), pm->priv->about_button);

	pm->priv->configure_button = scrivi_gtk_button_new_with_stock_icon (_("C_onfigure Plugin"),
	...
	pm->priv->engine = scrivi_plugins_engine_get_default ();
	...
	if (scrivi_plugins_engine_get_plugin_list (pm->priv->engine) != NULL)
	{
...
static void
scrivi_plugin_manager_finalize (GObject *object)
{
	ScriviPluginManager *pm = SCRIVI_PLUGIN_MANAGER (object);
	...
	G_OBJECT_CLASS (scrivi_plugin_manager_parent_class)->finalize (object);

}

GtkWidget *scrivi_plugin_manager_new (void)
{
	return g_object_new (SCRIVI_TYPE_PLUGIN_MANAGER,0);
}
