...
G_DEFINE_TYPE (ScriviDocumentInputStream, scrivi_document_input_stream, G_TYPE_INPUT_STREAM);

struct _ScriviDocumentInputStreamPrivate
{
...
static gssize     scrivi_document_input_stream_read     (GInputStream      *stream,
							void              *buffer,
							gsize              count,
							GCancellable      *cancellable,
							GError           **error);
static gboolean   scrivi_document_input_stream_close    (GInputStream      *stream,
							GCancellable      *cancellable,
							GError           **error);

static void
scrivi_document_input_stream_set_property (GObject      *object,
					  guint         prop_id,
					  const GValue *value,
					  GParamSpec   *pspec)
{
	ScriviDocumentInputStream *stream = SCRIVI_DOCUMENT_INPUT_STREAM (object);
...
static void
scrivi_document_input_stream_get_property (GObject    *object,
					  guint       prop_id,
					  GValue     *value,
					  GParamSpec *pspec)
{
...
static void
scrivi_document_input_stream_class_init (ScriviDocumentInputStreamClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
	GInputStreamClass *stream_class = G_INPUT_STREAM_CLASS (klass);

	g_type_class_add_private (klass, sizeof (ScriviDocumentInputStreamPrivate));

	gobject_class->get_property = scrivi_document_input_stream_get_property;
	gobject_class->set_property = scrivi_document_input_stream_set_property;

	stream_class->read_fn = scrivi_document_input_stream_read;
	stream_class->close_fn = scrivi_document_input_stream_close;
	...
	/**
	 * ScriviDocumentInputStream:newline-type:
	 *
...
static void
scrivi_document_input_stream_init (ScriviDocumentInputStream *stream)
{
	stream->priv = G_TYPE_INSTANCE_GET_PRIVATE (stream,
						    SCRIVI_TYPE_DOCUMENT_INPUT_STREAM,
						    ScriviDocumentInputStreamPrivate);
}

static gsize
get_new_line_size (ScriviDocumentInputStream *stream)
{
	gsize ret;

	switch (stream->priv->newline_type)
	{
		case SCRIVI_DOCUMENT_NEWLINE_TYPE_CR:
		case SCRIVI_DOCUMENT_NEWLINE_TYPE_LF:
			ret = 1;
			break;

		case SCRIVI_DOCUMENT_NEWLINE_TYPE_CR_LF:
			ret = 2;
			break;
...
/**
 * scrivi_document_input_stream_new:
 ...
 */
GInputStream *
scrivi_document_input_stream_new (GtkTextBuffer           *buffer,
				 ScriviDocumentNewlineType type)
{
	ScriviDocumentInputStream *stream;

	g_return_val_if_fail (GTK_IS_TEXT_BUFFER (buffer), NULL);

	stream = g_object_new (SCRIVI_TYPE_DOCUMENT_INPUT_STREAM,
			       "buffer", buffer,
			       "newline-type", type,
			       NULL);
...
gsize
scrivi_document_input_stream_get_total_size (ScriviDocumentInputStream *stream)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT_INPUT_STREAM (stream), 0);

	return gtk_text_buffer_get_char_count (stream->priv->buffer);
}

gsize
scrivi_document_input_stream_tell (ScriviDocumentInputStream *stream)
{
	g_return_val_if_fail (SCRIVI_IS_DOCUMENT_INPUT_STREAM (stream), 0);
...
static const gchar *
get_new_line (ScriviDocumentInputStream *stream)
{
	const gchar *ret;
...
static gsize
read_line (ScriviDocumentInputStream *stream,
	   gchar                    *outbuf,
	   gsize                     space_left)
{
...
static gssize
scrivi_document_input_stream_read (GInputStream  *stream,
				  void          *buffer,
				  gsize          count,
				  GCancellable  *cancellable,
				  GError       **error)
{
	...
	dstream = SCRIVI_DOCUMENT_INPUT_STREAM (stream);
...
static gboolean
scrivi_document_input_stream_close (GInputStream  *stream,
				   GCancellable  *cancellable,
				   GError       **error)
{
	ScriviDocumentInputStream *dstream = SCRIVI_DOCUMENT_INPUT_STREAM (stream);
