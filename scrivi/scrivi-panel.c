...
#include "scrivi-close-button.h"
#include "scrivi-window.h"
#include "scrivi-debug.h"

#define PANEL_ITEM_KEY "ScriviPanelItemKey"

#define SCRIVI_PANEL_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), SCRIVI_TYPE_PANEL, ScriviPanelPrivate))
...
typedef struct _ScriviPanelItem ScriviPanelItem;

struct _ScriviPanelItem 
{
	gchar *name;
	GtkWidget *icon;
};
...
static GObject	*scrivi_panel_constructor	(GType type,
						 guint n_construct_properties,
						 GObjectConstructParam *construct_properties);


#if GTK_CHECK_VERSION (3, 0, 0)
G_DEFINE_TYPE(ScriviPanel, scrivi_panel, GTK_TYPE_BOX)
#else
G_DEFINE_TYPE(ScriviPanel, scrivi_panel, GTK_TYPE_VBOX)
#endif

static void
scrivi_panel_finalize (GObject *obj)
{
	if (G_OBJECT_CLASS (scrivi_panel_parent_class)->finalize)
		(*G_OBJECT_CLASS (scrivi_panel_parent_class)->finalize) (obj);
}

static void
scrivi_panel_get_property (GObject    *object,
			  guint       prop_id,
			  GValue     *value,
			  GParamSpec *pspec)
{
	ScriviPanel *panel = SCRIVI_PANEL (object);
...
static void
scrivi_panel_set_property (GObject      *object,
			  guint         prop_id,
			  const GValue *value,
			  GParamSpec   *pspec)
{
...
static void
scrivi_panel_close (ScriviPanel *panel)
{
	gtk_widget_hide (GTK_WIDGET (panel));
}

static void
scrivi_panel_focus_document (ScriviPanel *panel)
{
	GtkWidget *toplevel = gtk_widget_get_toplevel (GTK_WIDGET (panel));
	if (gtk_widget_is_toplevel (toplevel) && SCRIVI_IS_WINDOW (toplevel))
	{
		ScriviView *view;

		view = scrivi_window_get_active_view (SCRIVI_WINDOW (toplevel));
		if (view != NULL)
...
static void
scrivi_panel_grab_focus (GtkWidget *w)
{
	gint n;
	GtkWidget *tab;
	ScriviPanel *panel = SCRIVI_PANEL (w);
...
static void
scrivi_panel_class_init (ScriviPanelClass *klass)
{
	GtkBindingSet *binding_set;
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

	g_type_class_add_private (klass, sizeof (ScriviPanelPrivate));

	object_class->constructor = scrivi_panel_constructor;
	object_class->finalize = scrivi_panel_finalize;
	object_class->get_property = scrivi_panel_get_property;
	object_class->set_property = scrivi_panel_set_property;
	...
	widget_class->grab_focus = scrivi_panel_grab_focus;

	klass->close = scrivi_panel_close;
	klass->focus_document = scrivi_panel_focus_document;

	signals[ITEM_ADDED] =
		g_signal_new ("item_added",
			      G_OBJECT_CLASS_TYPE (klass),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviPanelClass, item_added),
			      NULL, NULL,
	...
	signals[ITEM_REMOVED] =
		g_signal_new ("item_removed",
			      G_OBJECT_CLASS_TYPE (klass),
			      G_SIGNAL_RUN_FIRST,
			      G_STRUCT_OFFSET (ScriviPanelClass, item_removed),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__OBJECT,
			      G_TYPE_NONE,
			      1,
	...
	signals[CLOSE] =
		g_signal_new ("close",
			      ...
			      G_STRUCT_OFFSET (ScriviPanelClass, close),
		  	      NULL, NULL,
	...
	signals[FOCUS_DOCUMENT] =
		g_signal_new ("focus_document",
			      G_OBJECT_CLASS_TYPE (klass),
			      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			      G_STRUCT_OFFSET (ScriviPanelClass, focus_document),
		  	      NULL, NULL,
		  	      g_cclosure_marshal_VOID__VOID,
			      G_TYPE_NONE, 0);					
...
static void
sync_title (ScriviPanel     *panel,
	    ScriviPanelItem *item)
{
...
{
	GtkWidget *item;
	ScriviPanelItem *data;

	item = gtk_notebook_get_nth_page (notebook, page_num);
	g_return_if_fail (item != NULL);

	data = (ScriviPanelItem *)g_object_get_data (G_OBJECT (item),
						    PANEL_ITEM_KEY);
...
static void
scrivi_panel_init (ScriviPanel *panel)
{
	panel->priv = SCRIVI_PANEL_GET_PRIVATE (panel);
...
static GtkWidget *
create_close_button (ScriviPanel *panel)
{
	GtkWidget *button;

	button = scrivi_close_button_new ();
...
static void
build_notebook_for_panel (ScriviPanel *panel)
{
...
static void
build_horizontal_panel (ScriviPanel *panel)
{
	GtkWidget *box;
	GtkWidget *sidebar;
...
static void
build_vertical_panel (ScriviPanel *panel)
{
	GtkWidget *close_button;
...
static GObject *
scrivi_panel_constructor (GType type,
			 guint n_construct_properties,
			 GObjectConstructParam *construct_properties)
{
	
	/* Invoke parent constructor. */
	ScriviPanelClass *klass = SCRIVI_PANEL_CLASS (g_type_class_peek (SCRIVI_TYPE_PANEL));
	...
	ScriviPanel *panel = SCRIVI_PANEL (obj);
 ...
 *
 * Creates a new #ScriviPanel with the given @orientation. You shouldn't create
 * a new panel use scrivi_window_get_side_panel() or scrivi_window_get_bottom_panel()
 * instead.
 *
 * Returns: a new #ScriviPanel object.
 */
GtkWidget *
scrivi_panel_new (GtkOrientation orientation)
{
	return GTK_WIDGET (g_object_new (SCRIVI_TYPE_PANEL, "orientation", orientation, NULL));
}

static GtkWidget *
build_tab_label (ScriviPanel  *panel,
		 GtkWidget   *item,
		 const gchar *name,
		 GtkWidget   *icon)
{
 ...
 */
void
scrivi_panel_add_item (ScriviPanel  *panel, 
		      GtkWidget   *item, 
		      const gchar *name,
		      GtkWidget   *image)
{
...
/**
 * scrivi_panel_add_item_with_stock_icon:
 * @panel: a #ScriviPanel
 * @item: the #GtkWidget to add to the @panel
 * @name: the name to be shown in the @panel
 * @stock_id: a stock id
 *
 * Same as scrivi_panel_add_item() but using an image from stock.
 */
void
scrivi_panel_add_item_with_stock_icon (ScriviPanel  *panel, 
				      GtkWidget   *item, 
				      const gchar *name,
				      const gchar *stock_id)
{
	...
	scrivi_panel_add_item (panel, item, name, icon);
}

/**
 * scrivi_panel_remove_item:
 * @panel: a #ScriviPanel
 ...
 */
gboolean
scrivi_panel_remove_item (ScriviPanel *panel,
			 GtkWidget  *item)
{
	ScriviPanelItem *data;
	gint page_num;
	
	g_return_val_if_fail (SCRIVI_IS_PANEL (panel), FALSE);
 ...
 */
gboolean
scrivi_panel_activate_item (ScriviPanel *panel,
			   GtkWidget  *item)
{
	gint page_num;
 ...
 */
gboolean
scrivi_panel_item_is_active (ScriviPanel *panel,
			    GtkWidget  *item)
{
	gint cur_page;
	gint page_num;
...
/**
 * scrivi_panel_get_orientation:
 * @panel: a #ScriviPanel
 *
 * Gets the orientation of the @panel. 
 *
 * Returns: the #GtkOrientation of #ScriviPanel
 */
GtkOrientation
scrivi_panel_get_orientation (ScriviPanel *panel)
{
	g_return_val_if_fail (SCRIVI_IS_PANEL (panel), GTK_ORIENTATION_VERTICAL);

	return panel->priv->orientation;
}

/**
 * scrivi_panel_get_n_items:
 * @panel: a #ScriviPanel
 *
 * Gets the number of items in a @panel.
 * 
 * Returns: the number of items contained in #ScriviPanel
 */
gint
scrivi_panel_get_n_items (ScriviPanel *panel)
{
	g_return_val_if_fail (SCRIVI_IS_PANEL (panel), -1);

	return gtk_notebook_get_n_pages (GTK_NOTEBOOK (panel->priv->notebook));
}

gint
_scrivi_panel_get_active_item_id (ScriviPanel *panel)
{
	gint cur_page;
	GtkWidget *item;
	ScriviPanelItem *data;

	g_return_val_if_fail (SCRIVI_IS_PANEL (panel), 0);
...
void
_scrivi_panel_set_active_item_by_id (ScriviPanel *panel,
				    gint        id)
{
	gint n, i;
