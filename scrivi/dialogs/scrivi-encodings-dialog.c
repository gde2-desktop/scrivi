...
#include "scrivi-encodings-dialog.h"
...
#define SCRIVI_ENCODINGS_DIALOG_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						   SCRIVI_TYPE_ENCODINGS_DIALOG,           \
						   ScriviEncodingsDialogPrivate))

struct _ScriviEncodingsDialogPrivate
{
...
G_DEFINE_TYPE(ScriviEncodingsDialog, scrivi_encodings_dialog, GTK_TYPE_DIALOG)

static void
scrivi_encodings_dialog_finalize (GObject *object)
{
	ScriviEncodingsDialogPrivate *priv = SCRIVI_ENCODINGS_DIALOG (object)->priv;

	g_slist_free (priv->show_in_menu_list);

	G_OBJECT_CLASS (scrivi_encodings_dialog_parent_class)->finalize (object);
}

static void
scrivi_encodings_dialog_class_init (ScriviEncodingsDialogClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = scrivi_encodings_dialog_finalize;

	g_type_class_add_private (object_class, sizeof (ScriviEncodingsDialogPrivate));
}
				 ...
				 GSList       *list)
{
	...
	while (list != NULL)
	{
		...
		enc = (const ScriviEncoding*) list->data;
		...
		gtk_list_store_set (store, &iter,
				    ...
				    COLUMN_NAME,
				    scrivi_encoding_get_name (enc), -1);
...
static void
init_shown_in_menu_tree_model (ScriviEncodingsDialog *dialog)
{
	GtkTreeIter iter;
	GSList *list, *tmp;

	/* add data to the list store */
	list = scrivi_prefs_manager_get_shown_in_menu_encodings ();
	...
	while (tmp != NULL)
	{
		...
		enc = (const ScriviEncoding *) tmp->data;
...
response_handler (GtkDialog            *dialog,
		  gint                  response_id,
                  ScriviEncodingsDialog *dlg)
{
	if (response_id == GTK_RESPONSE_HELP)
	{
		scrivi_help_display (GTK_WINDOW (dialog), "scrivi", NULL);
	...
	if (response_id == GTK_RESPONSE_OK)
	{
		...
		scrivi_prefs_manager_set_shown_in_menu_encodings (dlg->priv->show_in_menu_list);
	}
}

static void
scrivi_encodings_dialog_init (ScriviEncodingsDialog *dlg)
{
	GtkWidget *content;
	...
	dlg->priv = SCRIVI_ENCODINGS_DIALOG_GET_PRIVATE (dlg);
	...
	file = scrivi_dirs_get_ui_file ("scrivi-encodings-dialog.ui");
...
GtkWidget *
scrivi_encodings_dialog_new (void)
{
	GtkWidget *dlg;

	dlg = GTK_WIDGET (g_object_new (SCRIVI_TYPE_ENCODINGS_DIALOG, NULL));
