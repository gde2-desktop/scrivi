...
#include "scrivi-close-confirmation-dialog.h"
...
struct _ScriviCloseConfirmationDialogPrivate 
{
...
#define SCRIVI_CLOSE_CONFIRMATION_DIALOG_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), \
							SCRIVI_TYPE_CLOSE_CONFIRMATION_DIALOG, \
							ScriviCloseConfirmationDialogPrivate))
...
G_DEFINE_TYPE(ScriviCloseConfirmationDialog, scrivi_close_confirmation_dialog, GTK_TYPE_DIALOG)

static void 	 set_unsaved_document 		(ScriviCloseConfirmationDialog *dlg,
						 const GList                  *list);
 ...
 */
static void
response_cb (ScriviCloseConfirmationDialog *dlg,
             gint                          response_id,
             gpointer                      data)
{
	ScriviCloseConfirmationDialogPrivate *priv;

	g_return_if_fail (SCRIVI_IS_CLOSE_CONFIRMATION_DIALOG (dlg));
...
static void
set_logout_mode (ScriviCloseConfirmationDialog *dlg,
		 gboolean                      logout_mode)
{
	...
	if (logout_mode)
	{
		...
		scrivi_dialog_add_button (GTK_DIALOG (dlg),
					 _("_Cancel Logout"),
					 GTK_STOCK_CANCEL,
					 GTK_RESPONSE_CANCEL);
	}
	...
	else
	{
		...
		if (GET_MODE (dlg->priv) == SINGLE_DOC_MODE)
		{
			...
			doc = SCRIVI_DOCUMENT (dlg->priv->unsaved_documents->data);
...
static void 
scrivi_close_confirmation_dialog_init (ScriviCloseConfirmationDialog *dlg)
{
	AtkObject *atk_obj;

	dlg->priv = SCRIVI_CLOSE_CONFIRMATION_DIALOG_GET_PRIVATE (dlg);

	dlg->priv->disable_save_to_disk = 
			scrivi_app_get_lockdown (scrivi_app_get_default ()) 
...
static void 
scrivi_close_confirmation_dialog_finalize (GObject *object)
{
	ScriviCloseConfirmationDialogPrivate *priv;

	priv = SCRIVI_CLOSE_CONFIRMATION_DIALOG (object)->priv;
	...
	G_OBJECT_CLASS (scrivi_close_confirmation_dialog_parent_class)->finalize (object);
}

static void
scrivi_close_confirmation_dialog_set_property (GObject      *object, 
					      guint         prop_id, 
					      const GValue *value, 
					      GParamSpec   *pspec)
{
	ScriviCloseConfirmationDialog *dlg;

	dlg = SCRIVI_CLOSE_CONFIRMATION_DIALOG (object);
...
static void
scrivi_close_confirmation_dialog_get_property (GObject    *object, 
					      guint       prop_id, 
					      GValue     *value, 
					      GParamSpec *pspec)
{
...
static void 
scrivi_close_confirmation_dialog_class_init (ScriviCloseConfirmationDialogClass *klass)
{
	GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

	gobject_class->set_property = scrivi_close_confirmation_dialog_set_property;
	gobject_class->get_property = scrivi_close_confirmation_dialog_get_property;
	gobject_class->finalize = scrivi_close_confirmation_dialog_finalize;

	g_type_class_add_private (klass, sizeof (ScriviCloseConfirmationDialogPrivate));
...
GList *
scrivi_close_confirmation_dialog_get_selected_documents (ScriviCloseConfirmationDialog *dlg)
{
	g_return_val_if_fail (SCRIVI_IS_CLOSE_CONFIRMATION_DIALOG (dlg), NULL);

	return g_list_copy (dlg->priv->selected_documents);
}

GtkWidget *
scrivi_close_confirmation_dialog_new (GtkWindow *parent, 
				     GList     *unsaved_documents,
				     gboolean   logout_mode)
{
	GtkWidget *dlg;
	...
	dlg = GTK_WIDGET (g_object_new (SCRIVI_TYPE_CLOSE_CONFIRMATION_DIALOG,
	...
	if (parent != NULL)
	{
		gtk_window_group_add_window (scrivi_window_get_group (SCRIVI_WINDOW (parent)),
					     GTK_WINDOW (dlg));
...
GtkWidget *
scrivi_close_confirmation_dialog_new_single (GtkWindow     *parent, 
					    ScriviDocument *doc,
					    gboolean       logout_mode)
{
	GtkWidget *dlg;
	...
	dlg = scrivi_close_confirmation_dialog_new (parent, 
						   unsaved_documents,
						   logout_mode);
...
static gchar *
get_text_secondary_label (ScriviDocument *doc)
{
	glong  seconds;
	gchar *secondary_msg;
...
static void
build_single_doc_dialog (ScriviCloseConfirmationDialog *dlg)
{
	GtkWidget     *hbox;
	GtkWidget     *vbox;
	...
	doc_name = scrivi_document_get_short_name_for_display (doc);
...
static GtkWidget *
create_treeview (ScriviCloseConfirmationDialogPrivate *priv)
{
	GtkListStore *store;
	GtkWidget *treeview;
...
static void
build_multiple_docs_dialog (ScriviCloseConfirmationDialog *dlg)
{
	ScriviCloseConfirmationDialogPrivate *priv;
	GtkWidget *hbox;
	GtkWidget *image;
...
static void
set_unsaved_document (ScriviCloseConfirmationDialog *dlg,
		      const GList                  *list)
{
	ScriviCloseConfirmationDialogPrivate *priv;
...
const GList *
scrivi_close_confirmation_dialog_get_unsaved_documents (ScriviCloseConfirmationDialog *dlg)
{
	g_return_val_if_fail (SCRIVI_IS_CLOSE_CONFIRMATION_DIALOG (dlg), NULL);
