...
/*
 * Modified by the scrivi Team, 2001-2003. See the AUTHORS file for a 
...
#define SCRIVI_PREFERENCES_DIALOG_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						     SCRIVI_TYPE_PREFERENCES_DIALOG, \
						     ScriviPreferencesDialogPrivate))

struct _ScriviPreferencesDialogPrivate
{
	GtkWidget	*notebook;
...
G_DEFINE_TYPE(ScriviPreferencesDialog, scrivi_preferences_dialog, GTK_TYPE_DIALOG)


static void 
scrivi_preferences_dialog_class_init (ScriviPreferencesDialogClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (object_class, sizeof (ScriviPreferencesDialogPrivate));
}
			 ...
			 gint       res_id)
{
	...
	switch (res_id)
	{
		case GTK_RESPONSE_HELP:
			scrivi_help_display (GTK_WINDOW (dlg),
					    NULL,
					    "scrivi-prefs");
...
tabs_width_spinbutton_value_changed (GtkSpinButton          *spin_button,
				     ScriviPreferencesDialog *dlg)
{
	scrivi_debug (DEBUG_PREFS);

	g_return_if_fail (spin_button == GTK_SPIN_BUTTON (dlg->priv->tabs_width_spinbutton));

	scrivi_prefs_manager_set_tabs_size (gtk_spin_button_get_value_as_int (spin_button));
}
				   ...
				   ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_insert_spaces (gtk_toggle_button_get_active (button));
}
				 ...
				 ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_auto_indent (gtk_toggle_button_get_active (button));
}
			       ...
			       ScriviPreferencesDialog *dlg)
{
	...
	if (gtk_toggle_button_get_active (button))
	{
		gtk_widget_set_sensitive (dlg->priv->auto_save_spinbutton, 
					  scrivi_prefs_manager_auto_save_interval_can_set());

		scrivi_prefs_manager_set_auto_save (TRUE);
	}
	else	
	{
		gtk_widget_set_sensitive (dlg->priv->auto_save_spinbutton, FALSE);
		scrivi_prefs_manager_set_auto_save (FALSE);
	}
}
				 ...
				 ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_create_backup_copy (gtk_toggle_button_get_active (button));
}
				    ...
				    ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_auto_save_interval (
			MAX (1, gtk_spin_button_get_value_as_int (spin_button)));
}

static void
setup_editor_page (ScriviPreferencesDialog *dlg)
{
	gboolean auto_save;
					  ...
					  ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_display_line_numbers (gtk_toggle_button_get_active (button));
}
					    ...
					    ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_highlight_current_line (gtk_toggle_button_get_active (button));
}
				      ...
				      ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_bracket_matching (
			       ...
			       ScriviPreferencesDialog *dlg)
{
	...
	{
		scrivi_prefs_manager_set_wrap_mode (GTK_WRAP_NONE);
	...
	else
	{
		...
		if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (dlg->priv->split_checkbutton)))
		{
			...
			scrivi_prefs_manager_set_wrap_mode (GTK_WRAP_WORD);
		}
		else
		{
			split_button_state = FALSE;
			
			scrivi_prefs_manager_set_wrap_mode (GTK_WRAP_CHAR);
		}
	}
}
				  ...
				  ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_display_right_margin (active);

	gtk_widget_set_sensitive (dlg->priv->right_margin_position_hbox,
				  active && 
				  scrivi_prefs_manager_right_margin_position_can_set ());
}
						...
						ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_right_margin_position (value);
}

static void
setup_view_page (ScriviPreferencesDialog *dlg)
{
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (dlg->priv->display_line_numbers_checkbutton),
				      scrivi_prefs_manager_get_display_line_numbers ());
	...
	display_right_margin = scrivi_prefs_manager_get_display_right_margin ();
	...
	gtk_spin_button_set_value (
		...
		(guint)CLAMP (scrivi_prefs_manager_get_right_margin_position (), 1, 160));
		
	/* Set widgets sensitivity */
	gtk_widget_set_sensitive (dlg->priv->display_line_numbers_checkbutton,
				  scrivi_prefs_manager_display_line_numbers_can_set ());
	gtk_widget_set_sensitive (dlg->priv->highlight_current_line_checkbutton,
				  scrivi_prefs_manager_highlight_current_line_can_set ());
	gtk_widget_set_sensitive (dlg->priv->bracket_matching_checkbutton,
				  scrivi_prefs_manager_bracket_matching_can_set ());
	wrap_mode_can_set = scrivi_prefs_manager_wrap_mode_can_set ();
	...
	gtk_widget_set_sensitive (dlg->priv->right_margin_checkbutton,
				  scrivi_prefs_manager_display_right_margin_can_set ());
	gtk_widget_set_sensitive (dlg->priv->right_margin_position_hbox,
				  display_right_margin && 
				  scrivi_prefs_manager_right_margin_position_can_set ());
				       ...
				       ScriviPreferencesDialog *dlg)
{
	...
	if (gtk_toggle_button_get_active (button))
	{
		...
		scrivi_prefs_manager_set_use_default_font (TRUE);
	}
	else
	{
		gtk_widget_set_sensitive (dlg->priv->font_hbox, 
					  scrivi_prefs_manager_editor_font_can_set ());
		scrivi_prefs_manager_set_use_default_font (FALSE);
	}
}
			     ...
			     ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_editor_font (font_name);
}

static void
setup_font_colors_page_font_section (ScriviPreferencesDialog *dlg)
{
	...
	scrivi_utils_set_atk_relation (dlg->priv->font_button,
				      dlg->priv->default_font_checkbutton,
				      ATK_RELATION_CONTROLLED_BY);
	scrivi_utils_set_atk_relation (dlg->priv->default_font_checkbutton,
				      dlg->priv->font_button, 
				      ATK_RELATION_CONTROLLER_FOR);	

	editor_font = scrivi_prefs_manager_get_system_font ();
	...
	use_default_font = scrivi_prefs_manager_get_use_default_font ();
	...
	gtk_widget_set_sensitive (dlg->priv->default_font_checkbutton, 
				  scrivi_prefs_manager_use_default_font_can_set ());
...
static void
set_buttons_sensisitivity_according_to_scheme (ScriviPreferencesDialog *dlg,
					       const gchar            *scheme_id)
{
	gboolean editable;
	
	editable = (scheme_id != NULL) && 
	           _scrivi_style_scheme_manager_scheme_is_scrivi_user_scheme (
		      ...
		      ScriviPreferencesDialog *dlg)
{
	...
	scrivi_prefs_manager_set_source_style_scheme (id);
...
ensure_color_scheme_id (const gchar *id)
{
	...
	GtkSourceStyleSchemeManager *manager = scrivi_get_style_scheme_manager ();

	if (id == NULL)
	{
		gchar *pref_id;

		pref_id = scrivi_prefs_manager_get_source_style_scheme ();
 ...
 * scrivi_style_scheme_manager_get_default_scheme. If this one returns NULL
 * use the first available scheme as default */
static const gchar *
populate_color_scheme_list (ScriviPreferencesDialog *dlg, const gchar *def_id)
{
	GSList *schemes;
	GSList *l;
	...
	schemes = scrivi_style_scheme_manager_list_schemes_sorted (scrivi_get_style_scheme_manager ());
	l = schemes;
	while (l != NULL)
	{
				...
				ScriviPreferencesDialog *dlg)
{
	...
	scheme_id = _scrivi_style_scheme_manager_install_scheme (
					scrivi_get_style_scheme_manager (),
					filename);
	g_free (filename);
	...
	if (scheme_id == NULL)
	{
		scrivi_warning (GTK_WINDOW (dlg),
			       _("The selected color scheme cannot be installed."));

		return;
	}

	scrivi_prefs_manager_set_source_style_scheme (scheme_id);
			...
			ScriviPreferencesDialog *dlg)
{
	...
	scrivi_dialog_add_button (GTK_DIALOG (chooser), 
				 _("A_dd Scheme"),
				 GTK_STOCK_ADD,
				 GTK_RESPONSE_ACCEPT);
			  ...
			  ScriviPreferencesDialog *dlg)
{
					     ...
					     &iter))
	{
		...
		if (!_scrivi_style_scheme_manager_uninstall_scheme (scrivi_get_style_scheme_manager (), id))
		{
			scrivi_warning (GTK_WINDOW (dlg),
		...
		{	
			...
			if (real_new_id != NULL)
				scrivi_prefs_manager_set_source_style_scheme (real_new_id);
		}
...
static void
setup_font_colors_page_style_scheme_section (ScriviPreferencesDialog *dlg)
{
	GtkCellRenderer *renderer;
...
static void
setup_font_colors_page (ScriviPreferencesDialog *dlg)
{
...
static void
setup_plugins_page (ScriviPreferencesDialog *dlg)
{
	GtkWidget *page_content;

	scrivi_debug (DEBUG_PREFS);

	page_content = scrivi_plugin_manager_new ();
...
static void
scrivi_preferences_dialog_init (ScriviPreferencesDialog *dlg)
{
	...
	dlg->priv = SCRIVI_PREFERENCES_DIALOG_GET_PRIVATE (dlg);
	...
	gtk_window_set_title (GTK_WINDOW (dlg), _("Scrivi Preferences"));
	...
	file = scrivi_dirs_get_ui_file ("scrivi-preferences-dialog.ui");
	ret = scrivi_utils_get_ui_objects (file,
		root_objects,
		&error_widget,
...
void
scrivi_show_preferences_dialog (ScriviWindow *parent)
{
	scrivi_debug (DEBUG_PREFS);

	g_return_if_fail (SCRIVI_IS_WINDOW (parent));

	if (preferences_dialog == NULL)
	{
		preferences_dialog = GTK_WIDGET (g_object_new (SCRIVI_TYPE_PREFERENCES_DIALOG, NULL));
