...
#include "scrivi-search-dialog.h"
#include "scrivi-history-entry.h"
#include "scrivi-utils.h"
#include "scrivi-marshal.h"
#include "scrivi-dirs.h"

#define SCRIVI_SEARCH_DIALOG_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						SCRIVI_TYPE_SEARCH_DIALOG,              \
						ScriviSearchDialogPrivate))
...
struct _ScriviSearchDialogPrivate 
{
...
G_DEFINE_TYPE(ScriviSearchDialog, scrivi_search_dialog, GTK_TYPE_DIALOG)
...
static void
scrivi_search_dialog_set_property (GObject      *object,
				  guint         prop_id,
				  const GValue *value,
				  GParamSpec   *pspec)
{
	ScriviSearchDialog *dlg = SCRIVI_SEARCH_DIALOG (object);

	switch (prop_id)
	{
		case PROP_SHOW_REPLACE:
			scrivi_search_dialog_set_show_replace (dlg,
...
static void
scrivi_search_dialog_get_property (GObject    *object,
				  guint       prop_id,
				  GValue     *value,
				  GParamSpec *pspec)
{
...
void
scrivi_search_dialog_present_with_time (ScriviSearchDialog *dialog,
				       guint32            timestamp)
{
	g_return_if_fail (SCRIVI_SEARCH_DIALOG (dialog));
...
static gboolean
show_replace (ScriviSearchDialog *dlg)
{
	scrivi_search_dialog_set_show_replace (dlg, TRUE);
	
	return TRUE;
}

static void 
scrivi_search_dialog_class_init (ScriviSearchDialogClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkBindingSet *binding_set;
	
	object_class->set_property = scrivi_search_dialog_set_property;
	object_class->get_property = scrivi_search_dialog_get_property;
	...
	dialog_signals[SHOW_REPLACE] =
    		g_signal_new ("show_replace",
		  	      ...
		  	      G_STRUCT_OFFSET (ScriviSearchDialogClass, show_replace),
			      NULL, NULL,
	...
	g_type_class_add_private (object_class, sizeof (ScriviSearchDialogPrivate));

	binding_set = gtk_binding_set_by_class (klass);
	
	/* Note: we cannot use the keyval/modifier associated with the 
	 * GTK_STOCK_FIND_AND_REPLACE stock item since GDE2 HIG suggests Ctrl+h
			   ...
			   ScriviSearchDialog *dialog)
{
	...
	if (*search_string != '\0')
	{
		gtk_dialog_set_response_sensitive (GTK_DIALOG (dialog), 
			SCRIVI_SEARCH_DIALOG_FIND_RESPONSE, TRUE);
		gtk_dialog_set_response_sensitive (GTK_DIALOG (dialog), 
			SCRIVI_SEARCH_DIALOG_REPLACE_ALL_RESPONSE, TRUE);
	}
	else
	{
		gtk_dialog_set_response_sensitive (GTK_DIALOG (dialog), 
			SCRIVI_SEARCH_DIALOG_FIND_RESPONSE, FALSE);
		gtk_dialog_set_response_sensitive (GTK_DIALOG (dialog), 
			SCRIVI_SEARCH_DIALOG_REPLACE_RESPONSE, FALSE);
		gtk_dialog_set_response_sensitive (GTK_DIALOG (dialog), 
			SCRIVI_SEARCH_DIALOG_REPLACE_ALL_RESPONSE, FALSE);
	}
}

static void
response_handler (ScriviSearchDialog *dialog,
		  gint               response_id,
		  gpointer           data)
{
	const gchar *str;
	...
	switch (response_id)
	{
		...
		case SCRIVI_SEARCH_DIALOG_REPLACE_ALL_RESPONSE:
			...
			if (*str != '\0')
			{
				...
				scrivi_history_entry_prepend_text
						(SCRIVI_HISTORY_ENTRY (dialog->priv->replace_entry),
						 text);
		...
		case SCRIVI_SEARCH_DIALOG_FIND_RESPONSE:
			...
			if (*str != '\0')
			{
				...
				text = scrivi_utils_unescape_search_text (str);
				scrivi_history_entry_prepend_text
						(SCRIVI_HISTORY_ENTRY (dialog->priv->search_entry),
						 text);
...
static void
show_replace_widgets (ScriviSearchDialog *dlg,
		      gboolean           show_replace)
{
	if (show_replace)
	{
...
static void
scrivi_search_dialog_init (ScriviSearchDialog *dlg)
{
	GtkWidget *content;
	...
	dlg->priv = SCRIVI_SEARCH_DIALOG_GET_PRIVATE (dlg);
...
#if GTK_CHECK_VERSION (3, 0, 0)
	file = scrivi_dirs_get_ui_file ("scrivi-search-dialog-gtk3.ui");
#else
	file = scrivi_dirs_get_ui_file ("scrivi-search-dialog.ui");
#endif
	...
	dlg->priv->search_entry = scrivi_history_entry_new ("history-search-for",
							   TRUE);
	gtk_widget_set_size_request (dlg->priv->search_entry, 300, -1);
	scrivi_history_entry_set_escape_func
			(SCRIVI_HISTORY_ENTRY (dlg->priv->search_entry),
			 (ScriviHistoryEntryEscapeFunc) scrivi_utils_escape_search_text);

#if GTK_CHECK_VERSION (3, 0, 0)
	gtk_widget_set_hexpand (GTK_WIDGET (dlg->priv->search_entry), TRUE);
#endif
	dlg->priv->search_text_entry = scrivi_history_entry_get_entry
			(SCRIVI_HISTORY_ENTRY (dlg->priv->search_entry));
	...
	dlg->priv->replace_entry = scrivi_history_entry_new ("history-replace-with",
							    TRUE);
	scrivi_history_entry_set_escape_func
			(SCRIVI_HISTORY_ENTRY (dlg->priv->replace_entry),
			 (ScriviHistoryEntryEscapeFunc) scrivi_utils_escape_search_text);

#if GTK_CHECK_VERSION (3, 0, 0)
	gtk_widget_set_hexpand (GTK_WIDGET (dlg->priv->replace_entry), TRUE);
#endif
	dlg->priv->replace_text_entry = scrivi_history_entry_get_entry
			(SCRIVI_HISTORY_ENTRY (dlg->priv->replace_entry));
	...
	dlg->priv->replace_button = scrivi_gtk_button_new_with_stock_icon (_("_Replace"),
	...
	gtk_dialog_add_action_widget (GTK_DIALOG (dlg),
				      ...
				      SCRIVI_SEARCH_DIALOG_FIND_RESPONSE);
...
GtkWidget *
scrivi_search_dialog_new (GtkWindow *parent,
			 gboolean   show_replace)
{
	ScriviSearchDialog *dlg;

	dlg = g_object_new (SCRIVI_TYPE_SEARCH_DIALOG,
...
gboolean
scrivi_search_dialog_get_show_replace (ScriviSearchDialog *dialog)
{
	g_return_val_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog), FALSE);

	return dialog->priv->show_replace;
}

void
scrivi_search_dialog_set_show_replace (ScriviSearchDialog *dialog,
				      gboolean           show_replace)
{
	g_return_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog));
...
void
scrivi_search_dialog_set_search_text (ScriviSearchDialog *dialog,
				     const gchar       *text)
{
	g_return_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog));
 ...
 */
const gchar *
scrivi_search_dialog_get_search_text (ScriviSearchDialog *dialog)
{
	g_return_val_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog), NULL);

	return gtk_entry_get_text (GTK_ENTRY (dialog->priv->search_text_entry));
}

void
scrivi_search_dialog_set_replace_text (ScriviSearchDialog *dialog,
				      const gchar       *text)
{
	g_return_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog));
	g_return_if_fail (text != NULL);
...
const gchar *
scrivi_search_dialog_get_replace_text (ScriviSearchDialog *dialog)
{
	g_return_val_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog), NULL);
...
void
scrivi_search_dialog_set_match_case (ScriviSearchDialog *dialog,
				    gboolean           match_case)
{
	g_return_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog));
...
gboolean
scrivi_search_dialog_get_match_case (ScriviSearchDialog *dialog)
{
	g_return_val_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog), FALSE);
...
void
scrivi_search_dialog_set_entire_word (ScriviSearchDialog *dialog,
...
gboolean
scrivi_search_dialog_get_entire_word (ScriviSearchDialog *dialog)
{
	g_return_val_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog), FALSE);
...
void
scrivi_search_dialog_set_backwards (ScriviSearchDialog *dialog,
				  gboolean           backwards)
{
	g_return_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog));
...
gboolean
scrivi_search_dialog_get_backwards (ScriviSearchDialog *dialog)
{
	g_return_val_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog), FALSE);
...
void
scrivi_search_dialog_set_wrap_around (ScriviSearchDialog *dialog,
...
gboolean
scrivi_search_dialog_get_wrap_around (ScriviSearchDialog *dialog)
{
	g_return_val_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog), FALSE);
...
void
scrivi_search_dialog_set_parse_escapes (ScriviSearchDialog *dialog,
...
gboolean
scrivi_search_dialog_get_parse_escapes (ScriviSearchDialog *dialog)
{
	g_return_val_if_fail (SCRIVI_IS_SEARCH_DIALOG (dialog), FALSE);
