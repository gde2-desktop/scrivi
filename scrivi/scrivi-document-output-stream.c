...
#define SCRIVI_DOCUMENT_OUTPUT_STREAM_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE((object),\
							 SCRIVI_TYPE_DOCUMENT_OUTPUT_STREAM,\
							 ScriviDocumentOutputStreamPrivate))

#define MAX_UNICHAR_LEN 6

struct _ScriviDocumentOutputStreamPrivate
{
	ScriviDocument *doc;
	GtkTextIter    pos;
...
G_DEFINE_TYPE (ScriviDocumentOutputStream, scrivi_document_output_stream, G_TYPE_OUTPUT_STREAM)

static gssize	scrivi_document_output_stream_write (GOutputStream            *stream,
						    const void               *buffer,
						    gsize                     count,
						    GCancellable             *cancellable,
						    GError                  **error);

static gboolean scrivi_document_output_stream_flush (GOutputStream *stream,
                                                    GCancellable  *cancellable,
                                                    GError        **error);

static gboolean	scrivi_document_output_stream_close (GOutputStream     *stream,
						    GCancellable      *cancellable,
						    GError           **error);

static void
scrivi_document_output_stream_set_property (GObject      *object,
					   guint         prop_id,
					   const GValue *value,
					   GParamSpec   *pspec)
{
	ScriviDocumentOutputStream *stream = SCRIVI_DOCUMENT_OUTPUT_STREAM (object);

	switch (prop_id)
	{
		case PROP_DOCUMENT:
			stream->priv->doc = SCRIVI_DOCUMENT (g_value_get_object (value));
			break;
...
static void
scrivi_document_output_stream_get_property (GObject    *object,
					   guint       prop_id,
					   GValue     *value,
					   GParamSpec *pspec)
{
...
static void
scrivi_document_output_stream_finalize (GObject *object)
{
	ScriviDocumentOutputStream *stream = SCRIVI_DOCUMENT_OUTPUT_STREAM (object);

	g_free (stream->priv->buffer);

	G_OBJECT_CLASS (scrivi_document_output_stream_parent_class)->finalize (object);
}

static void
scrivi_document_output_stream_constructed (GObject *object)
{
	ScriviDocumentOutputStream *stream = SCRIVI_DOCUMENT_OUTPUT_STREAM (object);
...
static void
scrivi_document_output_stream_class_init (ScriviDocumentOutputStreamClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GOutputStreamClass *stream_class = G_OUTPUT_STREAM_CLASS (klass);

	object_class->get_property = scrivi_document_output_stream_get_property;
	object_class->set_property = scrivi_document_output_stream_set_property;
	object_class->finalize = scrivi_document_output_stream_finalize;
	object_class->constructed = scrivi_document_output_stream_constructed;

	stream_class->write_fn = scrivi_document_output_stream_write;
	stream_class->flush = scrivi_document_output_stream_flush;
	stream_class->close_fn = scrivi_document_output_stream_close;
	...
	g_type_class_add_private (object_class, sizeof (ScriviDocumentOutputStreamPrivate));
}

static void
scrivi_document_output_stream_init (ScriviDocumentOutputStream *stream)
{
	stream->priv = SCRIVI_DOCUMENT_OUTPUT_STREAM_GET_PRIVATE (stream);
...
static ScriviDocumentNewlineType
get_newline_type (GtkTextIter *end)
{
	ScriviDocumentNewlineType res;
	GtkTextIter copy;
	gunichar c;
	...
	if (g_unichar_break_type (c) == G_UNICODE_BREAK_CARRIAGE_RETURN)
	{
		...
		{
			res = SCRIVI_DOCUMENT_NEWLINE_TYPE_CR_LF;
		}
		else
		{
			res = SCRIVI_DOCUMENT_NEWLINE_TYPE_CR;
		}
	}
	else
	{
		res = SCRIVI_DOCUMENT_NEWLINE_TYPE_LF;
	}
...
GOutputStream *
scrivi_document_output_stream_new (ScriviDocument *doc)
{
	return G_OUTPUT_STREAM (g_object_new (SCRIVI_TYPE_DOCUMENT_OUTPUT_STREAM,
					      "document", doc, NULL));
}

ScriviDocumentNewlineType
scrivi_document_output_stream_detect_newline_type (ScriviDocumentOutputStream *stream)
{
	ScriviDocumentNewlineType type;
	GtkTextIter iter;

	g_return_val_if_fail (SCRIVI_IS_DOCUMENT_OUTPUT_STREAM (stream),
			      SCRIVI_DOCUMENT_NEWLINE_TYPE_DEFAULT);

	type = SCRIVI_DOCUMENT_NEWLINE_TYPE_DEFAULT;
...
static void
remove_ending_newline (ScriviDocumentOutputStream *stream)
{
	GtkTextIter end;
	GtkTextIter start;
...
static void
end_append_text_to_document (ScriviDocumentOutputStream *stream)
{
...
static gssize
scrivi_document_output_stream_write (GOutputStream            *stream,
				    const void               *buffer,
				    gsize                     count,
				    GCancellable             *cancellable,
				    GError                  **error)
{
	ScriviDocumentOutputStream *ostream;
	gchar *text;
	gsize len;
	...
	ostream = SCRIVI_DOCUMENT_OUTPUT_STREAM (stream);
...
static gboolean
scrivi_document_output_stream_flush (GOutputStream *stream,
                                    GCancellable  *cancellable,
                                    GError        **error)
{
	ScriviDocumentOutputStream *ostream = SCRIVI_DOCUMENT_OUTPUT_STREAM (stream);

	/* Flush deferred data if some. */
	if (!ostream->priv->is_closed && ostream->priv->is_initialized &&
	    ostream->priv->buflen > 0 &&
	    scrivi_document_output_stream_write (stream, "", 0, cancellable,
						error) == -1)
		return FALSE;
...
static gboolean
scrivi_document_output_stream_close (GOutputStream     *stream,
				    GCancellable      *cancellable,
				    GError           **error)
{
	ScriviDocumentOutputStream *ostream = SCRIVI_DOCUMENT_OUTPUT_STREAM (stream);
